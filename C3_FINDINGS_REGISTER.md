...
**As of:** 6 August 2026, 01:10 UTC, `eb7114b` at the time of writing.
...
- **Pokemon card pages are missing their HP, Weakness and Retreat Cost blocks,
  all 31,833 of them**, because all six enrichment columns are empty and no row
  has been updated since 29 July, while prices display normally. Root cause
  found in Task 19 (C3L-61): card writes are gated on `isNewSet` and all 235
  sets are marked synced, so the gate is closed permanently. **Not fixed, on
  measured grounds**, pokemon's sync peaks at 11.9 minutes against a 15 minute
  cap. Needs a bounded backfill as its own scheduled function. The sync health
  check does NOT catch this and, per the note in Section 3, should get a
  separate signal rather than a widened one.
...
## 3. Confirmed findings, live investigation (IDs C3L-01 to C3L-62)
