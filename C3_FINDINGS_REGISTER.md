...
**As of:** 6 August 2026, 02:05 UTC, `901a93d` at the time of writing.
...
**Immediate next action:** **check the C3L-61 backfill's first run, any time after 06:40 UTC today.** It has never executed. Expected: `sync_events` shows `pokemon-enrichment` `sync_start` then `sync_success`, `pokemon_enrichment_progress` gains up to 10 rows, and `pokemon_cards` gains non-zero `hp`. A start with no terminal event is the C3L-57 shape and means the 5 minute budget needs CUTTING, not raising. Signal B flags that automatically. Beyond that, no task file is pending. **Two dated checkpoints
...
- **Pokemon card pages are still missing their HP, Weakness and Retreat Cost
  ...
  (C3L-61) but **has not run once**, because Netlify refuses direct invocation
  of scheduled functions, so the first execution is the schedule at 06:40 UTC
  today. Until then nothing has changed on the site. Signal C in the health
  check now detects this condition and was verified doing so.
