...
async function handleSetPage(setSlug, htmlHeaders) {
  ...
  const [_psr0, _psr1] = await Promise.allSettled([
    supabaseGet(`onepiece_sets?slug=eq.${encodeURIComponent(setSlug)}&limit=1&select=*`),
    getEbayToken()
  ]);
  ...
  const ebaySetURL = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name + ' onepiece')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
...
<div class="wrap">
  ...
  <div class="cta-row">
    ...
    <a href="https://www.amazon.com.au/s?k=${encodeURIComponent(set.name + ' One Piece')}&tag=${AMAZON_TAG}" target="_blank" rel="noopener" class="cta-btn cta-secondary" style="border-color:rgba(255,153,0,.35);color:#ff9900">Amazon AU →</a>
  </div>
...
export default async (req) => {
  ...
  try {
    const cards = await supabaseGet(`onepiece_cards?slug=eq.${encodeURIComponent(slug)}&limit=1&select=*`);
    ...
    const snapshots = await supabaseGet(`onepiece_price_snapshots?card_id=eq.${encodeURIComponent(card.id)}&snapshot_date=gte.${new Date(Date.now()-90*86400000).toISOString().slice(0,10)}&order=snapshot_date.asc&limit=400&select=snapshot_date,price_aud,market_price`).catch(() => []);
    const priceChartHTML = priceChartHtml(snapshots);
    const _setRows = card.set_id ? await supabaseGet(`onepiece_sets?id=eq.${card.set_id}&limit=1&select=slug`).catch(() => []) : [];
    const setUrl = (Array.isArray(_setRows) && _setRows[0] && _setRows[0].slug) ? `/cards/onepiece/sets/${_setRows[0].slug}` : `/cards/onepiece`;
    ...
    const ebaySearchUrl = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(card.name+' onepiece card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;

    const [_psr0, _psr1] = await Promise.allSettled([
      supabaseGet(`onepiece_cards?set_id=eq.${card.set_id}&slug=neq.${encodeURIComponent(slug)}&market_price=gt.0&image_url=not.is.null&limit=12&order=market_price.desc&select=*`).catch(() => []),
...
${followBlockHtml({ game: 'onepiece', slug: card.slug, cardName: card.name, printingId: card.tcgplayer_id })}
...
${(() => {
  ...
  const boxes = [];
  const isLeader = ca.card_type === 'Leader';
  if (isLeader && ca.life != null) boxes.push(box('Life', esc(ca.life)));
  if (!isLeader && ca.card_cost != null) boxes.push(box('Cost', esc(ca.card_cost)));
  if (ca.card_power != null) boxes.push(box('Power', esc(ca.card_power)));
  if (!isLeader && ca.counter_amount != null) boxes.push(box('Counter', esc(ca.counter_amount)));
  if (ca.card_color) boxes.push(box('Color', esc(ca.card_color)));
  if (ca.attribute) boxes.push(box('Attribute', esc(ca.attribute)));
  if (ca.card_type) boxes.push(box('Type', esc(ca.card_type)));
...
${viewTrackingScript('onepiece', card.slug)}
</body>
</html>`;
