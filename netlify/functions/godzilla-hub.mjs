...
const EMOJI       = '&#129442;';
const SEARCH_PH   = "Godzilla, Mothra, King Ghidorah...";
const SET_PH      = "Kaiju War, Destruction...";
...
export default async (req) => {
  ...
  const [setsResult, topCardsResult] = await Promise.allSettled([
    supabaseGet('godzilla_sets?order=release_date.desc&limit=20&select=id,name,slug,abbreviation,release_date,card_count'),
    supabaseGet('godzilla_cards?order=market_price.desc&market_price=gt.0.5&market_price=lt.200&image_url=not.is.null&limit=24&select=slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at')
  ]);
  ...
  const carouselHTML = topCards.map(c => {
    ...
    const ebay  = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name+' godzilla card game')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<a href="/cards/godzilla/${c.slug}" class="carousel-card">
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/godzilla/sets/${encodeURIComponent(s.slug||s.id)}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}">
...
<head>
  ...
  <meta name="description" content="Browse ${sets.length||'8'}+ Godzilla TCG sets. Live AUD card prices and eBay AU buy links updated daily. Australia's Godzilla TCG price guide.">
  <link rel="canonical" href="${CANONICAL}">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="Godzilla TCG Card Prices Australia | Cards on Cards on Cards">
  <meta property="og:description" content="${sets.length||'8'}+ Godzilla TCG sets with live AUD prices and eBay AU buy links.">
...
<!-- OUT OF PRINT NOTICE -->
  <div style="background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.25);border-radius:10px;padding:14px 18px;margin:0 24px 28px;display:flex;align-items:center;gap:12px">
    ...
    <div>
      ...
      <div style="font-size:13px;color:rgba(160,168,192,.75);line-height:1.5">Godzilla TCG is no longer in print. Cards shown are from the collector&#39;s secondary market. Prices reflect current eBay AU and TCGPlayer market values.</div>
    </div>
  </div>
...
<div class="hero fade-up">
  ...
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length||'8'}</div><div class="stat-label">Sets</div></div>
    <div class="stat-item"><div class="stat-num">600+</div><div class="stat-label">Cards</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=godzilla+card+game&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=5339146789&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:var(--accent);color:#000">&#128722; Shop Godzilla TCG on eBay &#8599;</a>
  ...
  <a href="/blog/godzilla-tcg-beginners-guide-australia/" class="quick-link" style="background:rgba(var(--accent-rgb),.08);border-color:rgba(var(--accent-rgb),.3);color:var(--accent)">&#128214; Beginners Guide &#8594;</a>
  <a href="/blog/best-godzilla-tcg-cards-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128230; Most Valuable Cards &#8594;</a>
</div>
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header"><div class="section-title">Search Godzilla TCG Cards</div></div>
  ...
  <div class="section fade-up fade-up-3">
    <div class="section-header">
      <div class="section-title">Browse ${sets.length||'8'} Sets</div>
...
<footer>
  ...
  <p style="margin-top:6px;font-size:11px;opacity:.5">Affiliate disclosure: this site earns commissions from eBay AU and Amazon AU purchases made through affiliate links at no extra cost to you. Not affiliated with Toho or Legendary Pictures. USD prices converted to AUD at approximately 1.58.</p>
</footer>
...
async function searchCard() {
  ...
  try {
    const res = await fetch('/api/compare-search?q=' + encodeURIComponent(q) + '&game=godzilla&limit=24');
    ...
    results.innerHTML = cards.map(c => {
      ...
      return '<a href="/cards/godzilla/'+c.slug+'" style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center;display:block;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'var(--border)\'">'
