...
// netlify/functions/vanguard-set-page.mjs
// C3 set-page v4 -- full MVP rebuild
// Serves /cards/vanguard/sets/:slug+
...
<head>
  ...
  <title>Set Not Found | Cardfight Vanguard | Cards on Cards on Cards</title>
...
<body><div class="wrap"><div class="icon">🃏</div><h1>Set Not Found</h1><p>We could not find the Cardfight Vanguard set "${setSlug}". It may not be in our database yet.</p><a href="/cards/vanguard" class="btn">Browse All Vanguard Cards</a><a href="/" class="btn btn-sec">← Home</a></div></body>
</html>`;
}
...
export default async (req) => {
  ...
  const setSlug = decodeSlugSegment(url.pathname.replace(/^\/cards\/vanguard\/sets\//, '').replace(/\/$/, ''));
  ...
  try {
    const [setsR, ebayTokenR] = await Promise.allSettled([
      supabaseGet(`vanguard_sets?slug=eq.${encodeURIComponent(setSlug)}&limit=1`),
      getEbayToken()
    ]);
    ...
    const legacy = await numericSetRedirect(setSlug, 'vanguard_sets', '/cards/vanguard/sets', supabaseGet)
    ...
    const [cardsR, sealedR, ebayListingsR] = await Promise.allSettled([
      supabaseGet(`vanguard_cards?set_id=eq.${set.id}&rarity=neq.None&order=name.asc&limit=200&select=slug,name,number,image_url,market_price,price_aud,rarity,set_name,price_change_7d,price_change_30d`),
      ...
      supabaseGet(`vanguard_cards?set_id=eq.${set.id}&or=(rarity.eq.None,rarity.is.null)&order=market_price.desc.nullslast&limit=300&select=slug,name,image_url,market_price,price_aud,low_price`),
      getEbayListings(`${set.name} cardfight vanguard card`, ebayToken)
    ]);
    ...
    function moverCard(c, isGainer) {
      ...
      return `<a href="/cards/vanguard/${c.slug}" style="background:#0e1118;border:1px solid #1e2235;border-radius:8px;padding:10px 12px;text-decoration:none;display:flex;align-items:center;gap:10px;transition:border-color .2s" onmouseover="this.style.borderColor='${col}'" onmouseout="this.style.borderColor='#1e2235'">
    ...
    <div style="background:linear-gradient(135deg,rgba(16,185,129,.06),rgba(16,185,129,.02));border:1px solid rgba(16,185,129,.2);border-radius:12px;padding:20px;margin-bottom:28px">
      ...
      const aud = toAud(c);
      return `<a href="/cards/vanguard/${c.slug}" style="flex:0 0 140px;background:#0e1118;border:1px solid rgba(16,185,129,.35);border-radius:10px;padding:10px;text-align:center;text-decoration:none;transition:all .2s;display:block" onmouseover="this.style.borderColor='#10B981';this.style.transform='translateY(-2px)'" onmouseout="this.style.borderColor='rgba(16,185,129,.35)';this.style.transform='none'">
    ...
    const metaDesc = `Browse ${cards?.length||0} Cardfight Vanguard cards from ${set.name}. View card prices in AUD, find the most valuable cards and buy on eBay AU. Updated daily.`;


    const setSchemaLD = {
      "@context": "https://schema.org", "@type": "CollectionPage",
      "name": `${set.name} Cardfight Vanguard Card Prices Australia`,
      "description": `Browse all ${cards?.length||0} ${set.name} Cardfight Vanguard cards with AUD prices and eBay AU buy links.`,
      "url": `https://cardsoncardsoncards.com.au/cards/vanguard/sets/${setSlug}`,
      "breadcrumb": {
        "@type": "BreadcrumbList",
        "itemListElement": [
          { "@type": "ListItem", "position": 1, "name": "Home", "item": "https://cardsoncardsoncards.com.au" },
          { "@type": "ListItem", "position": 2, "name": "Cardfight Vanguard", "item": "https://cardsoncardsoncards.com.au/cards/vanguard" },
          { "@type": "ListItem", "position": 3, "name": set.name, "item": `https://cardsoncardsoncards.com.au/cards/vanguard/sets/${setSlug}` }
        ]
      }
    };
...
<head>
  ...
  <meta property="og:url" content="https://cardsoncardsoncards.com.au/cards/vanguard/sets/${setSlug}">
...
<div class="wrap">
  ...
  <div class="cta-row">
    <a href="${ebaySetURL}" target="_blank" rel="noopener" class="cta-btn cta-primary" onclick="if(typeof gtag!=='undefined')gtag('event','ebay_set_click',{set_name:'${set.name}',game:'vanguard'})">Buy Cards on eBay AU →</a>
  ...
  ${(() => {
    ...
    const itemsHTML = sealedItems.slice(0,4).map(p => {
      ...
      return `<a href="/cards/vanguard/${p.slug}" style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:14px;display:flex;flex-direction:column;gap:8px;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor='#EF4444'" onmouseout="this.style.borderColor='#1e2235'">
    ...
    return `<div class="section" style="background:#EF44440a;border:1px solid #EF444426;border-radius:14px;padding:22px 24px;margin-bottom:32px">
      ...
      <a href="https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent((set.name||'')+' cardfight vanguard booster')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:8px;font-weight:700;font-size:12px;text-decoration:none;background:#EF4444;color:#fff">&#128722; More sealed on eBay AU &#8599;</a>
    </div>`;
  })()}
  ...
  <div style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:20px;font-size:13px;color:#8892b0">
    <strong style="color:#F0F2FF">About this set:</strong> Cardfight Vanguard card prices in AUD, converted from USD at approximately ${audRate.toFixed(2)}x. Updated daily.
    <div style="margin-top:10px"><a href="/cards/vanguard" style="color:#10B981">Browse all Vanguard sets</a></div>
  </div>
</div>

<script>
if(typeof gtag!=='undefined'){
  document.querySelectorAll('a[href*="ebay"]').forEach(a=>a.addEventListener('click',()=>gtag('event','ebay_click',{game:'vanguard',set:'${set.name}'})));
}
...
<script>(function(){var urlInput=document.getElementById('bugPageUrl');if(urlInput)urlInput.value=window.location.href;var form=document.getElementById('bugReportForm');if(!form)return;form.addEventListener('submit',function(e){e.preventDefault();var btn=document.getElementById('bugSubmit');btn.disabled=true;btn.textContent='Sending...';var data=new FormData(form);fetch('/',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams(data).toString()}).then(function(){document.getElementById('bugThanks').style.display='block';form.querySelector('select').style.display='none';form.querySelector('textarea').style.display='none';btn.style.display='none';setTimeout(function(){document.getElementById('bugModal').classList.remove('open');},2000);}).catch(function(){btn.disabled=false;btn.textContent='Submit Report';});});})()</script>
    ...
    console.error('[vanguard-set-page.mjs] Error:', err.message);
...
export const config = { path: '/cards/vanguard/sets/:slug+', priority: 1 };
