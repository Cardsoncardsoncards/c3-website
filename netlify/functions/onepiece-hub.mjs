...
const GAME_LABEL  = 'One Piece TCG';
const ACCENT      = '#EF4444';
const ACCENT_RGB  = '239,68,68';
const EMOJI       = '&#9763;';
const SEARCH_PH   = 'Luffy, Zoro, Nami, Shanks...';
const SET_PH      = 'Romance Dawn, Paramount War, Royal Blood...';
const CANONICAL   = 'https://cardsoncardsoncards.com.au/cards/onepiece';

const CALENDAR_EVENTS = [
  { date: '2026-06-13', name: 'One Piece OP-10 Wide Release', type: 'Set Release' },
  { date: '2026-09-01', name: 'One Piece OP-11',              type: 'Set Release' },
];
...
export default async (req) => {
  ...
  const [setsResult, topCardsResult] = await Promise.allSettled([
    supabaseGet('onepiece_sets?order=release_date.desc&limit=200&select=id,name,slug,abbreviation,release_date,card_count'),
    supabaseGet('onepiece_cards?order=market_price.desc&market_price=gt.0&image_url=not.is.null&image_url=not.ilike.*sample*&rarity=not.is.null&rarity=neq.None&limit=24&select=slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at')
  ]);
  ...
  const carouselHTML = topCards.map(c => {
    ...
    const ebaySearch = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name + ' one piece tcg card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<a href="/cards/onepiece/${c.slug}" class="carousel-card">
  ...
  const setListHTML = sets.length
    ? sets.map(s => {
        ...
        return `<a href="/cards/onepiece/sets/${setId}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}">
...
<head>
  ...
  <title>One Piece TCG Card Prices Australia | AUD Updated Daily | C3</title>
  <meta name="description" content="Browse 70+ One Piece TCG sets. Live AUD card prices, eBay AU buy links. Australia's One Piece card price guide updated daily.">
  <link rel="canonical" href="${CANONICAL}">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="One Piece Card Prices Australia | Cards on Cards on Cards">
  <meta property="og:description" content="70+ One Piece TCG sets with live AUD prices and eBay AU buy links updated daily.">
...
<nav>
  <div class="nav-inner">
    ...
    <div class="nav-links">
      ...
      <a href="/cards/onepiece" class="nav-link" style="color:#DC2626;border-color:#DC262680;background:#DC262614">One Piece</a>
...
<div class="hero fade-up">
  ...
  <p class="hero-sub">One Piece TCG card prices in AUD. Browse by set or search by card name. eBay AU buy links updated daily.</p>
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length || '74'}</div><div class="stat-label">Sets</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=one+piece+tcg+cards&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=5339146789&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:linear-gradient(135deg,#991b1b,#EF4444);color:#fff">&#128722; Shop One Piece on eBay &#8599;</a>
  ...
  <a href="/blog/best-one-piece-tcg-booster-boxes-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128230; Best OP Boxes &#8594;</a>
  <a href="/blog/one-piece-card-game-australia-beginners-guide/" class="quick-link" style="background:rgba(var(--accent-rgb),.08);border-color:rgba(239,68,68,.3);color:var(--accent)">&#128214; Beginners Guide &#8594;</a>
  <a href="/blog/is-one-piece-card-game-worth-it-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128200; Is OP Worth It? &#8594;</a>
</div>

${topCards.length ? `<section class="carousel-section fade-up fade-up-2">
  <div class="carousel-label">Most Valuable</div>
  <div class="carousel-title">Top One Piece TCG Cards by Price (AUD)</div>
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header">
      <div class="section-title">Search One Piece TCG Cards</div>
    </div>
  ...
  <div class="section fade-up fade-up-3">
    <div class="section-header">
      <div class="section-title">Browse ${sets.length || '74'} Sets</div>
...
<footer>
  ...
  <p style="margin-top:6px;font-size:11px;opacity:.5">Affiliate disclosure: this site earns commissions from eBay AU and Amazon AU purchases made through affiliate links at no extra cost to you. Not affiliated with Bandai or Eiichiro Oda. USD prices converted to AUD at approximately 1.58.</p>
</footer>
...
async function searchCard() {
  ...
  try {
    const res = await fetch('/api/compare-search?q=' + encodeURIComponent(q) + '&game=onepiece&limit=24');
    ...
    results.innerHTML = cards.map(c => {
      ...
      return '<a href="/cards/onepiece/' + c.slug + '" style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center;display:block;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'var(--border)\'">'
