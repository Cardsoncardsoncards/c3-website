...
export default async (req) => {
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/onepiece/sets/${encodeURIComponent(s.abbreviation||s.slug||s.id)}" class="set-tile" data-name="${name.toLowerCase().replace(/"/g,'&quot;')}" data-letter="${letterKey}">
...
<head>
  ...
  <title>One Piece TCG Card Prices Australia | AUD Prices Updated Daily | C3</title>
  <meta name="description" content="Browse ${sets.length}+ One Piece TCG sets. Live AUD card prices, eBay AU buy links. Australia's One Piece price guide updated daily.">
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header"><div class="section-title">Search One Piece Cards</div></div>
    <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
      <input type="text" id="card-search" placeholder="Card name e.g. Monkey D. Luffy, Roronoa Zoro..." onkeyup="if(event.key==='Enter')searchCard()">
