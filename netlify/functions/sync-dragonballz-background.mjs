// netlify/functions/sync-dragonballz-background.mjs
// Daily sync -- schedule: 30 10 * * * UTC
// Fetches all dragon-ball-z-score sets + cards + prices from tcgapi.dev Pro
// Upserts into dragonballz_sets, dragonballz_cards, dragonballz_price_snapshots
...
const GAME_SLUG            = 'dragon-ball-z-tcg'; // task-137: tcgapi.dev renamed from 'dragon-ball-z-score'
...
export default async (req) => {
  ...
  console.log('[sync-dragonballz] Starting...');
  const start = Date.now();

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('[sync-dragonballz] Missing Supabase env vars');
    return new Response('Supabase env vars missing', { status: 500 });
  }
  if (!TCGAPI_KEY) {
    console.error('[sync-dragonballz] TCGAPI_KEY not set');
    return new Response('TCGAPI_KEY missing', { status: 500 });
  }

  try {
    const audRate = await getExchangeRate();
    console.log(`[sync-dragonballz] AUD rate: ${audRate}`);
    ...
    console.log(`[sync-dragonballz] Found ${allSets.length} sets`);
    ...
    for (let i = 0; i < setRows.length; i += 100) {
      await supabaseUpsert('dragonballz_sets', setRows.slice(i, i + 100));
    }
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-dragonballz] Bulk price fetch failed for set ${set.id}:`, e.message);
        }
      }
      ...
      for (const card of setCards) {
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-dragonballz-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: dragonballz_price_snapshots.card_id is a FOREIGN KEY onto
      // dragonballz_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('dragonballz_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('dragonballz_price_snapshots', chunk);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-dragonballz] Upsert error for set ${set.name}:`, r.reason?.message);
      ...
      console.log(`[sync-dragonballz] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }

    const elapsed = ((Date.now() - start) / 1000).toFixed(1);
    console.log(`[sync-dragonballz] Done. ${totalCards} cards, ${totalSnaps} snapshots in ${elapsed}s`);
    ...
    console.error('[sync-dragonballz] FATAL:', err.message);
