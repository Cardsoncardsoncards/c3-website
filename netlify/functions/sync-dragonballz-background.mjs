...
// Group A sync (Mon/Wed/Fri/Sun) - schedule: 45 23 * * 0,1,3,5 UTC
// Fetches all dragon-ball-z-tcg sets + cards + prices from tcgapi.dev Pro
...
const GAME_SLUG            = 'dragon-ball-z-tcg';
...
export default async (req) => {
  console.log('[sync-dragonballz] Starting (background function)...');
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-dragonballz] Bulk price error set ${set.id}:`, e.message);
        }
      }
