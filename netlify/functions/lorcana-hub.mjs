...
export default async (req) => {
  ...
  const [sets, topCards] = await Promise.all([
    ...
    supabaseGet('lorcana_cards?order=price_usd.desc&price_usd=gt.0&image_uri=not.is.null&limit=24&select=slug,name,version,image_uri,price_usd,ink,rarity')
  ]);
  ...
  const carouselHTML = topCards.map(c => `
    <a href="/cards/lorcana/${c.slug}" class="carousel-card">
      ...
      ${c.rarity ? `<div class="carousel-rarity" style="color:${c.ink ? getInkColour(c.ink) : '#888'}">${c.ink||''}</div>` : ''}
...
<head>
  ...
  <meta name="description" content="Browse Disney Lorcana card prices in AUD. Live AUD pricing, eBay AU buy links. Australia's Pokemon price guide updated daily.">
  ...
  <style>
    :root{
      ...
      --lorcana-gold:#F5B202;--lorcana-blue:#0189C4;--lorcana-ink:#7c3aed;
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .nav-link--pokemon{color:var(--lorcana-gold);border-color:rgba(255,204,0,.4);background:rgba(255,204,0,.07)}
    ...
    .hero-eyebrow{font-size:10px;font-weight:700;letter-spacing:.3em;text-transform:uppercase;color:var(--lorcana-gold);margin-bottom:14px}
    ...
    .stat-num{font-family:'Cinzel',serif;font-size:20px;font-weight:700;color:var(--lorcana-gold)}
    ...
    .carousel-label{font-size:10px;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:var(--lorcana-gold);margin-bottom:10px;padding:0 24px}
    ...
    .carousel-card:hover{border-color:var(--lorcana-gold);transform:translateY(-4px);box-shadow:0 8px 24px rgba(255,204,0,.15);text-decoration:none}
    ...
    .carousel-price{font-size:13px;color:var(--lorcana-gold);font-weight:700;margin-top:3px}
    ...
    input:focus{outline:none;border-color:var(--lorcana-gold)}
    ...
    .btn-primary{background:var(--lorcana-gold);color:#000}
    ...
    .set-tile:hover{border-color:var(--lorcana-gold);background:rgba(255,204,0,.04);text-decoration:none;transform:translateX(2px)}
...
<nav>
  <div class="nav-inner">
    ...
    <div class="nav-links">
      ...
      <a href="/cards/lorcana" class="nav-link nav-link--lorcana" style="background:rgba(56,189,248,.1);color:#38BDF8">Lorcana</a>
...
<div class="hero fade-up">
  ...
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length || '19'}</div><div class="stat-label">Sets</div></div>
...
<div class="wrap">
  ...
  <div style="background:rgba(255,204,0,.04);border:1px solid rgba(255,204,0,.15);border-radius:var(--radius);padding:24px;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px">
    <div>
      <div style="font-size:17px;font-weight:700;color:var(--text);margin-bottom:6px">Track Your Lorcana Collection</div>
...
<footer>
  ...
  <p style="margin-top:6px;font-size:11px;opacity:.5">Not affiliated with Disney or Ravensburger. Card data via Lorcast. Prices converted to AUD at approximately 1.58.</p>
</footer>
...
async function searchCard() {
  ...
  try {
    const url = window.C3_SUPA_URL + '/rest/v1/lorcana_cards?select=slug,name,version,image_uri,price_usd,ink,rarity&order=price_usd.desc&limit=24&name=ilike.*' + encodeURIComponent(q) + '*';
