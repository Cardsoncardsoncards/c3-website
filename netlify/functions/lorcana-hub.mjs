...
function buildTickerHTML(events) {
  ...
  return `<div class="release-ticker"><span class="ticker-label">&#129516; Lorcana</span><div class="ticker-track">${doubled}</div></div>`;
}
...
export default async (req) => {
  ...
  const carouselHTML = topCards.map(c => `
    <a href="/cards/lorcana/${c.slug}" class="carousel-card">
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/lorcana/sets/${encodeURIComponent(s.slug||s.id)}" class="set-tile" data-name="${name.toLowerCase().replace(/"/g,'&quot;')}" data-letter="${letterKey}">
...
<head>
  ...
  <meta name="description" content="Browse Disney Lorcana card prices in AUD. ${sets.length}+ sets, live eBay AU buy links. Australia's Lorcana price guide updated daily.">
  ...
  <style>
    :root{--bg:#0A0C14;--bg2:#111420;--bg3:#181d2e;--bg4:#1e2338;--gold:#C9A84C;--accent:#38BDF8;--accent-dim:rgba(56,189,248,.15);--text:#e8eaf0;--text2:#9ba3c4;--border:#242840;--radius:12px;--silver:#A0A8C0}
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    body::before{content:'';position:fixed;inset:0;pointer-events:none;z-index:0;background:radial-gradient(ellipse 70% 40% at 50% 0%,rgba(56,189,248,.05),transparent 60%)}
    ...
    .nav-link--active{color:var(--accent);border-color:rgba(56,189,248,.4);background:rgba(56,189,248,.07)}
    /* === C3 NAV COLOURS === */
    ...
    .release-ticker{background:rgba(56,189,248,.06);border-bottom:1px solid rgba(56,189,248,.15);height:34px;display:flex;align-items:center;overflow:hidden;position:relative}
    ...
    .ticker-badge{font-size:9px;font-weight:700;padding:2px 6px;border-radius:3px;background:rgba(56,189,248,.15);color:var(--accent);letter-spacing:.06em}
    ...
    .carousel-card:hover{border-color:var(--accent);transform:translateY(-4px);box-shadow:0 8px 20px rgba(56,189,248,.12);text-decoration:none}
    ...
    .set-tile:hover{border-color:var(--accent);background:rgba(56,189,248,.04);text-decoration:none;transform:translateX(2px)}
...
<div class="hero fade-up">
  ...
  <p class="hero-sub">Disney Lorcana card prices in AUD. Browse by ink colour or set. eBay AU buy links updated daily.</p>
  <div class="stat-bar">
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=disney+lorcana+cards&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:linear-gradient(135deg,#0ea5e9,var(--accent));color:#000">&#128722; Shop Lorcana on eBay &#8599;</a>
  ...
  <a href="/quizzes/lorcana-ink" class="quick-link" style="background:rgba(56,189,248,.08);border-color:rgba(56,189,248,.3);color:var(--accent)">&#127919; Which Lorcana Ink Are You? &#8594;</a>
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header"><div class="section-title">Search Lorcana Cards</div></div>
    <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
      <input type="text" id="card-search" placeholder="Card name e.g. Elsa, Moana, Stitch..." onkeyup="if(event.key==='Enter')searchCard()">
  ...
  <div class="section fade-up fade-up-3">
    ...
    <input type="text" id="set-search" placeholder="Search sets e.g. Wilds Unknown, Fabled..." oninput="filterSets(this.value)" style="margin-bottom:12px">
    <div id="set-list" class="set-grid">${setListHTML}</div>
  </div>
  <div style="background:rgba(56,189,248,.04);border:1px solid rgba(56,189,248,.15);border-radius:var(--radius);padding:22px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px">
    <div>
...
<footer>
  ...
  <p style="margin-top:6px;font-size:11px;opacity:.5">Not affiliated with Disney or Ravensburger. Prices converted to AUD at approximately 1.58.</p>
</footer>
<script>
let activeAZ = 'All';
