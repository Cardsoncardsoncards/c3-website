...
// Generates XML sitemap for Yu-Gi-Oh card pages at /cards/yugioh/[slug]
// Registered in sitemap-index.xml as /api/sitemap-yugioh
// Lower price threshold (0.50) to capture competitive staples
...
export default async (req) => {
  ...
  try {
    const countRes = await fetch(
      `${SUPABASE_URL}/rest/v1/yugioh_cards?select=id&price_usd=gte.${PRICE_THRESHOLD}&slug=not.is.null&limit=1`,
      {
        headers: {
          'apikey': SUPABASE_ANON_KEY,
          'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    if (totalCount === 0) {
      return new Response(
        `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n<!-- Yu-Gi-Oh card pages: pending sync -->\n</urlset>`,
        { status: 200, headers }
      );
    }
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <!-- Yu-Gi-Oh card pages: ${allCards.length} cards with price >= USD$${PRICE_THRESHOLD} -->
