...
export default async (req) => {
  ...
  try {
    const countRes = await supabaseFetch(
      `${SUPABASE_URL}/rest/v1/yugioh_cards?select=id&market_price=gte.${PRICE_THRESHOLD}&slug=not.is.null&limit=1`,
      { 'Prefer': 'count=exact' }
    );
    if (!countRes.ok) return empty('yugioh sitemap: count query failed');

    const contentRange = countRes.headers.get('content-range');
    const totalCount = (contentRange && contentRange.includes('/'))
      ? parseInt(contentRange.split('/')[1], 10) : 0;

    if (totalCount === 0) return empty('yugioh card pages: pending sync or no priced cards');
