...
// netlify/functions/bakugan-card-page.mjs
// Serves /cards/bakugan/:slug
...
async function getEbayListings(cardName, token) {
  if (!token) return [];
  try {
    const q = encodeURIComponent(`${cardName} bakugan card game card`);
...
function notFoundPage(slug) {
  return `<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><title>Card Not Found | Cards on Cards on Cards</title><link rel="icon" type="image/png" href="/c3logo.png"></head><body style="background:#0A0C14;color:#F0F2FF;font-family:sans-serif"><style>${NAV_CSS}</style>${navHtml({ gameLabel: 'Bakugan', gameHref: '/cards/bakugan' })}<div style="padding:80px 24px;text-align:center"><h1 style="font-family:'Cinzel',serif;color:#C9A84C;margin-bottom:16px">Card Not Found</h1><p style="color:#A0A8C0;margin-bottom:24px">The card "${slug}" doesn't exist or hasn't synced yet.</p><a href="/cards/bakugan" style="display:inline-block;padding:10px 24px;background:rgba(201,168,76,.15);border:1px solid rgba(201,168,76,.4);color:#E8C97A;border-radius:8px;text-decoration:none">Browse Bakugan TCG Cards</a></div></body></html>`;
}

function setNotFoundPage(setSlug) {
  return `<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><title>Set Not Found | Bakugan TCG | Cards on Cards on Cards</title><meta name="robots" content="noindex"><link rel="icon" type="image/png" href="/c3logo.png"><link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@700&family=DM+Sans:wght@400;600&display=swap" rel="stylesheet"></head><body style="background:#0A0C14;color:#F0F2FF;font-family:'DM Sans',sans-serif"><style>${NAV_CSS}</style>${navHtml({ gameLabel: 'Bakugan', gameHref: '/cards/bakugan' })}<div style="display:flex;align-items:center;justify-content:center;min-height:60vh;padding:24px;text-align:center"><div><div style="font-size:48px;margin-bottom:16px">🃏</div><h1 style="font-family:'Cinzel',serif;color:#880E4F;font-size:22px;margin-bottom:10px">Set Not Found</h1><p style="color:#8892b0;font-size:14px;margin-bottom:24px">We couldn't find the Bakugan TCG set "${setSlug}".</p><a href="/cards/bakugan" style="display:inline-block;background:#880E4F;color:#000;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px">Browse All Bakugan</a><a href="/" style="display:inline-block;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);color:#F0F2FF;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px">← Home</a></div></div></body></html>`;
}
...
async function handleSetPage(setSlug, htmlHeaders) {
  ...
  const [_psr0, _psr1] = await Promise.allSettled([
    supabaseGet(`bakugan_sets?slug=eq.${encodeURIComponent(setSlug)}&limit=1&select=*`),
    getEbayToken()
  ]);
  ...
  const [_psr2, _psr3] = await Promise.allSettled([
    supabaseGet(`bakugan_cards?set_id=eq.${set.id}&order=market_price.desc.nullslast&limit=400&select=slug,name,number,image_url,market_price,price_aud,rarity,set_name`),
    ebayToken ? getEbayListings(set.name + ' bakugan tcg', ebayToken).catch(() => []) : []
  ]);
  ...
  const ebaySetURL = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name + ' bakugan tcg')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
  const metaDesc = `Browse ${cards?.length || 0} Bakugan TCG cards from ${set.name}. View card prices in AUD and buy on eBay AU. Updated daily.`;

  const top5HTML = top5.map(c => {
    const aud = toAud(c);
    return `<a href="/cards/bakugan/${c.slug}" style="flex:0 0 140px;background:#0e1118;border:1px solid rgba(16,185,129,.35);border-radius:10px;padding:10px;text-align:center;text-decoration:none;display:block">
  ...
  const allCardsHTML = (cards && cards.length) ? cards.filter(isSingles).map(c => {
    const aud = toAud(c);
    return `<a href="/cards/bakugan/${c.slug}" style="background:#0e1118;border:1px solid #1e2235;border-radius:8px;padding:8px;text-decoration:none;text-align:center;display:block">
...
<head>
  ...
  <title>${set.name} | Bakugan TCG Set | Cards on Cards on Cards</title>
  <meta name="description" content="${metaDesc}">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/bakugan/sets/${setSlug}">
  <meta property="og:title" content="${set.name} | Bakugan TCG | C3">
...
<div class="wrap">
  <div class="hero">
    <div class="hero-eyebrow">Bakugan TCG · Set</div>
    <h1 class="hero-title">${set.name}</h1>
    <div class="hero-meta">
      <span class="meta-badge">Bakugan TCG</span>
  ...
  <div class="cta-row">
    ...
    <a href="https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name + ' bakugan tcg booster box')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="cta-btn cta-secondary">Find Booster Box →</a>
    <a href="https://www.amazon.com.au/s?k=${encodeURIComponent(set.name + ' Bakugan TCG')}&tag=${AMAZON_TAG}" target="_blank" rel="noopener" class="cta-btn cta-secondary" style="border-color:rgba(255,153,0,.35);color:#ff9900">Amazon AU →</a>
  </div>
  ...
  <div style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:20px;font-size:13px;color:#8892b0">
    <strong style="color:#F0F2FF">About this set:</strong> Bakugan TCG card prices shown in AUD, converted from USD market data. Prices update daily via tcgapi.dev.
    <div style="margin-top:10px"><a href="/cards/bakugan" style="color:${accent}">← Back to all Bakugan TCG cards</a></div>
  </div>
</div>
...
export default async (req) => {
  ...
  const slug = url.pathname.replace('/cards/bakugan/', '').replace(/^\/|\/$/g, '');
  ...
  try {
    ...
    const snapshots = await supabaseGet(`bakugan_price_snapshots?card_id=eq.${encodeURIComponent(card.id)}&snapshot_date=gte.${new Date(Date.now()-90*86400000).toISOString().slice(0,10)}&order=snapshot_date.asc&limit=400&select=snapshot_date,price_aud,market_price`).catch(() => []);
    ...
    const pageUrl = encodeURIComponent(`https://cardsoncardsoncards.com.au/cards/bakugan/${card.slug}`);
    const shareText = encodeURIComponent(`${card.name} Bakugan TCG Card Game -- ${priceAud ? 'AU$'+priceAud.toFixed(2) : 'check price'} on Cards on Cards on Cards`);
    const ebaySearchUrl = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(card.name+' bakugan card game card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;

    const [_psr0, _psr1] = await Promise.allSettled([
      supabaseGet(`bakugan_cards?set_id=eq.${card.set_id}&slug=neq.${encodeURIComponent(slug)}&market_price=gt.0&image_url=not.is.null&limit=12&order=market_price.desc&select=*`).catch(() => []),
  ...
  const ebayToken = _psr1.status === 'fulfilled' ? _psr1.value : [];
    ...
    const breadcrumb = { "@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[
      ...
      {"@type":"ListItem","position":2,"name":"Bakugan TCG Cards","item":"https://cardsoncardsoncards.com.au/cards/bakugan"},
      {"@type":"ListItem","position":3,"name":card.name,"item":`https://cardsoncardsoncards.com.au/cards/bakugan/${card.slug}`}
    ]};

    const productSchema = priceAud ? { "@context":"https://schema.org","@type":"Product","name":card.name,"image":card.image_url||'',
      "offers":{"@type":"Offer","priceCurrency":"AUD","price":priceAud.toFixed(2),"availability":"https://schema.org/InStock","url":`https://cardsoncardsoncards.com.au/cards/bakugan/${card.slug}`}
    } : null;
    ...
    <div style="max-width:1100px;margin:0 auto 32px;padding:0 24px">
      ...
      <div style="display:flex;gap:12px;overflow-x:auto;padding-bottom:8px;scrollbar-width:none">
        ${relatedCards.map(c => {
          ...
          return `<a href="/cards/bakugan/${c.slug}" style="flex:0 0 130px;background:#161929;border:1px solid #252840;border-radius:8px;padding:8px;text-decoration:none">
...
<head>
  ...
  <title>${card.name} Price Australia | Bakugan TCG | Cards on Cards on Cards</title>
  <meta name="description" content="${card.name} Bakugan TCG card${priceAud ? ` -- ~AU$${priceAud.toFixed(2)}` : ''}. ${card.rarity ? `${card.rarity}. ` : ''}View price and buy on eBay AU.">
  <meta property="og:site_name" content="Cards on Cards on Cards">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/bakugan/${card.slug}">
  ...
  <meta property="og:title" content="${card.name} | Bakugan TCG Price AU">
...
<div class="card-hero">
  ...
  <div class="card-details">
    <div style="font-size:12px;color:var(--text2);margin-bottom:8px">
      <a href="/cards/bakugan" style="color:var(--text2);text-decoration:none">Bakugan TCG</a>
    ...
    ${followBlockHtml({ game: 'bakugan', slug: card.slug, cardName: card.name, printingId: card.tcgplayer_id })}
...
function addToCompare(slug,name,img,price,game){
  ...
  tray.push({slug,name,img,price,game:'bakugan'});saveCompareTray(tray);renderCompareTray();
}
...
${viewTrackingScript('bakugan', card.slug)}
</body>
</html>`;

    return new Response(html, { status: 200, headers });
  } catch (err) {
    console.error('[bakugan-card-page]', err.message);
...
export const config = { path: '/cards/bakugan/:slug+', excludedPath: '/cards/bakugan/sets/*' };
