...
export default async (req) => {
  ...
  try {
    ...
    for (const set of allSets) {
      ...
      for (const card of setCards) {
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-weissschwarz-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: weissschwarz_price_snapshots.card_id is a FOREIGN KEY onto
      // weissschwarz_cards(id), so a snapshot row is the CHILD of a card row. Racing
