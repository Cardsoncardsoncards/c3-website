...
// Group B sync (Tue/Thu/Sat) - schedule: 30 21 * * 2,4,6 UTC
...
export default async (req) => {
  console.log('[sync-weissschwarz] Starting (background function)...');
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-weissschwarz] Bulk price error set ${set.id}:`, e.message);
        }
      }
