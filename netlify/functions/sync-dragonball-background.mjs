    ...
    for (const set of allSets) {
      ...
      for (const card of setCards) {
        ...
        cardRows.push({
          ...
          // update_dragonball_price_changes() computes each night from C3's OWN price_aud snapshots
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-dragonball-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: dragonball_price_snapshots.card_id is a FOREIGN KEY onto
      // dragonball_cards(id), so a snapshot row is the CHILD of a card row. Racing
