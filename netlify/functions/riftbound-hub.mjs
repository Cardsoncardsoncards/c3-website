...
const GAME_LABEL  = 'Riftbound';
const ACCENT      = '#7C6AF5';
const ACCENT_RGB  = '124,106,245';
const EMOJI       = '&#9876;';
const SEARCH_PH   = 'Ahri, Jinx, Yasuo, Thresh...';
const SET_PH      = 'Launch, Reckoning, Harmony...';
...
export default async (req) => {
  ...
  const [setsResult, topCardsResult] = await Promise.allSettled([
    supabaseGet('riftbound_sets?order=release_date.desc&limit=50&select=id,name,slug,abbreviation,release_date,card_count'),
    supabaseGet('riftbound_cards?order=market_price.desc&market_price=gt.0&image_url=not.is.null&limit=24&select=slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at')
  ]);
  ...
  const carouselHTML = topCards.map(c => {
    ...
    const ebaySearch = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name + ' riftbound card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<a href="/cards/riftbound/${c.slug}" class="carousel-card">
  ...
  const setListHTML = sets.length
    ? sets.map(s => {
        ...
        return `<a href="/cards/riftbound/sets/${setId}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}">
...
<head>
  ...
  <title>Riftbound Card Prices Australia | AUD Updated Daily | C3</title>
  <meta name="description" content="Browse Riftbound (League of Legends TCG) card prices in AUD. Live eBay AU buy links and set browser updated daily.">
  <link rel="canonical" href="${CANONICAL}">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="Riftbound Card Prices Australia | Cards on Cards on Cards">
  <meta property="og:description" content="Riftbound (League of Legends TCG) card prices in AUD with live eBay AU buy links, updated daily.">
...
<nav>
  <div class="nav-inner">
    ...
    <div class="nav-links">
      ...
      <a href="/cards/riftbound" class="nav-link" style="color:#5865F2;border-color:#5865F280;background:#5865F214">Riftbound</a>
...
<div class="hero fade-up">
  ...
  <p class="hero-sub">Riftbound (League of Legends TCG) card prices in AUD. Browse by set or search by card name. eBay AU buy links updated daily.</p>
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length || '8'}</div><div class="stat-label">Sets</div></div>
    <div class="stat-item"><div class="stat-num">500+</div><div class="stat-label">Cards</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=riftbound+cards&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=5339146789&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:linear-gradient(135deg,#4c1d95,#7C6AF5);color:#fff">&#128722; Shop Riftbound on eBay &#8599;</a>
  ...
  <a href="/blog/riftbound-card-game-beginners-guide-australia/" class="quick-link" style="background:rgba(var(--accent-rgb),.08);border-color:rgba(124,106,245,.3);color:var(--accent)">&#128214; Beginners Guide &#8594;</a>
  <a href="/blog/riftbound-australia-beginners-guide/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128230; Australia Guide &#8594;</a>
  <a href="/blog/riftbound-card-prices-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128200; Card Prices Guide &#8594;</a>
</div>
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header">
      <div class="section-title">Search Riftbound Cards</div>
    </div>
...
<footer>
  ...
  <p style="margin-top:6px;font-size:11px;opacity:.5">Affiliate disclosure: this site earns commissions from eBay AU and Amazon AU purchases made through affiliate links at no extra cost to you. Not affiliated with Riot Games or League of Legends. USD prices converted to AUD at approximately 1.58.</p>
</footer>
