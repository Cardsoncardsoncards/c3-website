...
const GAME_LABEL = 'Riftbound TCG';
const ACCENT     = '#10B981';
const ACCENT_RGB = '16,185,129';
const EMOJI      = '&#127756;';
const SET_PH     = 'Origins, Spiritforged, Unleashed, Vendetta...';
const CANONICAL  = 'https://cardsoncardsoncards.com.au/cards/riftbound';
...
function sharedCSS() {
  return `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .carousel-card{flex-shrink:0;width:155px;background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:10px;text-align:center;display:block;transition:all .25s}
...
export default async (req) => {
  ...
  const [setsRes, cardsRes, gainersRes, losersRes] = await Promise.allSettled([
    supabaseGet('riftbound_sets?order=release_date.desc.nullslast&limit=300&select=id,name,slug,release_date,card_count'),
    supabaseGet('riftbound_cards?order=price_aud.desc&price_aud=gt.0&image_url=not.is.null&rarity=not.is.null&rarity=neq.None&limit=24&select=slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at'),
    supabaseGet('riftbound_cards?order=price_change_7d.desc&price_change_7d=gt.5&price_aud=gt.2&price_change_7d=lt.5000&image_url=not.is.null&limit=5&select=slug,name,image_url,price_aud,price_change_7d,set_name'),
    supabaseGet('riftbound_cards?order=price_change_7d.asc&price_change_7d=lt.-5&price_aud=gt.2&image_url=not.is.null&limit=5&select=slug,name,image_url,price_aud,price_change_7d,set_name')
  ]);
  ...
  function carouselCard(c, suffix) {
    ...
    const ebay  = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name+' riftbound card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<div class="carousel-card"><a href="/cards/riftbound/${esc(c.slug)}" style="display:block;text-decoration:none"><div class="carousel-img-wrap"><img src="${esc(c.image_url)}${suffix}" alt="${esc(c.name).replace(/"/g,'&quot;')}" loading="eager" onerror="this.onerror=null;this.src='/card-placeholder.svg';this.style.opacity='1'"></div><div class="carousel-name">${esc(c.name)}</div>${c.rarity?`<div class="carousel-rarity">${esc(c.rarity)}</div>`:''}<div class="carousel-price">${price}</div></a><div class="carousel-buy-row"><a href="${ebay}" target="_blank" rel="noopener" onclick="gtag('event','ebay_click',{'event_category':'affiliate','event_label':'riftbound-hub'})" class="carousel-buy-btn">Buy eBay &#8599;</a></div></div>`;
  }
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/riftbound/sets/${encodeURIComponent(s.slug||s.id)}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}"><span class="set-tile-name">${esc(name)}${newBadge}</span><span class="set-tile-meta">${year}${s.card_count?' &middot; '+s.card_count+' cards':''}</span></a>`;
...
<head>
  ...
  ${hubSchemaHtml({ game: 'riftbound', label: "Riftbound", name: "Riftbound Card Prices Australia", description: `Browse ${sets.length||'4'}+ Riftbound TCG sets. Live AUD card prices, eBay AU buy links. Australia's most complete Riftbound price guide, updated daily.` })}
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="Riftbound Card Prices Australia | Cards on Cards on Cards">
  <meta property="og:description" content="${sets.length||'4'}+ Riftbound sets. Live AUD prices and eBay AU buy links updated daily.">
...
<div class="hero fade-up">
  <div class="hero-eyebrow">Card Vault -- Riftbound TCG</div>
  <h1>Riftbound Card Prices <span>in Australia</span></h1>
  <p class="hero-sub">Riftbound TCG card prices in AUD. Browse by set or search by card name. eBay AU buy links updated daily.</p>
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length||'4'}</div><div class="stat-label">Sets</div></div>
    <div class="stat-item"><div class="stat-num">1.1k+</div><div class="stat-label">Cards</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=riftbound+cards&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" onclick="gtag('event','ebay_click',{'event_category':'affiliate','event_label':'riftbound-hub'})" class="quick-link" style="background:linear-gradient(135deg,#064e3b,${ACCENT});color:#000">&#128722; Shop Riftbound on eBay &#8599;</a>
  ...
  <a href="/blog/riftbound-booster-box-value-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128230; Best Boxes &#8594;</a>
  <a href="/blog/is-riftbound-worth-starting-australia-2026/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128214; Is It Worth It? &#8594;</a>
...
${hasMovers ? `<div style="margin-bottom:28px">
  ...
  <p style="color:var(--text2);font-size:13px;margin-bottom:8px">Biggest price movers across all Riftbound cards in the last 7 days.</p>
  ...
  <div class="section fade-up fade-up-3">
    <div style="text-align:center;margin-bottom:16px">
      <h2 style="font-family:'Cinzel',serif;font-size:24px;font-weight:700;color:var(--text);margin-bottom:6px">Browse Every Riftbound Set</h2>
      <p style="font-size:13px;color:var(--text2)">${sets.length||'4'}+ sets. Pick a letter or search to explore.</p>
    </div>
  ...
  <div style="margin-bottom:28px">
    <h2 style="font-size:18px;margin-bottom:16px">Riftbound Guides and Blogs</h2>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px">
      <a href="/blog/riftbound-booster-box-value-australia/" class="guide-card"><div class="guide-title">Best Riftbound Booster Boxes in Australia</div><div class="guide-desc">Which Riftbound boxes are worth opening and where to buy in Australia.</div></a>
      <a href="/blog/is-riftbound-worth-starting-australia-2026/" class="guide-card"><div class="guide-title">Is Riftbound Worth It?</div><div class="guide-desc">An honest look at Riftbound collecting from an Australian perspective.</div></a>
...
<footer>
  ...
  <div style="margin-bottom:10px">
    <a href="/">Home</a><a href="/blog">Blog</a><a href="/calendar">Calendar</a><a href="/account">Account</a><a href="/tracker">Tracker</a><a href="/cards">Card Prices</a><a href="/compare">Compare</a><a href="/market">Market</a><a href="/tools">Tools</a><a href="/contact">Contact</a><a href="/legal">Legal</a><a href="https://www.ebay.com.au/str/cardsoncardsoncards?mkcid=1&amp;mkrid=705-53470-19255-0&amp;campid=5339146789&amp;customid=Footer&amp;toolid=10001&amp;mkevt=1" target="_blank" rel="noopener" onclick="gtag('event','ebay_click',{'event_category':'affiliate','event_label':'footer'})">eBay</a><a href="https://blasdigital.etsy.com" target="_blank" rel="noopener">D&amp;D Tools on Etsy &#8599;</a><br><a href="/cards/riftbound">Riftbound</a><a href="/cards/pokemon">Pokemon</a><a href="/cards/mtg">MTG</a><a href="/cards/yugioh">Yu-Gi-Oh</a><a href="/cards/onepiece">One Piece</a>
  </div>
  <p>&#169; 2026 Cards on Cards on Cards &middot; cardsoncardsoncards.com.au</p>
  <p style="margin-top:6px;font-size:11px;opacity:.5">Affiliate disclosure: this site earns commissions from eBay AU and Amazon AU purchases made through affiliate links at no extra cost to you. Not affiliated with Riot Games. USD prices converted to AUD at approximately ${audRate.toFixed(2)}.</p>
