...
async function getAlreadySyncedSetIds() {
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/riftbound_sync_progress?select=set_id&limit=1000`,
    {
      headers: {
        'apikey': SUPABASE_SERVICE_KEY,
        'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}`
  ...
  if (!res.ok) {
    console.error('[sync-riftbound] Could not fetch sync progress, will do full sync');
    return new Set();
  }
...
async function markSetSynced(setId) {
  await fetch(
    `${SUPABASE_URL}/rest/v1/riftbound_sync_progress`,
    {
      method: 'POST',
      headers: {
        'apikey': SUPABASE_SERVICE_KEY,
...
export default async (req) => {
  ...
  try {
    ...
    console.log(`[sync-riftbound] ${syncedSetIds.size} sets already synced — will skip these`);
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < snapRows.length; i += 500) {
        await supabaseUpsertSnapshots('riftbound_price_snapshots', snapRows.slice(i, i + 500));
      }
