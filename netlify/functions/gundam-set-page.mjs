...
<head>
  ...
  <style>*{box-sizing:border-box;margin:0;padding:0}body{background:#0A0C14;color:#F0F2FF;font-family:'DM Sans',sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px;text-align:center}.wrap{max-width:420px}h1{color:#64748B;font-size:22px;margin-bottom:10px}p{color:#8892b0;font-size:14px;margin-bottom:24px;line-height:1.6}.btn{display:inline-block;background:#64748B;color:#000;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px}.btn-sec{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);color:#F0F2FF}</style>
</head>
<body><div class="wrap">
  <h1>Set Not Found</h1>
  <p>We couldn't find that Gundam Card Game set. It may not be in our database yet.</p>
  <a href="/cards/gundam" class="btn">Browse All Gundam Card Game Cards</a>
...
export default async (req) => {
  ...
  const [sets] = await Promise.allSettled([
    supabaseGet(`gundam_sets?slug=eq.${encodeURIComponent(setSlug)}&limit=1&select=id,name,slug,release_date,card_count`)
  ]);
  ...
  const cardsResult = await supabaseGet(`gundam_cards?set_id=eq.${set.id}&order=market_price.desc.nullslast&limit=200&select=slug,name,image_url,market_price,price_aud,rarity,number`).catch(() => []);
  ...
  const top6HTML = top6.map(c => {
    const aud = toAud(c);
    return `<a href="/cards/gundam/${c.slug}" style="flex:0 0 140px;background:#111420;border:1px solid rgba(var(--accent-rgb),.3);border-radius:10px;padding:10px;text-align:center;text-decoration:none;display:block;transition:all .2s" onmouseover="this.style.borderColor='#64748B';this.style.transform='translateY(-2px)'" onmouseout="this.style.borderColor='#242840';this.style.transform='none'">
      ...
      ${c.rarity ? `<div style="font-size:10px;color:#64748B">${esc(c.rarity)}</div>` : ''}
  ...
  const allCardsHTML = cards.map(c => {
    const aud = toAud(c);
    return `<a href="/cards/gundam/${c.slug}" style="background:#111420;border:1px solid #242840;border-radius:8px;padding:8px;text-align:center;display:block;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor='#64748B'" onmouseout="this.style.borderColor='#242840'">
      ...
      ${aud > 0 ? `<div style="font-size:11px;color:#64748B;font-weight:700">AU$${aud.toFixed(0)}</div>` : ''}
    </a>`;
  }).join('');
  ...
  const sealedHTML = (() => {
    ...
    const itemsHTML = sealedItems.slice(0,4).map(p => {
      ...
      return `<a href="/cards/gundam/${p.slug}" style="background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:14px;display:flex;flex-direction:column;gap:8px;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor='var(--accent)'" onmouseout="this.style.borderColor='var(--border)'">
    ...
    return `<div style="background:rgba(var(--accent-rgb),.04);border:1px solid rgba(var(--accent-rgb),.15);border-radius:14px;padding:22px 24px;margin-bottom:32px">
      ...
      <a href="https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent((set?.name||'')+' gundam booster')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:8px;font-weight:700;font-size:12px;text-decoration:none;background:var(--accent);color:#000">&#128722; More sealed on eBay AU &#8599;</a>
    </div>`;
  })();
...
<head>
  ...
  <title>${esc(set.name)} | Gundam Card Game Cards | Cards on Cards on Cards</title>
  <meta name="description" content="Browse all ${esc(set.name)} cards from Gundam Card Game with live AUD prices and eBay AU buy links. Updated daily.">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/gundam/sets/${esc(setSlug)}">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="${esc(set.name)} | Gundam Card Game | C3">
  ...
  <meta property="og:url" content="https://cardsoncardsoncards.com.au/cards/gundam/sets/${esc(setSlug)}">
  ...
  <style>
    :root{--bg:#0A0C14;--bg2:#111420;--bg3:#181d2e;--gold:#C9A84C;--accent:#64748B;--accent-rgb:100,116,139;--text:#e8eaf0;--text2:#9ba3c4;--border:#242840;--radius:12px}
...
<nav>
  <div class="nav-inner">
    ...
    <div class="nav-links">
      ...
      <a href="/cards/gundam" class="nav-link" style="color:#64748B;border-color:#64748B80;background:#64748B14">Gundam</a>
...
<div class="wrap">
  <div style="margin-bottom:6px"><a href="/cards/gundam" style="color:var(--text2);font-size:13px">&larr; Back to Gundam Card Game</a></div>
  ...
  <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:32px">
    ...
    <a href="/cards/gundam" class="btn" style="background:var(--bg2);border:1px solid var(--border);color:var(--text)">&#8592; All Gundam Card Game Sets</a>
  </div>
...
<footer>
  ...
  <p style="margin-top:6px;font-size:11px;opacity:.5">Affiliate disclosure: this site earns commissions from eBay AU purchases via affiliate links at no extra cost to you. Not affiliated with Bandai or Sunrise.</p>
</footer>
