// netlify/functions/gateruler-hub.mjs
// Serves /cards/gateruler
...
const EMOJI       = '&#128302;';
const SEARCH_PH   = "Dragon, Demon, Phantom...";
const SET_PH      = "GR-001, GR-002...";
const CANONICAL   = 'https://cardsoncardsoncards.com.au/cards/gateruler';
...
export default async (req) => {
  ...
  const [setsResult, topCardsResult] = await Promise.allSettled([
    supabaseGet('gateruler_sets?order=release_date.desc&limit=30&select=id,name,slug,abbreviation,release_date,card_count'),
    supabaseGet('gateruler_cards?order=market_price.desc&market_price=gt.0.5&market_price=lt.200&image_url=not.is.null&limit=24&select=slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at')
  ]);
  ...
  const carouselHTML = topCards.map(c => {
    ...
    const ebay  = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name+' gate ruler card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<a href="/cards/gateruler/${c.slug}" class="carousel-card">
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/gateruler/sets/${encodeURIComponent(s.slug||s.id)}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}">
...
<head>
  ...
  <title>Gate Ruler Card Prices Australia | AUD Prices Updated Daily | C3</title>
  <meta name="description" content="Browse ${sets.length||'11'}+ Gate Ruler sets. Live AUD card prices and eBay AU buy links updated daily. Australia's Gate Ruler price guide.">
  <link rel="canonical" href="${CANONICAL}">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="Gate Ruler Card Prices Australia | Cards on Cards on Cards">
  <meta property="og:description" content="${sets.length||'11'}+ Gate Ruler sets with live AUD prices and eBay AU buy links.">
...
<!-- OUT OF PRINT NOTICE -->
  <div style="background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.25);border-radius:10px;padding:14px 18px;margin:0 24px 28px;display:flex;align-items:center;gap:12px">
    ...
    <div>
      ...
      <div style="font-size:13px;color:rgba(160,168,192,.75);line-height:1.5">Gate Ruler is no longer in print. Cards shown are from the collector&#39;s secondary market. Prices reflect current eBay AU and TCGPlayer market values.</div>
    </div>
  </div>
...
<div class="hero fade-up">
  <div class="hero-eyebrow">Card Vault -- Gate Ruler</div>
  <h1>Gate Ruler Card Prices <span>in Australia</span></h1>
  <p class="hero-sub">Gate Ruler card prices in AUD. Browse by set or search by card name. eBay AU buy links updated daily.</p>
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length||'11'}</div><div class="stat-label">Sets</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=gate+ruler+card&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=5339146789&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:var(--accent);color:#000">&#128722; Shop Gate Ruler on eBay &#8599;</a>
  ...
  <a href="/blog/gate-ruler-tcg-beginners-guide-australia/" class="quick-link" style="background:rgba(var(--accent-rgb),.08);border-color:rgba(var(--accent-rgb),.3);color:var(--accent)">&#128214; Beginners Guide &#8594;</a>
  <a href="/blog/best-gate-ruler-cards-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128230; Most Valuable Cards &#8594;</a>
</div>

${topCards.length ? `<section class="carousel-section fade-up fade-up-2">
  <div class="carousel-label">Most Valuable</div>
  <div class="carousel-title">Top Gate Ruler Cards by Price (AUD)</div>
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header"><div class="section-title">Search Gate Ruler Cards</div></div>
  ...
  <div class="section fade-up fade-up-3">
    <div class="section-header">
      <div class="section-title">Browse ${sets.length||'11'} Sets</div>
  ...
  <div style="background:rgba(var(--accent-rgb),.04);border:1px solid rgba(var(--accent-rgb),.15);border-radius:var(--radius);padding:22px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px">
    <div>
      <div style="font-size:16px;font-weight:700;color:var(--text);margin-bottom:5px">Track Your Gate Ruler Collection</div>
...
<footer>
  ...
  <p style="margin-top:6px;font-size:11px;opacity:.5">Affiliate disclosure: this site earns commissions from eBay AU and Amazon AU purchases made through affiliate links at no extra cost to you. Not affiliated with Gate Ruler. USD prices converted to AUD at approximately 1.58.</p>
</footer>
...
async function searchCard() {
  ...
  try {
    const res = await fetch('/api/compare-search?q=' + encodeURIComponent(q) + '&game=gateruler&limit=24');
    ...
    results.innerHTML = cards.map(c => {
      ...
      return '<a href="/cards/gateruler/'+c.slug+'" style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center;display:block;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'var(--border)\'">'
...
export const config = { path: '/cards/gateruler' };
