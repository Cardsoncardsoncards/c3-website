...
// netlify/functions/battlespiritssaga-hub.mjs
// Serves /cards/battlespiritssaga
...
const EMOJI       = '&#128293;';
const SET_PH      = 'Aquatic Invaders, Savior of Chaos...';
const CANONICAL   = 'https://cardsoncardsoncards.com.au/cards/battlespiritssaga';
...
export default async (req) => {
  ...
  const [setsRes, cardsRes] = await Promise.allSettled([
    supabaseGet('battlespiritssaga_sets?order=release_date.desc.nullslast&limit=50&select=id,name,slug,release_date,card_count'),
    supabaseGet('battlespiritssaga_cards?order=market_price.desc&market_price=gt.0&image_url=not.is.null&rarity=not.is.null&rarity=neq.None&limit=24&select=slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at'),
  ]);
  ...
  function carouselCard(c, suffix) {
    ...
    const ebay  = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name+' Battle Spirits Saga card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<div class="carousel-card"><a href="/cards/battlespiritssaga/${esc(c.slug)}" style="display:block;text-decoration:none"><div class="carousel-img-wrap"><img src="${esc(c.image_url)}${suffix}" alt="${esc(c.name).replace(/"/g,'&quot;')}" loading="eager" onerror="this.onerror=null;this.src='/card-placeholder.svg';this.style.opacity='1'"></div><div class="carousel-name">${esc(c.name)}</div>${c.rarity?`<div class="carousel-rarity">${esc(c.rarity)}</div>`:''}<div class="carousel-price">${price}</div></a><div class="carousel-buy-row"><a href="${ebay}" target="_blank" rel="noopener" class="carousel-buy-btn">Buy eBay &#8599;</a></div></div>`;
  }
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/battlespiritssaga/sets/${encodeURIComponent(s.slug||s.id)}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}"><span class="set-tile-name">${esc(name)}${newBadge}</span><span class="set-tile-meta">${year}${s.card_count?' &middot; '+s.card_count+' cards':''}</span></a>`;
...
<head>
  ...
  <title>Battle Spirits Saga Card Prices Australia | AUD Prices Updated Daily | C3</title>
  <meta name="description" content="Battle Spirits Saga card prices in AUD. Browse sets and buy on eBay AU.">
  <link rel="canonical" href="${CANONICAL}">
  ${hubSchemaHtml({ game: 'battlespiritssaga', label: "Battle Spirits Saga", name: "Battle Spirits Saga Card Prices Australia", description: "Battle Spirits Saga card prices in AUD. Browse sets and buy on eBay AU." })}
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="Battle Spirits Saga Card Prices Australia | Cards on Cards on Cards">
  <meta property="og:description" content="Battle Spirits Saga card prices in AUD. Browse sets and buy on eBay AU.">
...
<div class="hero fade-up">
  <div class="hero-eyebrow">Card Vault -- Battle Spirits Saga</div>
  <h1>Battle Spirits Saga Card Prices <span>in Australia</span></h1>
  <p class="hero-sub">Battle Spirits Saga card prices in AUD. Browse by set or search by card name. eBay AU buy links updated daily.</p>
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length||'?'}</div><div class="stat-label">Sets</div></div>
    <div class="stat-item"><div class="stat-num">1.9k+</div><div class="stat-label">Cards</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=battle+spirits+saga+card&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:linear-gradient(135deg,#1a1a2e,${ACCENT});color:#fff">&#128722; Shop Battle Spirits Saga on eBay &#8599;</a>
...
${topCards.length ? `<section class="carousel-section fade-up fade-up-2">
  ...
  <div class="carousel-title">Top Battle Spirits Saga Cards by Price (AUD)</div>
...
<div class="wrap">
  ...
  <div class="section fade-up fade-up-3">
    <div style="text-align:center;margin-bottom:16px">
      <h2 style="font-family:'Cinzel',serif;font-size:24px;font-weight:700;color:var(--text);margin-bottom:6px">Browse Every Battle Spirits Saga Set</h2>
  ...
  <div style="margin-bottom:28px">
    <h2 style="font-size:18px;margin-bottom:16px">Battle Spirits Saga Guides and Resources</h2>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px">
      <a href="/ev-calculator.html" class="guide-card"><div class="guide-title">Booster Box EV Calculator</div><div class="guide-desc">Is your Battle Spirits Saga box worth opening? Calculate expected value before you crack it.</div></a>
      ...
      <a href="/tracker.html" class="guide-card"><div class="guide-title">Free Collection Tracker</div><div class="guide-desc">Track your Battle Spirits Saga collection value in AUD with our free Google Sheets tracker.</div></a>
    </div>
  </div>
  ...
  <div style="background:rgba(${ACCENT_RGB},.04);border:1px solid rgba(${ACCENT_RGB},.15);border-radius:var(--radius);padding:22px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px">
    <div>
      <div style="font-size:16px;font-weight:700;color:var(--text);margin-bottom:5px">Track Your Battle Spirits Saga Collection</div>
...
<footer>
  ...
  <div style="margin-bottom:10px">
    <a href="/">Home</a><a href="/blog">Blog</a><a href="/calendar">Calendar</a><a href="/account">Account</a><a href="/tracker">Tracker</a><a href="/cards">Card Prices</a><a href="/compare">Compare</a><a href="/market">Market</a><a href="/tools">Tools</a><a href="/contact">Contact</a><a href="/legal">Legal</a><a href="https://www.ebay.com.au/str/cardsoncardsoncards?mkcid=1&amp;mkrid=705-53470-19255-0&amp;campid=5339146789&amp;customid=Footer&amp;toolid=10001&amp;mkevt=1" target="_blank" rel="noopener" onclick="gtag('event','ebay_click',{'event_category':'affiliate','event_label':'footer'})">eBay</a><a href="https://blasdigital.etsy.com" target="_blank" rel="noopener">D&amp;D Tools on Etsy &#8599;</a><br><a href="/cards/battlespiritssaga">Battle Spirits Saga</a><a href="/cards/pokemon">Pokemon</a><a href="/cards/mtg">MTG</a><a href="/cards/yugioh">Yu-Gi-Oh</a><a href="/cards/lorcana">Lorcana</a>
  </div>
...
export const config = { path: '/cards/battlespiritssaga' };
