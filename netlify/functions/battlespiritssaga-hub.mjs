...
const EMOJI       = '&#9876;';
const SEARCH_PH   = "Tempest Dragon, Mirage, Saga...";
const SET_PH      = "BSS01, BSS02, False Gods...";
...
export default async (req) => {
  ...
  const [setsResult, topCardsResult] = await Promise.allSettled([
    supabaseGet('battlespiritssaga_sets?order=release_date.desc&limit=50&select=id,name,slug,abbreviation,release_date,card_count'),
    supabaseGet('battlespiritssaga_cards?order=market_price.desc&market_price=gt.0.5&market_price=lt.200&image_url=not.is.null&limit=24&select=slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at')
  ]);
  ...
  const carouselHTML = topCards.map(c => {
    ...
    const ebay  = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name+' battle spirits saga card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<a href="/cards/battlespiritssaga/${c.slug}" class="carousel-card">
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/battlespiritssaga/sets/${encodeURIComponent(s.slug||s.id)}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}">
...
<head>
  ...
  <meta name="description" content="Browse ${sets.length||'20'}+ Battle Spirits Saga sets. Live AUD card prices and eBay AU buy links updated daily. Australia's Battle Spirits Saga price guide.">
  <link rel="canonical" href="${CANONICAL}">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="Battle Spirits Saga Card Prices Australia | Cards on Cards on Cards">
  <meta property="og:description" content="${sets.length||'20'}+ Battle Spirits Saga sets with live AUD prices and eBay AU buy links.">
...
<!-- OUT OF PRINT NOTICE -->
  <div style="background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.25);border-radius:10px;padding:14px 18px;margin:0 24px 28px;display:flex;align-items:center;gap:12px">
    ...
    <div>
      ...
      <div style="font-size:13px;color:rgba(160,168,192,.75);line-height:1.5">Battle Spirits Saga is no longer in print. Cards shown are from the collector&#39;s secondary market. Prices reflect current eBay AU and TCGPlayer market values.</div>
    </div>
  </div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=battle+spirits+saga&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=5339146789&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:var(--accent);color:#000">&#128722; Shop Battle Spirits Saga on eBay &#8599;</a>
  ...
  <a href="/blog/battle-spirits-saga-beginners-guide-australia/" class="quick-link" style="background:rgba(var(--accent-rgb),.08);border-color:rgba(var(--accent-rgb),.3);color:var(--accent)">&#128214; Beginners Guide &#8594;</a>
  <a href="/blog/best-battle-spirits-saga-cards-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128230; Most Valuable Cards &#8594;</a>
</div>
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header"><div class="section-title">Search Battle Spirits Saga Cards</div></div>
...
async function searchCard() {
  ...
  try {
    const res = await fetch('/api/compare-search?q=' + encodeURIComponent(q) + '&game=battlespiritssaga&limit=24');
    ...
    results.innerHTML = cards.map(c => {
      ...
      return '<a href="/cards/battlespiritssaga/'+c.slug+'" style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center;display:block;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'var(--border)\'">'
