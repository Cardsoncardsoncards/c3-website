// NAV_CSS is no longer imported here: the only thing that used it was BASE_STYLES, which
// moved to shared/mtg-base-styles.mjs in Task D and imports it directly.
...
//
// Task D. This file used to serve three things through one handler branching on
// url.pathname. /cards/mtg/random-commander moved to mtg-random-commander.mjs, matching the
// one-route-per-file convention every other page type in this repo already follows.
//
// The /cards/mtg branch below is DEAD CODE and is left in place rather than deleted, with
// this note so the next reader does not spend time on it. That path is NOT in this file's
// export const config, so Netlify never routes to it. /cards/mtg is served by mtg-hub.mjs,
// confirmed live: it returns mtg-hub's title and mtg-hub's cache header. Deleting
// renderCardHub and its query would be a larger change than Task D asked for, so it is
...
// BASE_STYLES moved to shared/mtg-base-styles.mjs (Task D) so this file and
...
// renderRandomCommander() moved to mtg-random-commander.mjs (Task D). One route per file,
// matching every other page type in this repo.
...
<footer style="border-top:1px solid var(--border);padding:24px 0;margin-top:48px">
  ...
  <div style="max-width:1100px;margin:0 auto;padding:0 24px;display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;gap:12px">
    ...
    <div style="display:flex;gap:16px;flex-wrap:wrap">
      ...
      <a href="/calendar" style="font-size:12px;color:rgba(160,168,192,.4);text-decoration:none">Calendar</a>
      <a href="/account" style="font-size:12px;color:rgba(160,168,192,.4);text-decoration:none">Account</a>
...
export default async (req) => {
  ...
  if (path.startsWith('/cards/mtg/sets/')) {
    ...
    return new Response(html, {
      // Durations carried over unchanged. What is new is the durable tier and
      // stale-while-revalidate: this route previously set no Netlify-CDN-Cache-Control at all,
      // so the durable cache reported fwd=bypass on every request.
...
export const config = {
  // /cards/mtg/random-commander now lives in mtg-random-commander.mjs (Task D).
  path: ['/cards/mtg/sets/:setSlug']
};
