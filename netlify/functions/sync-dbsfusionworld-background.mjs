...
// Group A sync (Mon/Wed/Fri/Sun) - schedule: 0 23 * * 0,1,3,5 UTC
...
export default async (req) => {
  console.log('[sync-dbsfusionworld] Starting (background function)...');
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-dbsfusionworld] Bulk price error set ${set.id}:`, e.message);
        }
      }
