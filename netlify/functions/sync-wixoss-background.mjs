...
export default async (req) => {
  ...
  console.log('[sync-wixoss] Starting...');
  const start = Date.now();

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('[sync-wixoss] Missing Supabase env vars');
    return new Response('Supabase env vars missing', { status: 500 });
  }
  if (!TCGAPI_KEY) {
    console.error('[sync-wixoss] TCGAPI_KEY not set');
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-wixoss] Bulk price fetch failed for set ${set.id}:`, e.message);
        }
      }
      ...
      // they are not independent: wixoss_price_snapshots.card_id is a FOREIGN KEY onto
      // wixoss_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('wixoss_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('wixoss_price_snapshots', chunk);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-wixoss] Upsert error for set ${set.name}:`, r.reason?.message);
      ...
      console.log(`[sync-wixoss] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }
