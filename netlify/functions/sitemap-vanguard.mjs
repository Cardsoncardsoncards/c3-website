// netlify/functions/sitemap-vanguard.mjs
// Generates XML sitemap for vanguard card pages at /cards/vanguard/[slug]
// Registered in sitemap-index.xml as /api/sitemap-vanguard
...
async function fetchSlugs(offset) {
  const url = SUPABASE_URL + '/rest/v1/vanguard_cards'
...
export default async (req) => {
  ...
  try {
    const countRes = await supabaseFetch(
      SUPABASE_URL + '/rest/v1/vanguard_cards?select=id&market_price=gte.' + PRICE_THRESHOLD + '&slug=not.is.null&limit=1',
      { Prefer: 'count=exact' }
    );
    if (!countRes.ok) return empty('vanguard sitemap: count query failed');

    const contentRange = countRes.headers.get('content-range');
    const totalCount = (contentRange && contentRange.includes('/'))
      ? parseInt(contentRange.split('/')[1], 10) : 0;

    if (totalCount === 0) return empty('vanguard card pages: pending sync or no priced cards');
    ...
    const urlItems = allCards
      ...
      .map(function(c) {
        ...
        return '  <url>\n    <loc>' + SITE_URL + '/cards/vanguard/' + c.slug + '</loc>\n    <lastmod>' + lastmod + '</lastmod>\n    <changefreq>daily</changefreq>\n    <priority>' + priority + '</priority>\n  </url>';
      })
      .join('\n');

    const xml = '<?xml version="1.0" encoding="UTF-8"?>\n'
      + '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'
      + '  <!-- vanguard card pages: ' + allCards.length + ' cards with price >= AUD$' + PRICE_THRESHOLD + ' -->\n'
    ...
    console.error('[sitemap-vanguard] error:', err.message);
    return new Response(
...
export const config = { path: '/api/sitemap-vanguard' };
