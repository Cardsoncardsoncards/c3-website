// netlify/functions/sync-metazoo-background.mjs
// Daily sync -- schedule: 30 8 * * * UTC
// Fetches all metazoo sets + cards + prices from tcgapi.dev Pro
// Upserts into metazoo_sets, metazoo_cards, metazoo_price_snapshots

const SUPABASE_URL         = Netlify.env.get('SUPABASE_URL');
const SUPABASE_SERVICE_KEY = Netlify.env.get('SUPABASE_SERVICE_KEY');
const TCGAPI_KEY           = Netlify.env.get('TCGAPI_KEY');
const SYNC_SECRET          = Netlify.env.get('SYNC_SECRET');
const GAME_SLUG            = 'metazoo';
...
export default async (req) => {
  ...
  console.log('[sync-metazoo] Starting...');
  const start = Date.now();

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('[sync-metazoo] Missing Supabase env vars');
    return new Response('Supabase env vars missing', { status: 500 });
  }
  if (!TCGAPI_KEY) {
    console.error('[sync-metazoo] TCGAPI_KEY not set');
    return new Response('TCGAPI_KEY missing', { status: 500 });
  }

  try {
    const audRate = await getExchangeRate();
    console.log(`[sync-metazoo] AUD rate: ${audRate}`);
    ...
    console.log(`[sync-metazoo] Found ${allSets.length} sets`);
    ...
    for (let i = 0; i < setRows.length; i += 100) {
      await supabaseUpsert('metazoo_sets', setRows.slice(i, i + 100));
    }
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-metazoo] Bulk price fetch failed for set ${set.id}:`, e.message);
        }
      }
      ...
      const results = await Promise.allSettled([
        (async () => {
          for (let i = 0; i < cardRows.length; i += 200) {
            await supabaseUpsert('metazoo_cards', cardRows.slice(i, i + 200));
          }
        })(),
        (async () => {
          for (let i = 0; i < snapRows.length; i += 500) {
            await supabaseUpsertSnapshots('metazoo_price_snapshots', snapRows.slice(i, i + 500));
          }
        })()
      ]);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-metazoo] Upsert error for set ${set.name}:`, r.reason?.message);
        }
      }

      totalCards += cardRows.length;
      totalSnaps += snapRows.length;
      console.log(`[sync-metazoo] ${set.name}: ${cardRows.length} cards, ${snapRows.length} snapshots`);
    }

    const elapsed = ((Date.now() - start) / 1000).toFixed(1);
    console.log(`[sync-metazoo] Done. ${totalCards} cards, ${totalSnaps} snapshots in ${elapsed}s`);
    ...
    console.error('[sync-metazoo] FATAL:', err.message);
