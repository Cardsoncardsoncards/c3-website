...
// netlify/functions/dragonballz-card-page.mjs
// Serves /cards/dragonballz/:slug
...
async function getEbayListings(cardName, token) {
  if (!token) return [];
  try {
    const q = encodeURIComponent(`${cardName} dragon ball z card`);
...
function notFoundPage(slug) {
  return `<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><title>Card Not Found | Cards on Cards on Cards</title><link rel="icon" type="image/png" href="/c3logo.png"></head><body style="background:#0A0C14;color:#F0F2FF;font-family:sans-serif"><style>${NAV_CSS}</style>${navHtml({ gameLabel: 'Dragon Ball Z', gameHref: '/cards/dragonballz' })}<div style="padding:80px 24px;text-align:center"><h1 style="font-family:'Cinzel',serif;color:#C9A84C;margin-bottom:16px">Card Not Found</h1><p style="color:#A0A8C0;margin-bottom:24px">The card "${slug}" doesn't exist or hasn't synced yet.</p><a href="/cards/dragonballz" style="display:inline-block;padding:10px 24px;background:rgba(201,168,76,.15);border:1px solid rgba(201,168,76,.4);color:#E8C97A;border-radius:8px;text-decoration:none">Browse Dragon Ball Z TCG Cards</a></div></body></html>`;
}

function setNotFoundPage(setSlug) {
  return `<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><title>Set Not Found | Dragon Ball Z TCG | Cards on Cards on Cards</title><meta name="robots" content="noindex"><link rel="icon" type="image/png" href="/c3logo.png"><link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@700&family=DM+Sans:wght@400;600&display=swap" rel="stylesheet"></head><body style="background:#0A0C14;color:#F0F2FF;font-family:'DM Sans',sans-serif"><style>${NAV_CSS}</style>${navHtml({ gameLabel: 'Dragon Ball Z', gameHref: '/cards/dragonballz' })}<div style="display:flex;align-items:center;justify-content:center;min-height:60vh;padding:24px;text-align:center"><div><div style="font-size:48px;margin-bottom:16px">🃏</div><h1 style="font-family:'Cinzel',serif;color:#FF6F00;font-size:22px;margin-bottom:10px">Set Not Found</h1><p style="color:#8892b0;font-size:14px;margin-bottom:24px">We couldn't find the Dragon Ball Z TCG set "${setSlug}".</p><a href="/cards/dragonballz" style="display:inline-block;background:#FF6F00;color:#000;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px">Browse All Dragon Ball Z</a><a href="/" style="display:inline-block;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);color:#F0F2FF;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px">← Home</a></div></div></body></html>`;
}
...
async function handleSetPage(setSlug, htmlHeaders) {
  ...
  const [_psr0, _psr1] = await Promise.allSettled([
    supabaseGet(`dragonballz_sets?slug=eq.${encodeURIComponent(setSlug)}&limit=1&select=*`),
    getEbayToken()
  ]);
  ...
  const [_psr2, _psr3] = await Promise.allSettled([
    supabaseGet(`dragonballz_cards?set_id=eq.${set.id}&order=market_price.desc.nullslast&limit=400&select=slug,name,number,image_url,market_price,price_aud,rarity,set_name`),
    ebayToken ? getEbayListings(set.name + ' dragon ball z', ebayToken).catch(() => []) : []
  ]);
  ...
  const ebaySetURL = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name + ' dragon ball z')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
  const metaDesc = `Browse ${cards?.length || 0} Dragon Ball Z TCG cards from ${set.name}. View card prices in AUD and buy on eBay AU. Updated daily.`;

  const top5HTML = top5.map(c => {
    const aud = toAud(c);
    return `<a href="/cards/dragonballz/${c.slug}" style="flex:0 0 140px;background:#0e1118;border:1px solid rgba(16,185,129,.35);border-radius:10px;padding:10px;text-align:center;text-decoration:none;display:block">
  ...
  const allCardsHTML = (cards && cards.length) ? cards.filter(isSingles).map(c => {
    const aud = toAud(c);
    return `<a href="/cards/dragonballz/${c.slug}" style="background:#0e1118;border:1px solid #1e2235;border-radius:8px;padding:8px;text-decoration:none;text-align:center;display:block">
...
<head>
  ...
  <title>${set.name} | Dragon Ball Z TCG Set | Cards on Cards on Cards</title>
  <meta name="description" content="${metaDesc}">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/dragonballz/sets/${setSlug}">
  <meta property="og:title" content="${set.name} | Dragon Ball Z TCG | C3">
...
<body>
<style>${NAV_CSS}</style>${navHtml({ gameLabel: 'Dragon Ball Z', gameHref: '/cards/dragonballz' })}
<div class="wrap">
  <div class="hero">
    <div class="hero-eyebrow">Dragon Ball Z TCG · Set</div>
    <h1 class="hero-title">${set.name}</h1>
    <div class="hero-meta">
      <span class="meta-badge">Dragon Ball Z TCG</span>
  ...
  <div class="cta-row">
    ...
    <a href="https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name + ' dragon ball z booster box')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="cta-btn cta-secondary">Find Booster Box →</a>
    <a href="https://www.amazon.com.au/s?k=${encodeURIComponent(set.name + ' Dragon Ball Z TCG')}&tag=${AMAZON_TAG}" target="_blank" rel="noopener" class="cta-btn cta-secondary" style="border-color:rgba(255,153,0,.35);color:#ff9900">Amazon AU →</a>
  </div>
  ...
  <div style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:20px;font-size:13px;color:#8892b0">
    <strong style="color:#F0F2FF">About this set:</strong> Dragon Ball Z TCG card prices shown in AUD, converted from USD market data. Prices update daily via tcgapi.dev.
    <div style="margin-top:10px"><a href="/cards/dragonballz" style="color:${accent}">← Back to all Dragon Ball Z TCG cards</a></div>
  </div>
</div>
...
export default async (req) => {
  ...
  const slug = url.pathname.replace('/cards/dragonballz/', '').replace(/^\/|\/$/g, '');
  ...
  try {
    const cards = await supabaseGet(`dragonballz_cards?slug=eq.${encodeURIComponent(slug)}&limit=1&select=*`);
    ...
    const card = cards[0];
    const _setRows = card.set_id ? await supabaseGet(`dragonballz_sets?id=eq.${card.set_id}&limit=1&select=slug`).catch(() => []) : [];
    const setUrl = (Array.isArray(_setRows) && _setRows[0] && _setRows[0].slug) ? `/cards/dragonballz/sets/${_setRows[0].slug}` : `/cards/dragonballz`;

    const priceAud = parseFloat(card.price_aud) || (card.market_price ? card.market_price * audRate : null);
    const pageUrl = encodeURIComponent(`https://cardsoncardsoncards.com.au/cards/dragonballz/${card.slug}`);
    const shareText = encodeURIComponent(`${card.name} Dragon Ball Z TCG Card Game -- ${priceAud ? 'AU$'+priceAud.toFixed(2) : 'check price'} on Cards on Cards on Cards`);
    const ebaySearchUrl = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(card.name+' dragon ball z card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;

    const [_psr0, _psr1] = await Promise.allSettled([
      supabaseGet(`dragonballz_cards?set_id=eq.${card.set_id}&slug=neq.${encodeURIComponent(slug)}&market_price=gt.0&image_url=not.is.null&limit=12&order=market_price.desc&select=*`).catch(() => []),
  ...
  const ebayToken = _psr1.status === 'fulfilled' ? _psr1.value : [];
    ...
    const breadcrumb = { "@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[
      ...
      {"@type":"ListItem","position":2,"name":"Dragon Ball Z TCG Cards","item":"https://cardsoncardsoncards.com.au/cards/dragonballz"},
      {"@type":"ListItem","position":3,"name":card.name,"item":`https://cardsoncardsoncards.com.au/cards/dragonballz/${card.slug}`}
    ]};

    const productSchema = priceAud ? { "@context":"https://schema.org","@type":"Product","name":card.name,"image":card.image_url||'',
      "offers":{"@type":"Offer","priceCurrency":"AUD","price":priceAud.toFixed(2),"availability":"https://schema.org/InStock","url":`https://cardsoncardsoncards.com.au/cards/dragonballz/${card.slug}`}
    } : null;
    ...
    <div style="max-width:1100px;margin:0 auto 32px;padding:0 24px">
      ...
      <div style="display:flex;gap:12px;overflow-x:auto;padding-bottom:8px;scrollbar-width:none">
        ${relatedCards.map(c => {
          ...
          return `<a href="/cards/dragonballz/${c.slug}" style="flex:0 0 130px;background:#161929;border:1px solid #252840;border-radius:8px;padding:8px;text-decoration:none">
...
<head>
  ...
  <title>${card.name} Price Australia | Dragon Ball Z TCG | Cards on Cards on Cards</title>
  <meta name="description" content="${card.name} Dragon Ball Z TCG card${priceAud ? ` -- ~AU$${priceAud.toFixed(2)}` : ''}. ${card.rarity ? `${card.rarity}. ` : ''}View price and buy on eBay AU.">
  <meta property="og:site_name" content="Cards on Cards on Cards">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/dragonballz/${card.slug}">
  ...
  <meta property="og:title" content="${card.name} | Dragon Ball Z TCG Price AU">
...
<div class="card-hero">
  ...
  <div class="card-details">
    <div style="font-size:12px;color:var(--text2);margin-bottom:8px">
      <a href="/cards/dragonballz" style="color:var(--text2);text-decoration:none">Dragon Ball Z TCG</a>
    ...
    ${priceAud ? '' : `<p style="font-size:12px;color:var(--text2);line-height:1.5;margin:-4px 0 12px;max-width:460px">We have no current market price for this card. That means it has not traded recently in the data we track, not that it is worth nothing. Pricing data covers only part of the Dragon Ball Z TCG catalogue. Check the live eBay AU listings below for what it is actually selling for.</p>`}
    <div class="meta-grid">
    ...
    ${followBlockHtml({ game: 'dragonballz', slug: card.slug, cardName: card.name, printingId: card.tcgplayer_id })}
...
function addToCompare(slug,name,img,price,game){
  ...
  tray.push({slug,name,img,price,game:'dragonballz'});saveCompareTray(tray);renderCompareTray();
}
function removeFromCompare(slug){saveCompareTray(getCompareTray().filter(c=>c.slug!==slug));renderCompareTray();}
function goToCompare(){const tray=getCompareTray();if(!tray.length)return;window.location.href='/compare?cards='+tray.map(c=>(c.game||'dragonballz')+':'+c.slug).join(',');}
renderCompareTray();
...
${viewTrackingScript('dragonballz', card.slug)}
</body>
</html>`;

    return new Response(html, { status: 200, headers });
  } catch (err) {
    console.error('[dragonballz-card-page]', err.message);
...
export const config = { path: '/cards/dragonballz/:slug+', excludedPath: '/cards/dragonballz/sets/*' };
