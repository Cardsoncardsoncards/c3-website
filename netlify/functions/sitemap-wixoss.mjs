// netlify/functions/sitemap-wixoss.mjs
// Generates XML sitemap for wixoss card pages at /cards/wixoss/[slug]
// Registered in sitemap-index.xml as /api/sitemap-wixoss
...
export default async (req) => {
  ...
  try {
    const countRes = await supabaseFetch(
      SUPABASE_URL + '/rest/v1/wixoss_cards?select=id&market_price=gte.' + PRICE_THRESHOLD + '&slug=not.is.null&limit=1',
      { Prefer: 'count=exact' }
    );
    if (!countRes.ok) return empty('wixoss sitemap: count query failed');
    ...
    const urlItems = allCards
      ...
      .map(function(c) {
        ...
        return '  <url>\n    <loc>' + SITE_URL + '/cards/wixoss/' + c.slug + '</loc>\n    <lastmod>' + lastmod + '</lastmod>\n    <changefreq>daily</changefreq>\n    <priority>' + priority + '</priority>\n  </url>';
      })
      .join('\n');

    const xml = '<?xml version="1.0" encoding="UTF-8"?>\n'
      + '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'
      + '  <!-- wixoss card pages: ' + allCards.length + ' cards with price >= AUD$' + PRICE_THRESHOLD + ' -->\n'
    ...
    console.error('[sitemap-wixoss] error:', err.message);
    return new Response(
...
export const config = { path: '/api/sitemap-wixoss' };
