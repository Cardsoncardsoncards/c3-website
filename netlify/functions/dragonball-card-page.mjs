...
async function getEbayListings(cardName, token) {
  if (!token) return [];
  try {
    const q = encodeURIComponent(`${cardName} dragon ball super`);
...
export default async (req) => {
  ...
  const breadcrumb_dragonball = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 3, "name": card.set_name || "Dragon Ball Super", "item": `https://cardsoncardsoncards.com.au/cards/dragonball/sets/${encodeURIComponent(card.set_name||'')}` },
      { "@type": "ListItem", "position": 4, "name": card.name, "item": `https://cardsoncardsoncards.com.au/cards/dragonball/${card.slug}` }
    ]
  };
  const productSchema_dragonball = priceAUD > 0 ? {
    "@context": "https://schema.org", "@type": "Product",
    "name": card.name,
    "description": `${card.name} Dragon Ball Super card from ${card.set_name||'Dragon Ball Super'}.`,
...
<head>
  ...
  <meta name="description" content="${card.name} card price in Australia.${priceAUD > 0 ? ' Currently AU$'+priceAUD.toFixed(2)+'.' : ''} Live AUD pricing and eBay AU buy links for Dragon Ball Super cards.">
  ...
  <script type="application/ld+json">${JSON.stringify(breadcrumb_dragonball)}</script>
  ${productSchema_dragonball ? `<script type="application/ld+json">${JSON.stringify(productSchema_dragonball)}</script>` : ''}
  <script type="application/ld+json">${JSON.stringify(faqSchema_dragonball)}</script>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .price-block{background:#111420;border:1px solid #F9731640;border-radius:12px;padding:20px;margin:20px 0}
    .price-main{font-family:'Cinzel',serif;font-size:32px;font-weight:900;color:#F97316}
...
<div class="wrap">
  <div class="breadcrumb"><a href="/">Home</a> → <a href="/cards/dragonball">Dragon Ball Super</a>${card.set_name ? ` → <a href="/cards/dragonball/sets/${encodeURIComponent(card.set_id||'')}">${card.set_name}</a>` : ''} → ${card.name}</div>
  ...
  <div class="card-layout">
    ...
    <div>
      ...
      <div class="meta-grid">
        ...
        <div class="meta-item"><div class="meta-label">Game</div><div class="meta-value">Dragon Ball Super</div></div>
      </div>
  ...
  <h2 class="section-head">Sealed ${esc(card.set_name || 'Dragon Ball Super')} Product</h2>
  <div style="background:#111420;border:1px solid #252840;border-radius:12px;padding:20px;margin-bottom:24px">
    ...
    <a href="https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent((card.set_name||'Dragon Ball Super')+' booster box dragon ball super')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener sponsored" style="display:inline-block;background:#F97316;color:#000;padding:11px 22px;border-radius:8px;font-weight:700;font-size:14px;text-decoration:none">&#128230; Shop Sealed Dragon Ball Super on eBay &#8599;</a>
  </div>
</div>
