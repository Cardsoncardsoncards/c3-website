...
async function getEbayListings(cardName, token) {
  if (!token) return [];
  try {
    const q = encodeURIComponent(`${cardName} dragon ball super card`);
...
function notFoundPage(slug) {
  return `<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><title>Card Not Found | Cards on Cards on Cards</title><link rel="icon" type="image/png" href="/c3logo.png"></head><body style="background:#0A0C14;color:#F0F2FF;font-family:sans-serif"><style>${NAV_CSS}</style>${navHtml({ gameLabel: 'Dragon Ball Super', gameHref: '/cards/dragonball' })}<div style="padding:80px 24px;text-align:center"><h1 style="font-family:'Cinzel',serif;color:#C9A84C;margin-bottom:16px">Card Not Found</h1><p style="color:#A0A8C0;margin-bottom:24px">The card "${slug}" doesn't exist or hasn't synced yet.</p><a href="/cards/dragonball" style="display:inline-block;padding:10px 24px;background:rgba(201,168,76,.15);border:1px solid rgba(201,168,76,.4);color:#E8C97A;border-radius:8px;text-decoration:none">Browse Dragon Ball Super Cards</a></div></body></html>`;
}

function setNotFoundPage(setSlug) {
  return `<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><title>Set Not Found | Dragon Ball Super | Cards on Cards on Cards</title><meta name="robots" content="noindex"><link rel="icon" type="image/png" href="/c3logo.png"><link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@700&family=DM+Sans:wght@400;600&display=swap" rel="stylesheet"></head><body style="background:#0A0C14;color:#F0F2FF;font-family:'DM Sans',sans-serif"><style>${NAV_CSS}</style>${navHtml({ gameLabel: 'Dragon Ball Super', gameHref: '/cards/dragonball' })}<div style="display:flex;align-items:center;justify-content:center;min-height:60vh;padding:24px;text-align:center"><div><div style="font-size:48px;margin-bottom:16px">🃏</div><h1 style="font-family:'Cinzel',serif;color:#FF6600;font-size:22px;margin-bottom:10px">Set Not Found</h1><p style="color:#8892b0;font-size:14px;margin-bottom:24px">We couldn't find the Dragon Ball Super set "${setSlug}".</p><a href="/cards/dragonball" style="display:inline-block;background:#FF6600;color:#000;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px">Browse All Dragon Ball</a><a href="/" style="display:inline-block;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);color:#F0F2FF;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px">← Home</a></div></div></body></html>`;
}
...
async function handleSetPage(setSlug, htmlHeaders) {
  ...
  const [_psr2, _psr3] = await Promise.allSettled([
    ...
    ebayToken ? getEbayListings(set.name + ' dragon ball super', ebayToken).catch(() => []) : []
  ]);
  ...
  const top5HTML = top5.map(c => {
    const aud = toAud(c);
    return `<a href="/cards/dragonball/${c.slug}" style="flex:0 0 140px;background:#0e1118;border:1px solid rgba(16,185,129,.35);border-radius:10px;padding:10px;text-align:center;text-decoration:none;display:block">
...
<head>
  ...
  <title>${set.name} | Dragon Ball Super Set | Cards on Cards on Cards</title>
  <meta name="description" content="${metaDesc}">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/dragonball/sets/${setSlug}">
  <meta property="og:title" content="${set.name} | Dragon Ball Super | C3">
...
<body>
<style>${NAV_CSS}</style>${navHtml({ gameLabel: 'Dragon Ball Super', gameHref: '/cards/dragonball' })}
<div class="wrap">
  <div class="hero">
  ...
  <div class="cta-row">
    ...
    <a href="https://www.amazon.com.au/s?k=${encodeURIComponent(set.name + ' Dragon Ball Super')}&tag=${AMAZON_TAG}" target="_blank" rel="noopener" class="cta-btn cta-secondary" style="border-color:rgba(255,153,0,.35);color:#ff9900">Amazon AU →</a>
  </div>
  ...
  <div style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:20px;font-size:13px;color:#8892b0">
    <strong style="color:#F0F2FF">About this set:</strong> Dragon Ball Super card prices shown in AUD, converted from USD market data. Prices update daily via tcgapi.dev.
    <div style="margin-top:10px"><a href="/cards/dragonball" style="color:${accent}">← Back to all Dragon Ball Super cards</a></div>
  </div>
</div>
...
export default async (req) => {
  ...
  try {
    const cards = await supabaseGet(`dragonball_cards?slug=eq.${encodeURIComponent(slug)}&limit=1&select=*`);
    ...
    const snapshots = await supabaseGet(`dragonball_price_snapshots?card_id=eq.${encodeURIComponent(card.id)}&snapshot_date=gte.${new Date(Date.now()-90*86400000).toISOString().slice(0,10)}&order=snapshot_date.asc&limit=400&select=snapshot_date,price_aud,market_price`).catch(() => []);
  ...
  const ebayToken = _psr1.status === 'fulfilled' ? _psr1.value : [];
    ...
    const breadcrumb = { "@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[
      ...
      {"@type":"ListItem","position":2,"name":"Dragon Ball Super Cards","item":"https://cardsoncardsoncards.com.au/cards/dragonball"},
      {"@type":"ListItem","position":3,"name":card.name,"item":`https://cardsoncardsoncards.com.au/cards/dragonball/${card.slug}`}
    ]};

    const productSchema = priceAud ? { "@context":"https://schema.org","@type":"Product","name":card.name,"image":card.image_url||'',
      "offers":{"@type":"Offer","priceCurrency":"AUD","price":priceAud.toFixed(2),"availability":"https://schema.org/InStock","url":`https://cardsoncardsoncards.com.au/cards/dragonball/${card.slug}`}
    } : null;
...
<head>
  ...
  <title>${card.name} Price Australia | Dragon Ball Super | Cards on Cards on Cards</title>
  <meta name="description" content="${card.name} Dragon Ball Super card${priceAud ? ` -- ~AU$${priceAud.toFixed(2)}` : ''}. ${card.rarity ? `${card.rarity}. ` : ''}View price and buy on eBay AU.">
  <meta property="og:site_name" content="Cards on Cards on Cards">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/dragonball/${card.slug}">
  ...
  <meta property="og:title" content="${card.name} | Dragon Ball Super Price AU">
...
<div class="card-hero">
  ...
  <div class="card-details">
    <div style="font-size:12px;color:var(--text2);margin-bottom:8px">
      <a href="/cards/dragonball" style="color:var(--text2);text-decoration:none">Dragon Ball Super</a>
    ...
    ${followBlockHtml({ game: 'dragonball', slug: card.slug, cardName: card.name, printingId: card.tcgplayer_id })}
...
function addToCompare(slug,name,img,price,game){
  ...
  tray.push({slug,name,img,price,game:'dragonball'});saveCompareTray(tray);renderCompareTray();
}
...
${viewTrackingScript('dragonball', card.slug)}
</body>
</html>`;
...
export const config = { path: '/cards/dragonball/:slug+', excludedPath: '/cards/dragonball/sets/*' };
