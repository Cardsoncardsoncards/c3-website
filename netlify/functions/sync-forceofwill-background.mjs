...
export default async (req) => {
  ...
  console.log('[sync-forceofwill] Starting...');
  const start = Date.now();

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('[sync-forceofwill] Missing Supabase env vars');
    return new Response('Supabase env vars missing', { status: 500 });
  }
  if (!TCGAPI_KEY) {
    console.error('[sync-forceofwill] TCGAPI_KEY not set');
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-forceofwill] Bulk price fetch failed for set ${set.id}:`, e.message);
        }
      }
      ...
      for (const card of setCards) {
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-forceofwill-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: forceofwill_price_snapshots.card_id is a FOREIGN KEY onto
      // forceofwill_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('forceofwill_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('forceofwill_price_snapshots', chunk);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-forceofwill] Upsert error for set ${set.name}:`, r.reason?.message);
      ...
      console.log(`[sync-forceofwill] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }
