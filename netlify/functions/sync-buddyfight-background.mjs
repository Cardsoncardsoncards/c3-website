// netlify/functions/sync-buddyfight-background.mjs
// Daily sync -- schedule: 0 3 * * * UTC
// Fetches all future-card-buddyfight sets + cards + prices from tcgapi.dev Pro
// Upserts into buddyfight_sets, buddyfight_cards, buddyfight_price_snapshots
...
const GAME_SLUG            = 'future-card-buddyfight';
...
export default async (req) => {
  ...
  console.log('[sync-buddyfight] Starting...');
  const start = Date.now();

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('[sync-buddyfight] Missing Supabase env vars');
    return new Response('Supabase env vars missing', { status: 500 });
  }
  if (!TCGAPI_KEY) {
    console.error('[sync-buddyfight] TCGAPI_KEY not set');
    return new Response('TCGAPI_KEY missing', { status: 500 });
  }

  try {
    const audRate = await getExchangeRate();
    console.log(`[sync-buddyfight] AUD rate: ${audRate}`);
    ...
    console.log(`[sync-buddyfight] Found ${allSets.length} sets`);
    ...
    for (let i = 0; i < setRows.length; i += 100) {
      await supabaseUpsert('buddyfight_sets', setRows.slice(i, i + 100));
    }
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-buddyfight] Bulk price fetch failed for set ${set.id}:`, e.message);
        }
      }
      ...
      for (const card of setCards) {
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-buddyfight-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: buddyfight_price_snapshots.card_id is a FOREIGN KEY onto
      // buddyfight_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('buddyfight_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('buddyfight_price_snapshots', chunk);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-buddyfight] Upsert error for set ${set.name}:`, r.reason?.message);
      ...
      console.log(`[sync-buddyfight] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }

    const elapsed = ((Date.now() - start) / 1000).toFixed(1);
    console.log(`[sync-buddyfight] Done. ${totalCards} cards, ${totalSnaps} snapshots in ${elapsed}s`);
    ...
    console.error('[sync-buddyfight] FATAL:', err.message);
