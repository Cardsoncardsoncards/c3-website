...
// Group B sync (Tue/Thu/Sat) - schedule: 30 23 * * 2,4,6 UTC
// Fetches all grand-archive sets + cards + prices from tcgapi.dev Pro
...
export default async (req) => {
  console.log('[sync-grandarchive] Starting (background function)...');
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-grandarchive] Bulk price error set ${set.id}:`, e.message);
        }
      }
