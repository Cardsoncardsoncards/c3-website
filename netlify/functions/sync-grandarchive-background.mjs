...
const GAME_SLUG            = 'grand-archive'; // task-137: tcgapi.dev renamed from 'grand-archive-tcg'
...
export default async (req) => {
  ...
  try {
    ...
    for (const set of allSets) {
      ...
      for (const card of setCards) {
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-grandarchive-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: grandarchive_price_snapshots.card_id is a FOREIGN KEY onto
      // grandarchive_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('grandarchive_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('grandarchive_price_snapshots', chunk);
      ...
      console.log(`[sync-grandarchive] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }
