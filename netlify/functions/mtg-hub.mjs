...
// Schema verified: mtg_sets uses set_slug for URLs, release_date (not released_at)
// mtg_cards uses image_uri_small (not image_url)
...
const CALENDAR_EVENTS = [
  {date:'2026-06-13',name:'Final Fantasy (MTG)',          type:'Set Release'},
...
export default async (req) => {
  ...
  const [allSets, topCards] = await Promise.all([
    supabaseGet('mtg_sets?order=release_date.desc&limit=800&select=set_code,set_slug,name,set_type,release_date,card_count'),
  ...
  const carouselHTML = topCards.length ? topCards.map(c => {
    ...
    return `<a href="/cards/mtg/${c.slug}" class="carousel-card">
      <div class="carousel-img-wrap">
        <img src="${c.image_uri_small}" alt="${c.name.replace(/"/g,'')}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=card-placeholder>&#9876;</div>'">
      </div>
  ...
  // MTG set links use set_slug (e.g. "eld") - matches card-index.mjs route /cards/mtg/sets/:setSlug
  const setListHTML = sets.length ? sets.map(s => {
    ...
    const urlSlug = encodeURIComponent(s.set_slug || s.set_code || '');
    return `<a href="/cards/mtg/sets/${urlSlug}" class="set-tile" data-name="${name.toLowerCase().replace(/"/g,'&quot;')}" data-letter="${letterKey}">
...
<head>
  ...
  <title>MTG Card Prices Australia | Magic: The Gathering AUD | C3</title>
  <meta name="description" content="Browse ${sets.length}+ MTG sets. Live AUD card prices, eBay AU buy links. Australia's Magic: The Gathering price guide updated daily.">
  ...
  <style>
    :root{--bg:#0A0C14;--bg2:#111420;--bg3:#181d2e;--gold:#C9A84C;--accent:#C9A84C;--text:#e8eaf0;--text2:#9ba3c4;--border:#242840;--radius:12px;--silver:#A0A8C0}
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .carousel-buy-btn{font-size:10px;font-weight:700;color:#fff;background:var(--accent);padding:3px 8px;border-radius:4px;letter-spacing:.04em;text-decoration:none}
    ...
    .set-tile:hover{border-color:var(--accent);background:rgba(201,168,76,.04);text-decoration:none;transform:translateX(2px)}
...
<div class="hero fade-up">
  <div class="hero-eyebrow">Card Vault &#183; Magic: The Gathering</div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=mtg+magic+gathering+cards&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:linear-gradient(135deg,#7a621e,var(--accent));color:#000">&#128722; Shop MTG on eBay &#8599;</a>
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header"><div class="section-title">Search MTG Cards</div></div>
    <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
      <input type="text" id="card-search" placeholder="Card name e.g. Black Lotus, Lightning Bolt..." onkeyup="if(event.key==='Enter')searchCard()">
  ...
  <div class="section fade-up fade-up-3">
    ...
    <input type="text" id="set-search" placeholder="Search sets e.g. Bloomburrow, Tarkir, Modern Horizons..." oninput="filterSets(this.value)" style="margin-bottom:12px">
    <div id="set-list" class="set-grid">${setListHTML}</div>
  </div>
  <div style="background:rgba(201,168,76,.04);border:1px solid rgba(201,168,76,.15);border-radius:var(--radius);padding:22px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px">
    <div>
      <div style="font-size:16px;font-weight:700;color:var(--text);margin-bottom:5px">Find Your Next Commander</div>
      <p style="font-size:13px;color:var(--text2)">Filter by colour identity. Get 4 random commanders and challenge a friend.</p>
    </div>
    <a href="/cards/mtg/random-commander" class="btn btn-primary">Random Commander &#8594;</a>
  </div>
</div>
<footer>
  <div style="margin-bottom:10px">
    <a href="/">Home</a><a href="/cards">Card Vault</a><a href="/cards/mtg">MTG</a>
    <a href="/cards/pokemon">Pokemon</a><a href="/cards/lorcana">Lorcana</a><a href="/cards/yugioh">Yu-Gi-Oh</a>
...
async function searchCard() {
  ...
  try {
    const res = await fetch('/api/compare-search?q=' + encodeURIComponent(q) + '&game=mtg&limit=24');
    const data = await res.json();
    ...
    results.innerHTML = cards.map(c => {
      const img = c.image_uri_small || c.image_url || '';
      const price = c.price_aud ? 'AU$'+parseFloat(c.price_aud).toFixed(0) : '';
      return '<a href="/cards/mtg/'+c.slug+'" style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center;display:block;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'var(--border)\'">'
