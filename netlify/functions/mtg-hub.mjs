...
export default async (req) => {
  ...
  const topCardHTML = topCardsData.map(function(c) {
    ...
    return '<div class="top-card" style="flex-shrink:0;width:155px;display:inline-block;background:rgba(201,168,76,.05);border:1px solid rgba(201,168,76,.15);border-radius:10px;padding:10px;text-align:center;transition:all .25s;vertical-align:top">'
      ...
      + '</a>'
      + '<a href="' + ebayHref + '" target="_blank" rel="noopener" onclick="gtag(\'event\',\'ebay_click\',{\'event_category\':\'affiliate\',\'event_label\':\'mtg-hub\'})" style="display:inline-block;font-size:10px;background:rgba(201,168,76,.15);color:var(--accent);border:1px solid rgba(201,168,76,.3);border-radius:4px;padding:3px 8px;text-decoration:none">Buy eBay &#8599;</a>'
      + '</div>';
  }).join('');
  ...
  const UPCOMING_DATES = [
    ...
    { name: 'Reality Fracture', date: '2026-10-02', type: 'Set Release' },
    { name: 'Star Trek', date: '2026-11-06', type: 'Set Release' },
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .ticker-badge{font-size:9px;font-weight:800;background:rgba(201,168,76,.15);color:var(--gold);border:1px solid rgba(201,168,76,.3);border-radius:3px;padding:1px 5px;letter-spacing:.06em;margin-right:6px}
    /* ROTATION WARNING */
    ...
    /* TOP CARDS CAROUSEL */
    @keyframes topScrollLeft{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}
    .top-carousel-wrap{overflow:hidden;position:relative;margin-bottom:8px}
    .top-carousel-wrap::before,.top-carousel-wrap::after{content:'';position:absolute;top:0;bottom:0;width:60px;z-index:2;pointer-events:none}
    .top-carousel-wrap::before{left:0;background:linear-gradient(to right,var(--bg),transparent)}
    .top-carousel-wrap::after{right:0;background:linear-gradient(to left,var(--bg),transparent)}
    .top-carousel-track{display:flex;gap:12px;padding:4px 24px 12px;animation:topScrollLeft 50s linear infinite;width:max-content}
    .top-carousel-track:hover{animation-play-state:paused}
    /* COMMANDER CAROUSEL */
...
<style>${NAV_CSS}</style>${navHtml({ gameLabel: 'MTG', gameHref: '/cards/mtg' })}
...
<div class="wrap">
  ...
  <div style="background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);padding:24px;margin-bottom:32px">
    <div style="text-align:center;margin-bottom:16px">
      <h2 style="font-size:24px;font-family:Cinzel,serif;margin-bottom:6px">Browse Every MTG Set</h2>
