// netlify/functions/sitemap-weissschwarz.mjs
// Generates XML sitemap for weissschwarz card pages at /cards/weissschwarz/[slug]
// Registered in sitemap-index.xml as /api/sitemap-weissschwarz
...
async function fetchSlugs(offset) {
  const url = SUPABASE_URL + '/rest/v1/weissschwarz_cards'
...
export default async (req) => {
  ...
  try {
    const countRes = await supabaseFetch(
      SUPABASE_URL + '/rest/v1/weissschwarz_cards?select=id&market_price=gte.' + PRICE_THRESHOLD + '&slug=not.is.null&limit=1',
      { Prefer: 'count=exact' }
    );
    if (!countRes.ok) return empty('weissschwarz sitemap: count query failed');

    const contentRange = countRes.headers.get('content-range');
    const totalCount = (contentRange && contentRange.includes('/'))
      ? parseInt(contentRange.split('/')[1], 10) : 0;

    if (totalCount === 0) return empty('weissschwarz card pages: pending sync or no priced cards');
    ...
    const urlItems = allCards
      ...
      .map(function(c) {
        ...
        return '  <url>\n    <loc>' + SITE_URL + '/cards/weissschwarz/' + c.slug + '</loc>\n    <lastmod>' + lastmod + '</lastmod>\n    <changefreq>daily</changefreq>\n    <priority>' + priority + '</priority>\n  </url>';
      })
      .join('\n');
    ...
    console.error('[sitemap-weissschwarz] error:', err.message);
    return new Response(
...
export const config = { path: '/api/sitemap-weissschwarz' };
