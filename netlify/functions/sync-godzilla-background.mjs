// netlify/functions/sync-godzilla-background.mjs
// Daily sync -- schedule: 0 11 * * * UTC
// Fetches all godzilla-tcg sets + cards + prices from tcgapi.dev Pro
// Upserts into godzilla_sets, godzilla_cards, godzilla_price_snapshots
...
export default async (req) => {
  ...
  console.log('[sync-godzilla] Starting...');
  const start = Date.now();

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('[sync-godzilla] Missing Supabase env vars');
    return new Response('Supabase env vars missing', { status: 500 });
  }
  if (!TCGAPI_KEY) {
    console.error('[sync-godzilla] TCGAPI_KEY not set');
    return new Response('TCGAPI_KEY missing', { status: 500 });
  }

  try {
    const audRate = await getExchangeRate();
    console.log(`[sync-godzilla] AUD rate: ${audRate}`);
    ...
    console.log(`[sync-godzilla] Found ${allSets.length} sets`);
    ...
    for (let i = 0; i < setRows.length; i += 100) {
      await supabaseUpsert('godzilla_sets', setRows.slice(i, i + 100));
    }
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-godzilla] Bulk price fetch failed for set ${set.id}:`, e.message);
        }
      }
      ...
      for (const card of setCards) {
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-godzilla-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: godzilla_price_snapshots.card_id is a FOREIGN KEY onto
      // godzilla_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('godzilla_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('godzilla_price_snapshots', chunk);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-godzilla] Upsert error for set ${set.name}:`, r.reason?.message);
      ...
      console.log(`[sync-godzilla] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }

    const elapsed = ((Date.now() - start) / 1000).toFixed(1);
    console.log(`[sync-godzilla] Done. ${totalCards} cards, ${totalSnaps} snapshots in ${elapsed}s`);
    ...
    console.error('[sync-godzilla] FATAL:', err.message);
