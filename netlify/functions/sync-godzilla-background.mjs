...
// Group B sync (Tue/Thu/Sat) - schedule: 15 1 * * 3,5,0 UTC
// Fetches all godzilla-card-game sets + cards + prices from tcgapi.dev Pro
...
const GAME_SLUG            = 'godzilla-card-game';
...
export default async (req) => {
  console.log('[sync-godzilla] Starting (background function)...');
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-godzilla] Bulk price error set ${set.id}:`, e.message);
        }
      }
