...
const GAME       = 'starwars';
const GAME_LABEL = 'Star Wars: Unlimited';
const ACCENT     = '#FFE81F';
const ACCENT_RGB = '255,232,31';
const EMOJI      = '&#11088;';
const SET_PH     = 'Spark of Rebellion, Shadows of the Galaxy, Twilight of the Republic...';
const CANONICAL  = 'https://cardsoncardsoncards.com.au/cards/starwars';
...
export default async (req) => {
  ...
  const [setsRes, cardsRes, gainersRes, losersRes] = await Promise.allSettled([
    ...
    supabaseGet('starwars_cards?order=price_aud.desc&price_aud=gt.0&image_url=not.is.null&rarity=not.is.null&rarity=neq.None&limit=24&select=slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at'),
    supabaseGet('starwars_cards?order=price_change_7d.desc&price_change_7d=gt.5&price_aud=gt.2&price_change_7d=lt.5000&image_url=not.is.null&limit=5&select=slug,name,image_url,price_aud,price_change_7d,set_name'),
    supabaseGet('starwars_cards?order=price_change_7d.asc&price_change_7d=lt.-5&price_aud=gt.2&image_url=not.is.null&limit=5&select=slug,name,image_url,price_aud,price_change_7d,set_name')
  ]);
  ...
  function carouselCard(c, suffix) {
    ...
    const ebay  = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name+' star wars unlimited card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<div class="carousel-card"><a href="/cards/starwars/${esc(c.slug)}" style="display:block;text-decoration:none"><div class="carousel-img-wrap"><img src="${esc(c.image_url)}${suffix}" alt="${esc(c.name).replace(/"/g,'&quot;')}" loading="eager" onerror="this.onerror=null;this.src='/card-placeholder.svg';this.style.opacity='1'"></div><div class="carousel-name">${esc(c.name)}</div>${c.rarity?`<div class="carousel-rarity">${esc(c.rarity)}</div>`:''}<div class="carousel-price">${price}</div></a><div class="carousel-buy-row"><a href="${ebay}" target="_blank" rel="noopener" onclick="gtag('event','ebay_click',{'event_category':'affiliate','event_label':'starwars-hub'})" class="carousel-buy-btn">Buy eBay &#8599;</a></div></div>`;
  }
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/starwars/sets/${encodeURIComponent(s.slug||s.id)}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}"><span class="set-tile-name">${esc(name)}${newBadge}</span><span class="set-tile-meta">${year}${s.card_count?' &middot; '+s.card_count+' cards':''}</span></a>`;
...
<head>
  ...
  ${hubSchemaHtml({ game: 'starwars', label: "Star Wars Unlimited", name: "Star Wars Card Prices Australia", description: `Browse ${sets.length||'10'}+ Star Wars: Unlimited sets. Live AUD card prices, eBay AU buy links. Australia's most complete Star Wars Unlimited price guide, updated daily.` })}
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="Star Wars Card Prices Australia | Cards on Cards on Cards">
  <meta property="og:description" content="${sets.length||'10'}+ Star Wars sets. Live AUD prices and eBay AU buy links updated daily.">
...
<div class="hero fade-up">
  ...
  <h1>Star Wars Unlimited Prices <span>in Australia</span></h1>
  <p class="hero-sub">Star Wars: Unlimited TCG card prices in AUD. Browse by set or search by card name. eBay AU buy links updated daily.</p>
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length||'10'}</div><div class="stat-label">Sets</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=star+wars+unlimited+cards&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" onclick="gtag('event','ebay_click',{'event_category':'affiliate','event_label':'starwars-hub'})" class="quick-link" style="background:linear-gradient(135deg,#1a1a1a,${ACCENT});color:#000">&#128722; Shop Star Wars Unlimited on eBay &#8599;</a>
  ...
  <a href="/blog/best-star-wars-unlimited-booster-boxes-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128230; Best Boxes &#8594;</a>
  <a href="/blog/is-star-wars-unlimited-worth-it-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128214; Is It Worth It? &#8594;</a>
...
${hasMovers ? `<div style="margin-bottom:28px">
  ...
  <p style="color:var(--text2);font-size:13px;margin-bottom:8px">Biggest price movers across all Star Wars: Unlimited cards in the last 7 days.</p>
  ...
  <div class="section fade-up fade-up-3">
    <div style="text-align:center;margin-bottom:16px">
      <h2 style="font-family:'Cinzel',serif;font-size:24px;font-weight:700;color:var(--text);margin-bottom:6px">Browse Every Star Wars Set</h2>
      <p style="font-size:13px;color:var(--text2)">${sets.length||'10'}+ sets. Pick a letter or search to explore.</p>
    </div>
  ...
  <div style="margin-bottom:28px">
    <h2 style="font-size:18px;margin-bottom:16px">Star Wars Guides and Blogs</h2>
  ...
  <div style="background:rgba(${ACCENT_RGB},.04);border:1px solid rgba(${ACCENT_RGB},.15);border-radius:var(--radius);padding:22px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px">
    <div>
      <div style="font-size:16px;font-weight:700;color:var(--text);margin-bottom:5px">Track Your Star Wars Unlimited Collection</div>
