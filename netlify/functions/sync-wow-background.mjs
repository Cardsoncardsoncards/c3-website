...
// Group B sync (Tue/Thu/Sat) - schedule: 15 23 * * 2,4,6 UTC
// Fetches all wow sets + cards + prices from tcgapi.dev Pro
...
export default async (req) => {
  console.log('[sync-wow] Starting (background function)...');
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-wow] Bulk price error set ${set.id}:`, e.message);
        }
      }
