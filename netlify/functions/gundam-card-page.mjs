...
// netlify/functions/gundam-card-page.mjs
// Serves /cards/gundam/:slug
...
async function getEbayListings(cardName, token) {
  if (!token) return [];
  try {
    const q = encodeURIComponent(`${cardName} gundam card game card`);
...
function notFoundPage(slug) {
  return `<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><title>Card Not Found | Cards on Cards on Cards</title><link rel="icon" type="image/png" href="/c3logo.png"></head><body style="background:#0A0C14;color:#F0F2FF;font-family:sans-serif"><style>${NAV_CSS}</style>${navHtml({ gameLabel: 'Gundam', gameHref: '/cards/gundam' })}<div style="padding:80px 24px;text-align:center"><h1 style="font-family:'Cinzel',serif;color:#C9A84C;margin-bottom:16px">Card Not Found</h1><p style="color:#A0A8C0;margin-bottom:24px">The card "${slug}" doesn't exist or hasn't synced yet.</p><a href="/cards/gundam" style="display:inline-block;padding:10px 24px;background:rgba(201,168,76,.15);border:1px solid rgba(201,168,76,.4);color:#E8C97A;border-radius:8px;text-decoration:none">Browse Gundam Card Game Cards</a></div></body></html>`;
}

function setNotFoundPage(setSlug) {
  return `<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><title>Set Not Found | Gundam Card Game | Cards on Cards on Cards</title><meta name="robots" content="noindex"><link rel="icon" type="image/png" href="/c3logo.png"><link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@700&family=DM+Sans:wght@400;600&display=swap" rel="stylesheet"></head><body style="background:#0A0C14;color:#F0F2FF;font-family:'DM Sans',sans-serif"><style>${NAV_CSS}</style>${navHtml({ gameLabel: 'Gundam', gameHref: '/cards/gundam' })}<div style="display:flex;align-items:center;justify-content:center;min-height:60vh;padding:24px;text-align:center"><div><div style="font-size:48px;margin-bottom:16px">🃏</div><h1 style="font-family:'Cinzel',serif;color:#37474F;font-size:22px;margin-bottom:10px">Set Not Found</h1><p style="color:#8892b0;font-size:14px;margin-bottom:24px">We couldn't find the Gundam Card Game set "${setSlug}".</p><a href="/cards/gundam" style="display:inline-block;background:#37474F;color:#000;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px">Browse All Gundam</a><a href="/" style="display:inline-block;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);color:#F0F2FF;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px">← Home</a></div></div></body></html>`;
}
...
async function handleSetPage(setSlug, htmlHeaders) {
  ...
  const [_psr0, _psr1] = await Promise.allSettled([
    supabaseGet(`gundam_sets?slug=eq.${encodeURIComponent(setSlug)}&limit=1&select=*`),
    getEbayToken()
  ]);
  ...
  const [_psr2, _psr3] = await Promise.allSettled([
    supabaseGet(`gundam_cards?set_id=eq.${set.id}&order=market_price.desc.nullslast&limit=400&select=slug,name,number,image_url,market_price,price_aud,rarity,set_name`),
    ebayToken ? getEbayListings(set.name + ' gundam card game', ebayToken).catch(() => []) : []
  ]);
  ...
  const ebaySetURL = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name + ' gundam card game')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
  const metaDesc = `Browse ${cards?.length || 0} Gundam Card Game cards from ${set.name}. View card prices in AUD and buy on eBay AU. Updated daily.`;

  const top5HTML = top5.map(c => {
    const aud = toAud(c);
    return `<a href="/cards/gundam/${c.slug}" style="flex:0 0 140px;background:#0e1118;border:1px solid rgba(16,185,129,.35);border-radius:10px;padding:10px;text-align:center;text-decoration:none;display:block">
  ...
  const allCardsHTML = (cards && cards.length) ? cards.filter(isSingles).map(c => {
    const aud = toAud(c);
    return `<a href="/cards/gundam/${c.slug}" style="background:#0e1118;border:1px solid #1e2235;border-radius:8px;padding:8px;text-decoration:none;text-align:center;display:block">
...
<head>
  ...
  <title>${set.name} | Gundam Card Game Set | Cards on Cards on Cards</title>
  <meta name="description" content="${metaDesc}">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/gundam/sets/${setSlug}">
  <meta property="og:title" content="${set.name} | Gundam Card Game | C3">
...
<body>
<style>${NAV_CSS}</style>${navHtml({ gameLabel: 'Gundam', gameHref: '/cards/gundam' })}
<div class="wrap">
  <div class="hero">
    <div class="hero-eyebrow">Gundam Card Game · Set</div>
    <h1 class="hero-title">${set.name}</h1>
    <div class="hero-meta">
      <span class="meta-badge">Gundam Card Game</span>
  ...
  <div class="cta-row">
    ...
    <a href="https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name + ' gundam card game booster box')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="cta-btn cta-secondary">Find Booster Box →</a>
    <a href="https://www.amazon.com.au/s?k=${encodeURIComponent(set.name + ' Gundam Card Game')}&tag=${AMAZON_TAG}" target="_blank" rel="noopener" class="cta-btn cta-secondary" style="border-color:rgba(255,153,0,.35);color:#ff9900">Amazon AU →</a>
  </div>
  ...
  <div style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:20px;font-size:13px;color:#8892b0">
    <strong style="color:#F0F2FF">About this set:</strong> Gundam Card Game card prices shown in AUD, converted from USD market data. Prices update daily via tcgapi.dev.
    <div style="margin-top:10px"><a href="/cards/gundam" style="color:${accent}">← Back to all Gundam Card Game cards</a></div>
  </div>
</div>
...
export default async (req) => {
  ...
  const slug = url.pathname.replace('/cards/gundam/', '').replace(/^\/|\/$/g, '');
  ...
  try {
    const cards = await supabaseGet(`gundam_cards?slug=eq.${encodeURIComponent(slug)}&limit=1&select=*`);
    ...
    const snapshots = await supabaseGet(`gundam_price_snapshots?card_id=eq.${encodeURIComponent(card.id)}&snapshot_date=gte.${new Date(Date.now()-90*86400000).toISOString().slice(0,10)}&order=snapshot_date.asc&limit=400&select=snapshot_date,price_aud,market_price`).catch(() => []);
    const priceChartHTML = priceChartHtml(snapshots);
    const _setRows = card.set_id ? await supabaseGet(`gundam_sets?id=eq.${card.set_id}&limit=1&select=slug`).catch(() => []) : [];
    const setUrl = (Array.isArray(_setRows) && _setRows[0] && _setRows[0].slug) ? `/cards/gundam/sets/${_setRows[0].slug}` : `/cards/gundam`;

    const priceAud = parseFloat(card.price_aud) || (card.market_price ? card.market_price * audRate : null);
    const pageUrl = encodeURIComponent(`https://cardsoncardsoncards.com.au/cards/gundam/${card.slug}`);
    const shareText = encodeURIComponent(`${card.name} Gundam Card Game Card Game -- ${priceAud ? 'AU$'+priceAud.toFixed(2) : 'check price'} on Cards on Cards on Cards`);
    const ebaySearchUrl = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(card.name+' gundam card game card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;

    const [_psr0, _psr1] = await Promise.allSettled([
      supabaseGet(`gundam_cards?set_id=eq.${card.set_id}&slug=neq.${encodeURIComponent(slug)}&market_price=gt.0&image_url=not.is.null&limit=12&order=market_price.desc&select=*`).catch(() => []),
  ...
  const ebayToken = _psr1.status === 'fulfilled' ? _psr1.value : [];
    ...
    const breadcrumb = { "@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[
      ...
      {"@type":"ListItem","position":2,"name":"Gundam Card Game Cards","item":"https://cardsoncardsoncards.com.au/cards/gundam"},
      {"@type":"ListItem","position":3,"name":card.name,"item":`https://cardsoncardsoncards.com.au/cards/gundam/${card.slug}`}
    ]};

    const productSchema = priceAud ? { "@context":"https://schema.org","@type":"Product","name":card.name,"image":card.image_url||'',
      "offers":{"@type":"Offer","priceCurrency":"AUD","price":priceAud.toFixed(2),"availability":"https://schema.org/InStock","url":`https://cardsoncardsoncards.com.au/cards/gundam/${card.slug}`}
    } : null;
    ...
    <div style="max-width:1100px;margin:0 auto 32px;padding:0 24px">
      ...
      <div style="display:flex;gap:12px;overflow-x:auto;padding-bottom:8px;scrollbar-width:none">
        ${relatedCards.map(c => {
          ...
          return `<a href="/cards/gundam/${c.slug}" style="flex:0 0 130px;background:#161929;border:1px solid #252840;border-radius:8px;padding:8px;text-decoration:none">
...
<head>
  ...
  <title>${card.name} Price Australia | Gundam Card Game | Cards on Cards on Cards</title>
  <meta name="description" content="${card.name} Gundam Card Game card${priceAud ? ` -- ~AU$${priceAud.toFixed(2)}` : ''}. ${card.rarity ? `${card.rarity}. ` : ''}View price and buy on eBay AU.">
  <meta property="og:site_name" content="Cards on Cards on Cards">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/gundam/${card.slug}">
  ...
  <meta property="og:title" content="${card.name} | Gundam Card Game Price AU">
...
<div class="card-hero">
  ...
  <div class="card-details">
    <div style="font-size:12px;color:var(--text2);margin-bottom:8px">
      <a href="/cards/gundam" style="color:var(--text2);text-decoration:none">Gundam Card Game</a>
    ...
    ${followBlockHtml({ game: 'gundam', slug: card.slug, cardName: card.name, printingId: card.tcgplayer_id })}
...
${(() => {
  ...
  const PREFERRED = ['Level', 'Cost', 'CardType', 'Color', 'Trait', 'Attack Points', 'Hit Points'];
...
function addToCompare(slug,name,img,price,game){
  ...
  tray.push({slug,name,img,price,game:'gundam'});saveCompareTray(tray);renderCompareTray();
}
function removeFromCompare(slug){saveCompareTray(getCompareTray().filter(c=>c.slug!==slug));renderCompareTray();}
function goToCompare(){const tray=getCompareTray();if(!tray.length)return;window.location.href='/compare?cards='+tray.map(c=>(c.game||'gundam')+':'+c.slug).join(',');}
renderCompareTray();
...
${viewTrackingScript('gundam', card.slug)}
</body>
</html>`;

    return new Response(html, { status: 200, headers });
  } catch (err) {
    console.error('[gundam-card-page]', err.message);
