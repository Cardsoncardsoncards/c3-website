...
// Gundam Card Game individual card pages with AUD pricing and affiliate links
// Built: 20 May 2026
...
function graceful404(slug) {
  ...
  const ebayUrl = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(cardName+' gundam card game')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
  return `<!DOCTYPE html>
<html lang="en-AU">
<head>
  <meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
  <title>${esc(cardName)} | Gundam Card Game | Cards on Cards on Cards</title>
  <meta name="robots" content="noindex">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <style>*{box-sizing:border-box;margin:0;padding:0}body{background:#0A0C14;color:#F0F2FF;font-family:'DM Sans',sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px;text-align:center}.wrap{max-width:440px}h1{color:#64748B;font-size:24px;margin-bottom:12px;font-family:'Cinzel',serif}p{color:#8892b0;font-size:14px;margin-bottom:24px;line-height:1.6}.btn{display:inline-block;background:#64748B;color:#000;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px}.btn-sec{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);color:#F0F2FF}</style>
</head>
<body><div class="wrap">
  <h1>${esc(cardName)}</h1>
  <p>We could not find that Gundam Card Game card. It may not be in our database yet.</p>
  <a href="${ebayUrl}" target="_blank" rel="noopener" class="btn">Search eBay AU &#8599;</a>
  <a href="/cards/gundam" class="btn btn-sec">Browse Gundam Card Game</a>
</div></body></html>`;
}
...
export default async (req) => {
  ...
  if (!slug || slug === '') {
    return Response.redirect('https://cardsoncardsoncards.com.au/cards/gundam', 302);
  }

  // Parallel: fetch card + set info
  const [cardResult] = await Promise.allSettled([
    supabaseGet(`gundam_cards?slug=eq.${encodeURIComponent(slug)}&limit=1&select=*`)
  ]);
  ...
  const [relatedResult, setResult] = await Promise.allSettled([
    card.set_id
      ? supabaseGet(`gundam_cards?set_id=eq.${card.set_id}&slug=neq.${encodeURIComponent(slug)}&order=market_price.desc.nullslast&limit=12&select=slug,name,image_url,market_price,rarity`)
      : Promise.resolve([]),
    card.set_id
      ? supabaseGet(`gundam_sets?id=eq.${card.set_id}&limit=1&select=id,name,slug,release_date`)
      : Promise.resolve([])
  ]);
  ...
  const amazonUrl    = `https://www.amazon.com.au/s?k=${encodeURIComponent((card.name||'')+' gundam card game')}&tag=${AMAZON_TAG}`;
  const setPageUrl   = set?.slug ? `/cards/gundam/sets/${esc(set.slug)}` : `/cards/gundam`;
  ...
  const relatedHTML = relatedCards.length ? relatedCards.map(c => {
    ...
    return `<a href="/cards/gundam/${c.slug}" style="background:#111420;border:1px solid #242840;border-radius:8px;padding:8px;text-align:center;display:block;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor='#64748B'" onmouseout="this.style.borderColor='#242840'">
      ...
      ${p > 0 ? `<div style="font-size:11px;color:#64748B;font-weight:700">AU$${p.toFixed(0)}</div>` : ''}
    </a>`;
  }).join('') : '';
...
<head>
  ...
  <title>${esc(card.name)} Price Australia | ${esc(card.set_name||'Gundam Card Game')} | Cards on Cards on Cards</title>
  <meta name="description" content="${esc(card.name)} from ${esc(card.set_name||'Gundam Card Game')}. ${priceDisplay} AUD. Buy on eBay AU. Australia's Gundam Card Game price guide.">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/gundam/${esc(slug)}">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="${esc(card.name)} | Gundam Card Game | Cards on Cards on Cards">
  ...
  <meta property="og:url" content="https://cardsoncardsoncards.com.au/cards/gundam/${esc(slug)}">
  <script type="application/ld+json">{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"Home","item":"https://cardsoncardsoncards.com.au"},{"@type":"ListItem","position":2,"name":"Gundam Card Game","item":"https://cardsoncardsoncards.com.au/cards/gundam"},{"@type":"ListItem","position":3,"name":"${(card.name||'').replace(/"/g,'&quot;')}","item":"https://cardsoncardsoncards.com.au/cards/gundam/${esc(slug)}"}]}</script>
  ...
  <style>
    :root{--bg:#0A0C14;--bg2:#111420;--bg3:#181d2e;--accent:#64748B;--accent-rgb:100,116,139;--gold:#C9A84C;--text:#e8eaf0;--text2:#9ba3c4;--border:#242840;--radius:12px}
...
<div class="wrap">
  <div class="breadcrumb">
    ...
    <a href="/cards/gundam">Gundam Card Game</a> &rsaquo;
    ...
    <div class="card-info">
      ...
      <div class="price-block">
        ...
        <div class="cta-row">
          ...
          <a href="/compare?cards=gundam:${esc(slug)}" class="btn btn-ghost">&#9878; Compare</a>
        </div>
      </div>

      <div class="section">
        <div class="section-title">Where to Buy ${esc(card.name)} in Australia</div>
        <p style="font-size:13px;color:var(--text2);line-height:1.6">${esc(card.name)} is a Gundam Card Game card from ${esc(card.set_name||'this set')}${card.rarity ? `, classified as ${esc(card.rarity)}` : ''}. The estimated price is ${priceDisplay} AUD, converted from US market data and updated daily. Buy directly on eBay AU for the best local pricing and fast shipping.</p>
      </div>
    </div>
  </div>
...
document.querySelectorAll('a[href*="ebay"]').forEach(a => a.addEventListener('click', () => {
  if (typeof gtag !== 'undefined') gtag('event','ebay_card_click',{card_name:'${(card.name||'').replace(/'/g,"\\'")}',game:'gundam'});
}));
</script>
