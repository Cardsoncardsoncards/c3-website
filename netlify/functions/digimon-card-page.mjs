...
export default async (req) => {
  ...
  const [relatedResult, setResult] = await Promise.allSettled([
    card.set_id
      ? supabaseGet(`digimon_cards?set_id=eq.${card.set_id}&slug=neq.${encodeURIComponent(slug)}&order=market_price.desc.nullslast&limit=12&select=slug,name,image_url,market_price,rarity`)
      : Promise.resolve([]),
    card.set_id
