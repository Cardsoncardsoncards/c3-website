...
<head>
  ...
  <title>Set Not Found | Pokemon TCG | Cards on Cards on Cards</title>
  ...
  <style>*{box-sizing:border-box;margin:0;padding:0}body{background:#0A0C14;color:#F0F2FF;font-family:'DM Sans',sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px;text-align:center}.wrap{max-width:420px}.icon{font-size:48px;margin-bottom:16px}h1{font-family:'Cinzel',serif;color:#FFCC00;font-size:22px;margin-bottom:10px}p{color:#8892b0;font-size:14px;margin-bottom:24px;line-height:1.6}.btn{display:inline-block;background:#FFCC00;color:#000;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px}.btn-sec{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);color:#F0F2FF}</style>
</head>
<body><div class="wrap"><div class="icon">🃏</div><h1>Set Not Found</h1><p>We couldn't find the Pokemon TCG set "${setSlug}". It may not be in our database yet.</p><a href="/cards/pokemon" class="btn">Browse All Pokemon Cards</a><a href="/" class="btn btn-sec">← Home</a></div></body>
</html>`;
}
...
export default async (req) => {
  ...
  try {
    const [sets, ebayToken] = await Promise.all([
      supabaseGet(`pokemon_sets?id=eq.${encodeURIComponent(setSlug)}&limit=1`),
      getEbayToken()
    ]);
    ...
    const [cards, ebayListings] = await Promise.all([
      supabaseGet(`pokemon_cards?set_id=eq.${set.id}&rarity=neq.None&order=market_price.desc.nullslast&limit=200&select=slug,name,number,image_url,market_price,price_aud,rarity,set_name,price_change_7d,price_change_30d`),
    ...
    function moverCard(c, isGainer) {
      ...
      const col = isGainer ? '#FFCC00' : '#F87171';
      return `<a href="/cards/pokemon/${c.slug}" style="background:#0e1118;border:1px solid #1e2235;border-radius:8px;padding:10px 12px;text-decoration:none;display:flex;align-items:center;gap:10px;transition:border-color .2s" onmouseover="this.style.borderColor='${col}'" onmouseout="this.style.borderColor='#1e2235'">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:13px;font-weight:700;margin-top:3px;color:#FFCC00">${aud > 0 ? `AU$${aud.toFixed(2)}` : ''}</div>
        </div>
    ...
    <div style="background:linear-gradient(135deg,rgba(255,204,0,.06),rgba(255,204,0,.02));border:1px solid rgba(255,204,0,.2);border-radius:12px;padding:20px;margin-bottom:28px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
        <div>
          <div style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#FFCC00;margin-bottom:8px">&#9650; Biggest Gainers</div>
      ...
      const aud = toAud(c);
      return `<a href="/cards/pokemon/${c.slug}" style="flex:0 0 140px;background:#0e1118;border:1px solid rgba(255,204,0,.35);border-radius:10px;padding:10px;text-align:center;text-decoration:none;transition:all .2s;display:block" onmouseover="this.style.borderColor='#FFCC00';this.style.transform='translateY(-2px)'" onmouseout="this.style.borderColor='rgba(255,204,0,.35)';this.style.transform='none'">
        ...
        ${c.rarity ? `<div style="font-size:10px;color:#FFCC00;margin-bottom:3px">${c.rarity}</div>` : ''}
    ...
    const allCardsHTML = cards && cards.length ? cards.map(c => {
      const aud = toAud(c);
      return `<a href="/cards/pokemon/${c.slug}" class="card-item" data-rarity="${(c.rarity||'none').toLowerCase().replace(/ /g,'-')}" data-price="${aud}" data-change7d="${c.price_change_7d||''}" data-name="${(c.name||'').toLowerCase().replace(/"/g,'')}" data-number="${c.number||''}" style="background:#0e1118;border:1px solid #1e2235;border-radius:8px;padding:8px;text-decoration:none;text-align:center;display:block;transition:all .2s" onmouseover="this.style.borderColor='#FFCC00'" onmouseout="this.style.borderColor='#1e2235'">
    ...
    const ebayListingsHTML = ebayListings.length ? ebayListings.slice(0,4).map(item => {
      ...
      return `<a href="${epnUrl}" target="_blank" rel="noopener" style="background:#0e1118;border:1px solid #1e2235;border-radius:8px;padding:12px;text-decoration:none;display:flex;gap:10px;align-items:center;transition:border-color .2s" onmouseover="this.style.borderColor='#FFCC00'" onmouseout="this.style.borderColor='#1e2235'">
    ...
    const metaDesc = `Browse ${cards?.length||0} Pokemon TCG cards from ${set.name}. View card prices in AUD, find the most valuable cards and buy on eBay AU. Updated daily.`;


    const setSchemaLD = {
      "@context": "https://schema.org", "@type": "CollectionPage",
      "name": `${set.name} Riftbound Card Prices Australia`,
      "description": `Browse all ${cards?.length||0} ${set.name} Pokemon TCG cards with AUD prices and eBay AU buy links.`,
      ...
      "breadcrumb": {
        ...
        "itemListElement": [
          ...
          { "@type": "ListItem", "position": 2, "name": "Pokemon TCG", "item": "https://cardsoncardsoncards.com.au/cards/pokemon" },
          { "@type": "ListItem", "position": 3, "name": set.name, "item": `https://cardsoncardsoncards.com.au/cards/pokemon/sets/${setSlug}` }
        ]
      }
    };
...
<head>
  ...
  <meta property="og:url" content="https://cardsoncardsoncards.com.au/cards/pokemon/sets/${setSlug}">
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .hero-eyebrow{font-size:11px;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:#FFCC00;margin-bottom:8px}
    ...
    .meta-badge{background:rgba(255,204,0,.08);border:1px solid rgba(255,204,0,.35);color:#FFCC00;padding:3px 10px;border-radius:100px;font-size:11px;font-weight:700}
    ...
    .cta-primary{background:#FFCC00;color:#000}
    ...
    .nav-link.active{border-color:rgba(255,204,0,.35);color:#FFCC00;background:rgba(255,204,0,.08)}
    ...
    .filt-btn:hover,.filt-btn.active{border-color:#FFCC00;color:#FFCC00;background:rgba(255,204,0,.08)}
  </style>
</head>
<body>
<nav>
  <div class="nav-inner">
    ...
    <div class="nav-links">
      ...
      <a href="/cards/pokemon" class="nav-link active">Pokemon TCG</a>
...
<div class="wrap">
  <div class="hero">
    ...
    <div class="hero-meta">
      <span class="meta-badge">Pokemon TCG</span>
  ...
  <div class="cta-row">
    <a href="${ebaySetURL}" target="_blank" rel="noopener" class="cta-btn cta-primary" onclick="if(typeof gtag!=='undefined')gtag('event','ebay_set_click',{set_name:'${set.name}',game:'pokemon'})">Buy Cards on eBay AU →</a>
    <a href="${ebayBoxURL}" target="_blank" rel="noopener" class="cta-btn cta-secondary">Find Booster Box →</a>
    <a href="https://www.amazon.com.au/s?k=${encodeURIComponent(set.name + ' Pokemon')}&tag=${AMAZON_TAG}" target="_blank" rel="noopener" class="cta-btn cta-secondary" style="border-color:rgba(255,153,0,.35);color:#ff9900">Search Amazon AU →</a>
  </div>
  ...
  ${ebayListingsHTML ? `<div class="section">
    ...
    <a href="${ebaySetURL}" target="_blank" rel="noopener" style="font-size:13px;color:#FFCC00;text-decoration:none">See all listings on eBay AU →</a>
  </div>` : ''}
  ...
  <div style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:20px;font-size:13px;color:#8892b0">
    <strong style="color:#F0F2FF">About this set:</strong> Pokemon TCG card prices shown in AUD, converted from USD market data at approximately 1.58x. Prices update daily via tcgapi.dev. Always check eBay AU for live Australian market pricing before buying or selling.
    <div style="margin-top:10px"><a href="/cards/pokemon" style="color:#FFCC00">← Back to all Pokemon TCG cards</a></div>
  </div>
</div>

<script>
if(typeof gtag!=='undefined'){
  document.querySelectorAll('a[href*="ebay"]').forEach(a=>a.addEventListener('click',()=>gtag('event','ebay_click',{game:'pokemon',set:'${set.name}'})));
}
...
document.addEventListener('DOMContentLoaded', applyFilters);
    ...
    console.error('[pokemon-set-page.mjs] Error:', err.message);
