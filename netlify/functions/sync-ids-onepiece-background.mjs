// netlify/functions/sync-ids-onepiece-background.mjs
// Resolves tcgplayer_id -> tcgapi.dev internal ID for onepiece cards only.
...
const GAME_CONFIG = { game: 'onepiece', table: 'onepiece_cards', priceCol: 'market_price' };
...
async function selfChain() {
  try {
    fetch(`${SITE_URL}/.netlify/functions/sync-ids-onepiece-background`, {
      method: 'POST',
...
async function logSyncEvent(eventType, rowsAffected = null) {
  ...
  try {
    ...
    clearTimeout(timer);
    if (!res.ok) console.warn(`[sync-ids-onepiece] sync_events log failed ${res.status}`);
  } catch (e) {
    clearTimeout(timer);
    console.warn(`[sync-ids-onepiece] sync_events log error: ${e.message}`);
  }
}
