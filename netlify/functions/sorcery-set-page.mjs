...
export default async (req) => {
  ...
  try {
    ...
    const [cardsR, sealedR, ebayListingsR] = await Promise.allSettled([
      supabaseGet(`sorcery_cards?set_id=eq.${set.id}&rarity=neq.None&order=name.asc&limit=200&select=slug,name,number,image_url,market_price,price_aud,rarity,set_name,price_change_7d,price_change_30d`),
