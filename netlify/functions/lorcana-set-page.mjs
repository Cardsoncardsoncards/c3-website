...
function graceful404(setSlug) {
  return `<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Set Not Found | Disney Lorcana | Cards on Cards on Cards</title><meta name="robots" content="noindex"><link rel="icon" type="image/png" href="/c3logo.png"><style>*{box-sizing:border-box;margin:0;padding:0}body{background:#0A0C14;color:#F0F2FF;font-family:sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px;text-align:center}.wrap{max-width:420px}.icon{font-size:48px;margin-bottom:16px}h1{color:#818CF8;font-size:22px;margin-bottom:10px}p{color:#8892b0;font-size:14px;margin-bottom:24px;line-height:1.6}.btn{display:inline-block;background:#818CF8;color:#000;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px}.btn-sec{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);color:#F0F2FF}</style></head><body><div class="wrap"><div class="icon">&#127183;</div><h1>Set Not Found</h1><p>We could not find the Disney Lorcana set "${esc(setSlug)}". It may not be in our database yet.</p><a href="/cards/lorcana" class="btn">Browse All Disney Lorcana Cards</a><a href="/" class="btn btn-sec">Back to Home</a></div></body></html>`;
}
...
export default async (req) => {
  ...
  try {
    const [sets, ebayToken] = await Promise.allSettled([
      supabaseGet(`lorcana_sets?slug=eq.${encodeURIComponent(setSlug)}&limit=1&select=id,name,slug,release_date,card_count`),
      getEbayToken()
    ]);
    ...
    const [cardsRes, ebayRes] = await Promise.allSettled([
      supabaseGet(`lorcana_cards?set_id=eq.${set.id}&order=name.asc&limit=400&select=slug,name,number,image_url,market_price,price_aud,rarity,set_name,price_change_7d,price_change_30d`),
      getEbayListings(`${set.name} disney lorcana card`, ebayTokenVal)
    ]);
    ...
    const ebaySetURL = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name + ' disney lorcana')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    ...
    const amazonURL  = `https://www.amazon.com.au/s?k=${encodeURIComponent(set.name + ' disney lorcana')}&tag=${AMAZON_TAG}`;

    const top5HTML = top5.length ? top5.map(c => {
      const aud = toAud(c);
      return `<a href="/cards/lorcana/${esc(c.slug)}" style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:12px;text-decoration:none;display:flex;flex-direction:column;align-items:center;gap:6px;transition:border-color .2s" onmouseover="this.style.borderColor='#818CF8'" onmouseout="this.style.borderColor='#1e2235'">
        ...
        <div style="font-size:13px;color:#818CF8;font-weight:700">AU$${aud.toFixed(2)}</div>
      </a>`;
    ...
    const allCardsHTML = cards.map(c => {
      ...
      return `<a class="card-item" href="/cards/lorcana/${esc(c.slug)}" data-name="${esc(c.name.toLowerCase())}" data-price="${aud.toFixed(2)}" data-rarity="${rar}" data-change7d="${c.price_change_7d||0}" data-number="${c.number||0}" style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:12px;text-decoration:none;display:flex;flex-direction:column;align-items:center;gap:6px;transition:border-color .2s" onmouseover="this.style.borderColor='#818CF8'" onmouseout="this.style.borderColor='#1e2235'">
        <img src="${esc(c.image_url)}" alt="${esc(c.name)}" style="width:80px;height:112px;object-fit:contain;border-radius:5px" loading="lazy">
        <div style="font-size:11px;color:#F0F2FF;text-align:center;font-weight:600;line-height:1.3">${esc(c.name)}</div>
        ${aud > 0 ? `<div style="font-size:12px;color:#818CF8;font-weight:700">AU$${aud.toFixed(2)}</div>` : '<div style="font-size:11px;color:#8892b0">No price</div>'}
      </a>`;
    }).join('');
...
<head>
  ...
  <title>${esc(set.name)} | Disney Lorcana Set | Cards on Cards on Cards</title>
  <meta name="description" content="Browse all ${esc(set.name)} Disney Lorcana cards with AUD prices. ${priced} cards priced on Cards on Cards on Cards.">
  <link rel="canonical" href="https://www.cardsoncardsoncards.com.au/cards/lorcana/sets/${esc(set.slug)}">
  <meta property="og:title" content="${esc(set.name)} | Disney Lorcana | C3">
  <meta property="og:description" content="${priced} cards priced in AUD for ${esc(set.name)}.">
  <meta property="og:url" content="https://www.cardsoncardsoncards.com.au/cards/lorcana/sets/${esc(set.slug)}">
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .filt-btn.active,.filt-btn:hover{background:#818CF8;border-color:#818CF8;color:#000;font-weight:700}
...
<div class="wrap">
  <div style="font-size:12px;color:#8892b0;margin-bottom:12px"><a href="/" style="color:#8892b0;text-decoration:none">Home</a> &rsaquo; <a href="/cards/lorcana" style="color:#8892b0;text-decoration:none">Disney Lorcana</a> &rsaquo; ${esc(set.name)}</div>

  <div style="margin-bottom:4px;font-size:11px;font-weight:700;letter-spacing:.08em;color:#818CF8;text-transform:uppercase">Disney Lorcana &middot; Set</div>
  ...
  <div style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:20px;font-size:13px;color:#8892b0;margin-top:28px">
    <strong style="color:#F0F2FF">About this set:</strong> Disney Lorcana card prices shown in AUD, converted from USD market data. Prices update daily via tcgapi.dev. Always check eBay AU for live Australian market pricing.
    <div style="margin-top:10px"><a href="/cards/lorcana" style="color:#818CF8">Back to all Disney Lorcana cards</a></div>
  </div>
</div>
...
if(typeof gtag!=='undefined'){
  document.querySelectorAll('a[href*="ebay"]').forEach(a=>a.addEventListener('click',()=>gtag('event','ebay_click',{game:'lorcana',set:'${esc(set.name)}'})));
}
</script>
</body>
</html>`;
    ...
    console.error('[lorcana-set-page] Error:', err.message);
