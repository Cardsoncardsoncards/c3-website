// netlify/functions/mtg-random-commander.mjs
//
// Task D. Split out of card-index.mjs, which was serving two unrelated things through one
// handler branching on url.pathname. Every other page type in this repo is one route per
// file (32 *-card-page.mjs, 31 *-set-page.mjs, 33 *-hub.mjs), so this follows the existing
// convention rather than inventing a split.
//
// ON CACHING, AND WHY THIS PAGE IS DELIBERATELY CACHED HARD.
// The task that commissioned this split expected the random picker to need an uncached
// response. It does not, and caching it is correct. This route returns a STATIC HTML SHELL:
// renderRandomCommander() takes no arguments, reads nothing, and picks nothing. The
// randomness happens in the browser, which queries Supabase directly and chooses with
// Math.floor(Math.random() * cards.length). There is also a separate server-side picker at
// /api/random-commander in card-api.mjs, and that one is correctly uncached: verified live,
// it returns Cache-Control: no-cache with Netlify Durable fwd=bypass, and three consecutive
// calls returned three different commanders.
//
// So setting no-store here, as the task literally instructed, would be a REGRESSION: it
// would force an origin render of an unchanging shell on every request and buy nothing. The
// 24 hour window is kept exactly as it was and only the durable tier and
// stale-while-revalidate are added, matching every other page fixed tonight.

import { NAV_HTML } from './shared/nav.mjs';
...
// the randomness actually happens. They lived at module level in card-index.mjs, so moving
// the function without them left two dangling references that node --check cannot see: it
// parses, and then throws ReferenceError the first time the route is hit. Caught by rendering
// the function locally before pushing, which is why that check is worth running.
//
// ANON key, correctly: this is public read data and the value ships to the browser by design,
// exactly as it did before the split. Nothing here needs the service key.
...
export default async (req) => {
  // C3L-107/C3L-118. First statement, same as every other page function.
