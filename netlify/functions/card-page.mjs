...
function renderHTML({ card, snapshots, relatedCards, sealedProducts, prevCard, nextCard, ebayListings, likeCount, setSlugResolved, otherPrintings, fxRate = FX_FALLBACK, signals = null }) {
  ...
  const setSlug = setSlugResolved || card.set_code;
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Price rows */
    .price-rows { display: flex; flex-direction: column; gap: 6px; margin-bottom: 4px; }
    .price-row { display: flex; align-items: center; gap: 10px; padding: 10px 14px; background: var(--bg3); border: 1px solid var(--border); border-radius: 8px; font-family: sans-serif; }
    .price-row--foil { border-color: rgba(124,106,245,.3); }
    .price-row--etched { border-color: rgba(74,158,255,.3); }
    .price-row-label { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: .08em; color: var(--text2); min-width: 60px; }
    .price-row--foil .price-row-label { color: var(--accent2); }
    .price-row--etched .price-row-label { color: var(--blue); }
    ...
    /* Foil toggle (keep for backwards compat) */
...
<div class="card-layout">
  ...
  <div class="card-info">
    ...
    <div class="price-block">
      ...
      <!-- All printings shown simultaneously -->
...
<div class="card-sections">
  ...
  <!-- Newsletter signup (coming soon - hidden)
  <div class="newsletter-inline">...</div>
  -->
...
function switchPrinting(thumb) {
  ...
  _currentThumbIdx = parseInt(thumb.dataset.idx);
  ...
  if (newImg && heroImg) {
    ...
    setTimeout(() => {
      ...
      else heroImg.onload = () => { heroImg.style.opacity = '1'; };
    }, 180);
  }

  // Update carousel info line
  const rarity = thumb.dataset.rarity ? thumb.dataset.rarity.charAt(0).toUpperCase() + thumb.dataset.rarity.slice(1) : '';
  const priceNF = thumb.dataset.priceNf || '';
  const priceFoil = thumb.dataset.priceFoil || '';
  const priceLine = priceNF ? ' · ' + priceNF + (priceFoil ? ' / Foil ' + priceFoil : '') : '';
  if (info) {
    info.style.opacity = '0';
    setTimeout(() => {
      info.innerHTML = '<strong>' + thumb.dataset.set + '</strong>#' + thumb.dataset.collector + ' · ' + rarity + priceLine;
  ...
  if (priceBlock && (priceNF || priceFoil)) {
    const nfRow = priceBlock.querySelector('.price-row--nf');
    const foilRow = priceBlock.querySelector('.price-row--foil');
    const usdNF = thumb.dataset.priceUsdNf;
    const usdFoil = thumb.dataset.priceUsdFoil;

    if (nfRow) {
      const audEl = nfRow.querySelector('.price-row-aud');
      const usdEl = nfRow.querySelector('.price-row-usd');
      if (audEl) audEl.textContent = priceNF || 'N/A';
      if (usdEl && usdNF) usdEl.innerHTML = '$' + usdNF + '<span class="price-row-usd-label">USD</span>';
    }
    if (foilRow && priceFoil) {
      foilRow.style.display = '';
      const audEl = foilRow.querySelector('.price-row-aud');
      const usdEl = foilRow.querySelector('.price-row-usd');
      if (audEl) audEl.textContent = priceFoil;
      if (usdEl && usdFoil) usdEl.innerHTML = '$' + usdFoil + '<span class="price-row-usd-label">USD</span>';
    } else if (foilRow && !priceFoil) {
      foilRow.style.display = 'none';
    }
    // Flash the price block to draw attention to the update
    priceBlock.style.transition = 'box-shadow .18s';
    priceBlock.style.boxShadow = '0 0 0 2px var(--accent)';
    setTimeout(() => { priceBlock.style.boxShadow = ''; }, 600);
  }
...
function scrollPrintings(direction) {
  const track = document.getElementById('printings-track');
  const thumbs = track ? Array.from(track.querySelectorAll('.printing-thumb')) : [];
  if (!thumbs.length) return;
  const next = Math.max(0, Math.min(thumbs.length - 1, _currentThumbIdx + direction));
  if (next !== _currentThumbIdx) switchPrinting(thumbs[next]);
}

// Per-row USD toggle
function toggleRowCurrency(btn, usd, aud) {
  const row = btn.closest('.price-row');
  const audEl = row.querySelector('.price-row-aud');
...
async function loadCKPrices() {
  ...
  try {
    ...
    if (block && (data.retail || data.buylist)) block.style.display = '';
...
function goToCompare() {
  ...
  // Build canonical game:ref tokens (ref is the scryfall_id for MTG). Guard against
  // any item missing a ref (e.g. stale cache) so we never navigate to a bare/broken
  // /compare?cards= URL. The compare page resolves scryfall_id or slug refs alike.
  const tokens = tray
    .map(c => c && c.slug ? (c.game || 'mtg') + ':' + c.slug : null)
    .filter(Boolean);
...
<footer>
  <div style="text-align:center;margin:16px 0"><a href="https://buy.stripe.com/3cIdR836CeXk95C475aIM02" target="_blank" rel="noopener" style="background:#C9A84C;color:#0A0C14;padding:9px 20px;border-radius:20px;font-weight:700;text-decoration:none;font-size:13px;display:inline-block">&#10084;&#65039; Support C3</a></div>
  <p><a href="/">Home</a><a href="/blog">Blog</a><a href="/calendar">Calendar</a><a href="/account">Account</a><a href="/tracker">Tracker</a><a href="/cards">Card Prices</a><a href="/compare">Compare</a><a href="/market">Market</a><a href="/tools">Tools</a><a href="/contact">Contact</a><a href="/legal">Legal</a><a href="https://www.ebay.com.au/str/cardsoncardsoncards?mkcid=1&amp;mkrid=705-53470-19255-0&amp;campid=5339146789&amp;customid=Footer&amp;toolid=10001&amp;mkevt=1" target="_blank" rel="noopener" data-c3-ev="ebay_click" data-c3-cat="affiliate" data-c3-label="footer">eBay</a><a href="https://blasdigital.etsy.com" target="_blank" rel="noopener">D&amp;D Tools on Etsy &#8599;</a><br><a href="/cards/mtg">MTG</a></p>
