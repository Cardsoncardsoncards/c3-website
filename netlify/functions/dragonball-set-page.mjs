...
function graceful404(setSlug) {
  return `<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Set Not Found | Dragon Ball Super | Cards on Cards on Cards</title><meta name="robots" content="noindex"><link rel="icon" type="image/png" href="/c3logo.png"><style>*{box-sizing:border-box;margin:0;padding:0}body{background:#0A0C14;color:#F0F2FF;font-family:sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px;text-align:center}.wrap{max-width:420px}.icon{font-size:48px;margin-bottom:16px}h1{color:#EAB308;font-size:22px;margin-bottom:10px}p{color:#8892b0;font-size:14px;margin-bottom:24px;line-height:1.6}.btn{display:inline-block;background:#EAB308;color:#000;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px}.btn-sec{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);color:#F0F2FF}</style></head><body><div class="wrap"><div class="icon">&#127183;</div><h1>Set Not Found</h1><p>We could not find the Dragon Ball Super set "${esc(setSlug)}". It may not be in our database yet.</p><a href="/cards/dragonball" class="btn">Browse All Dragon Ball Super Cards</a><a href="/" class="btn btn-sec">Back to Home</a></div></body></html>`;
}
...
export default async (req) => {
  ...
  try {
    const [sets, ebayToken] = await Promise.allSettled([
      supabaseGet(`dragonball_sets?slug=eq.${encodeURIComponent(setSlug)}&limit=1&select=id,name,slug,release_date,card_count`),
      getEbayToken()
    ]);
    ...
    const [cardsRes, ebayRes] = await Promise.allSettled([
      supabaseGet(`dragonball_cards?set_id=eq.${set.id}&order=name.asc&limit=400&select=slug,name,number,image_url,market_price,price_aud,rarity,set_name,price_change_7d,price_change_30d`),
      getEbayListings(`${set.name} dragon ball super card`, ebayTokenVal)
    ]);
    ...
    const amazonURL  = `https://www.amazon.com.au/s?k=${encodeURIComponent(set.name + ' dragon ball super')}&tag=${AMAZON_TAG}`;

    const top5HTML = top5.length ? top5.map(c => {
      const aud = toAud(c);
      return `<a href="/cards/dragonball/${esc(c.slug)}" style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:12px;text-decoration:none;display:flex;flex-direction:column;align-items:center;gap:6px;transition:border-color .2s" onmouseover="this.style.borderColor='#EAB308'" onmouseout="this.style.borderColor='#1e2235'">
        ...
        <div style="font-size:13px;color:#EAB308;font-weight:700">AU$${aud.toFixed(2)}</div>
      </a>`;
    ...
    const allCardsHTML = cards.map(c => {
      ...
      return `<a class="card-item" href="/cards/dragonball/${esc(c.slug)}" data-name="${esc(c.name.toLowerCase())}" data-price="${aud.toFixed(2)}" data-rarity="${rar}" data-change7d="${c.price_change_7d||0}" data-number="${c.number||0}" style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:12px;text-decoration:none;display:flex;flex-direction:column;align-items:center;gap:6px;transition:border-color .2s" onmouseover="this.style.borderColor='#EAB308'" onmouseout="this.style.borderColor='#1e2235'">
        <img src="${esc(c.image_url)}" alt="${esc(c.name)}" style="width:80px;height:112px;object-fit:contain;border-radius:5px" loading="lazy">
        <div style="font-size:11px;color:#F0F2FF;text-align:center;font-weight:600;line-height:1.3">${esc(c.name)}</div>
        ${aud > 0 ? `<div style="font-size:12px;color:#EAB308;font-weight:700">AU$${aud.toFixed(2)}</div>` : '<div style="font-size:11px;color:#8892b0">No price</div>'}
      </a>`;
    }).join('');
...
<head>
  ...
  <title>${esc(set.name)} | Dragon Ball Super Set | Cards on Cards on Cards</title>
  <meta name="description" content="Browse all ${esc(set.name)} Dragon Ball Super cards with AUD prices. ${priced} cards priced on Cards on Cards on Cards.">
  <link rel="canonical" href="https://www.cardsoncardsoncards.com.au/cards/dragonball/sets/${esc(set.slug)}">
  <meta property="og:title" content="${esc(set.name)} | Dragon Ball Super | C3">
  <meta property="og:description" content="${priced} cards priced in AUD for ${esc(set.name)}.">
  <meta property="og:url" content="https://www.cardsoncardsoncards.com.au/cards/dragonball/sets/${esc(set.slug)}">
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .filt-btn.active,.filt-btn:hover{background:#EAB308;border-color:#EAB308;color:#000;font-weight:700}
...
<div class="wrap">
  <div style="font-size:12px;color:#8892b0;margin-bottom:12px"><a href="/" style="color:#8892b0;text-decoration:none">Home</a> &rsaquo; <a href="/cards/dragonball" style="color:#8892b0;text-decoration:none">Dragon Ball Super</a> &rsaquo; ${esc(set.name)}</div>

  <div style="margin-bottom:4px;font-size:11px;font-weight:700;letter-spacing:.08em;color:#EAB308;text-transform:uppercase">Dragon Ball Super &middot; Set</div>
  ...
  <div style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:20px;font-size:13px;color:#8892b0;margin-top:28px">
    <strong style="color:#F0F2FF">About this set:</strong> Dragon Ball Super card prices shown in AUD, converted from USD market data. Prices update daily via tcgapi.dev. Always check eBay AU for live Australian market pricing.
...
if(typeof gtag!=='undefined'){
  document.querySelectorAll('a[href*="ebay"]').forEach(a=>a.addEventListener('click',()=>gtag('event','ebay_click',{game:'dragonball',set:'${esc(set.name)}'})));
}
</script>
</body>
</html>`;
    ...
    console.error('[dragonball-set-page] Error:', err.message);
