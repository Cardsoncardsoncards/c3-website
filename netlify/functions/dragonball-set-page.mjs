...
// Serves /cards/dragonball/sets/:setCode
...
export default async (req) => {
  ...
  const setCode = url.pathname.replace(/^\/cards\/dragonball\/sets\//, '').replace(/\/$/, '');
  if (!setCode) return new Response('Not found', { status: 404, headers });

  const [setsById, setsbySlug, ebayToken] = await Promise.all([
    supabaseGet(`dragonball_sets?id=eq.${encodeURIComponent(setCode)}&limit=1`),
    supabaseGet(`dragonball_sets?slug=eq.${encodeURIComponent(setCode)}&limit=1`),
    getEbayToken()
  ]);

  const set = (setsById.length ? setsById : setsbySlug)[0];
  if (!set) return new Response(`<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><title>Set Not Found | C3</title><link rel="icon" type="image/png" href="/c3logo.png"></head><body style="background:#0A0C14;color:#F0F2FF;font-family:sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;flex-direction:column;gap:16px"><h1>Set not found</h1><a href="/cards/dragonball" style="color:#F97316">← Back to Dragon Ball Super</a></body></html>`, { status: 404, headers });

  const cards = await supabaseGet(`dragonball_cards?set_id=eq.${encodeURIComponent(set.id)}&order=market_price.desc.nullslast&limit=500&select=slug,name,number,image_url,market_price,price_aud,rarity,set_name,custom_attributes`);

  const ebaySearchURL = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name+' dragon ball super card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
  const ebayBoxURL   = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name+' dragon ball super card booster box')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
  const ebayListings = await getEbayListings(`${set.name} dragon ball super card`, ebayToken);
  ...
  const topCardsHTML = top5.map(c => `
    <a href="/cards/dragonball/${c.slug}" style="flex:0 0 140px;background:#111420;border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:10px;text-align:center;text-decoration:none;transition:all .2s;display:block" onmouseover="this.style.borderColor='#F97316';this.style.transform='translateY(-2px)'" onmouseout="this.style.borderColor='rgba(255,255,255,.1)';this.style.transform='none'">
      ...
      <div style="font-size:11px;color:#F97316;font-weight:700;margin-top:3px">AU$${toAud(c).toFixed(2)}</div>
    </a>`).join('');

  const allCardsHTML = cards.map(c => `
    <a href="/cards/dragonball/${c.slug}" class="card-item" data-rarity="${(c.rarity||'').toLowerCase()}" data-price="${toAud(c).toFixed(2)}">
      ...
      ${toAud(c) > 0 ? `<div style="font-size:10px;color:#F97316;font-weight:700;margin-top:2px">AU$${toAud(c).toFixed(2)}</div>` : ''}
    </a>`).join('');

  const ebayItemsHTML = ebayListings.slice(0,6).map(item => `
    <a href="${item.itemWebUrl||'#'}?mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener sponsored" style="background:#111420;border:1px solid #252840;border-radius:10px;overflow:hidden;text-decoration:none;transition:border-color .2s;display:block" onmouseover="this.style.borderColor='#F97316'" onmouseout="this.style.borderColor='#252840'">
      ...
      <div style="padding:10px">
        ...
        <div style="font-size:13px;color:#F97316;font-weight:700">${item.price?.value ? 'AU$'+parseFloat(item.price.value).toFixed(2) : ''}</div>
      </div>
    </a>`).join('');
...
<head>
  ...
  <title>${set.name} Card Prices Australia | Dragon Ball Super | Cards on Cards on Cards</title>
  <meta name="description" content="Browse all ${cards.length} cards in ${set.name}. Live AUD prices, card images, and eBay AU buy links. Australia's Dragon Ball Super price guide.">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/dragonball/sets/${encodeURIComponent(set.slug||set.id)}">
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    body::before{content:'';position:fixed;inset:0;pointer-events:none;background:radial-gradient(ellipse 80% 40% at 50% -10%,#F973160f,transparent 60%);z-index:0}
    ...
    .stat-num{font-family:'Cinzel',serif;font-size:18px;font-weight:700;color:#F97316}
    ...
    .filter-btn.active,.filter-btn:hover{border-color:#F97316;color:#F97316;background:rgba(255,255,255,.04)}
    ...
    .card-item:hover{border-color:#F97316;transform:translateY(-2px)}
...
<div class="wrap" style="padding-top:40px;padding-bottom:60px">
  <div class="breadcrumb"><a href="/">Home</a> → <a href="/cards/dragonball">Dragon Ball Super</a> → ${set.name}</div>
  ...
  <div class="cta-bar">
    <a href="${ebayBoxURL}" target="_blank" rel="noopener" class="cta-btn" style="background:linear-gradient(135deg,#F97316,#F97316aa);color:#0A0C14">🛒 Buy ${set.name} Booster Box on eBay ↗</a>
    ...
    <a href="/quizzes/dragonball-character" class="cta-btn" style="background:#111420;border:1px solid #252840;color:#F97316">🐉 Which DBS Character Are You?</a>
  </div>
...
<footer>
  <p><a href="/">Home</a><a href="/cards/dragonball">Dragon Ball Super</a><a href="/shop.html">Shop</a><a href="/blog">Blog</a><a href="/ev-calculator.html">EV Calc</a><a href="/tracker.html">Tracker</a></p>
...
export const config = { path: '/cards/dragonball/sets/:setCode+' };
