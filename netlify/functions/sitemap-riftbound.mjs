// netlify/functions/sitemap-riftbound.mjs
// Generates XML sitemap for Riftbound card pages at /cards/riftbound/[slug]
// Registered in sitemap-index.xml as /api/sitemap-riftbound
// Lower threshold (0.50) — new game, small catalogue, capture everything priced
// Returns valid empty sitemap until riftbound_cards table is created and synced
...
export default async (req) => {
  ...
  try {
    const countRes = await supabaseFetch(
      `${SUPABASE_URL}/rest/v1/riftbound_cards?select=id&market_price=gte.${PRICE_THRESHOLD}&slug=not.is.null&limit=1`,
      { 'Prefer': 'count=exact' }
    );
    if (!countRes.ok) return empty('riftbound sitemap: count query failed');
    const contentRange = countRes.headers.get('content-range');
    const totalCount = (contentRange && contentRange.includes('/'))
      ? parseInt(contentRange.split('/')[1], 10) : 0;

    if (totalCount === 0) return empty('riftbound card pages: pending sync or no priced cards');
...
${urls}
    ...
    console.error('[sitemap-riftbound] error:', err.message);
    return new Response(
...
export const config = { path: '/api/sitemap-riftbound' };
