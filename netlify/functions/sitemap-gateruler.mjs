// netlify/functions/sitemap-gateruler.mjs
// Generates XML sitemap for gateruler card pages at /cards/gateruler/[slug]
// Registered in sitemap-index.xml as /api/sitemap-gateruler
...
async function fetchSlugs(offset) {
  const url = SUPABASE_URL + '/rest/v1/gateruler_cards'
...
export default async (req) => {
  ...
  try {
    const countRes = await supabaseFetch(
      SUPABASE_URL + '/rest/v1/gateruler_cards?select=id&market_price=gte.' + PRICE_THRESHOLD + '&slug=not.is.null&limit=1',
      { Prefer: 'count=exact' }
    );
    if (!countRes.ok) return empty('gateruler sitemap: count query failed');

    const contentRange = countRes.headers.get('content-range');
    const totalCount = (contentRange && contentRange.includes('/'))
      ? parseInt(contentRange.split('/')[1], 10) : 0;

    if (totalCount === 0) return empty('gateruler card pages: pending sync or no priced cards');
    ...
    const urlItems = allCards
      ...
      .map(function(c) {
        ...
        return '  <url>\n    <loc>' + SITE_URL + '/cards/gateruler/' + c.slug + '</loc>\n    <lastmod>' + lastmod + '</lastmod>\n    <changefreq>daily</changefreq>\n    <priority>' + priority + '</priority>\n  </url>';
      })
      .join('\n');

    const xml = '<?xml version="1.0" encoding="UTF-8"?>\n'
      + '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'
      + '  <!-- gateruler card pages: ' + allCards.length + ' cards with price >= AUD$' + PRICE_THRESHOLD + ' -->\n'
    ...
    console.error('[sitemap-gateruler] error:', err.message);
    return new Response(
...
export const config = { path: '/api/sitemap-gateruler' };
