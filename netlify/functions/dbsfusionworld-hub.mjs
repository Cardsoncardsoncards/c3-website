...
const EMOJI       = '&#128007;';
const SEARCH_PH   = "Goku, Vegeta, Gohan, Broly...";
const SET_PH      = "FB-01, FB-02, Blazing Aura...";
...
export default async (req) => {
  ...
  const [setsResult, topCardsResult] = await Promise.allSettled([
    supabaseGet('dbsfusionworld_sets?order=release_date.desc&limit=100&select=id,name,slug,abbreviation,release_date,card_count'),
    supabaseGet('dbsfusionworld_cards?order=market_price.desc&market_price=gt.0.5&market_price=lt.200&image_url=not.is.null&limit=24&select=slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at')
  ]);
  ...
  const carouselHTML = topCards.map(c => {
    ...
    const ebay  = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name+' dragon ball fusion world card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<a href="/cards/dbsfusionworld/${c.slug}" class="carousel-card">
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/dbsfusionworld/sets/${encodeURIComponent(s.slug||s.id)}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}">
...
<head>
  ...
  <title>DBS Fusion World Card Prices Australia | AUD Prices Updated Daily | C3</title>
  <meta name="description" content="Browse ${sets.length||'48'}+ DBS Fusion World sets. Live AUD card prices and eBay AU buy links updated daily. Australia's DBS Fusion World price guide.">
  <link rel="canonical" href="${CANONICAL}">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="DBS Fusion World Card Prices Australia | Cards on Cards on Cards">
  <meta property="og:description" content="${sets.length||'48'}+ DBS Fusion World sets with live AUD prices and eBay AU buy links.">
...
<div class="hero fade-up">
  ...
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length||'48'}</div><div class="stat-label">Sets</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=dragon+ball+fusion+world+cards&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=5339146789&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:var(--accent);color:#000">&#128722; Shop DBS Fusion World on eBay &#8599;</a>
  ...
  <a href="/blog/dragon-ball-super-card-game-beginners-guide-australia/" class="quick-link" style="background:rgba(var(--accent-rgb),.08);border-color:rgba(var(--accent-rgb),.3);color:var(--accent)">&#128214; Beginners Guide &#8594;</a>
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header"><div class="section-title">Search DBS Fusion World Cards</div></div>
  ...
  <div class="section fade-up fade-up-3">
    <div class="section-header">
      <div class="section-title">Browse ${sets.length||'48'} Sets</div>
...
async function searchCard() {
  ...
  try {
    const res = await fetch('/api/compare-search?q=' + encodeURIComponent(q) + '&game=dbsfusionworld&limit=24');
    ...
    results.innerHTML = cards.map(c => {
      ...
      return '<a href="/cards/dbsfusionworld/'+c.slug+'" style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center;display:block;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'var(--border)\'">'
