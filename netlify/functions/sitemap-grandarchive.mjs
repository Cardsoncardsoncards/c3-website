// netlify/functions/sitemap-grandarchive.mjs
// Generates XML sitemap for grandarchive card pages at /cards/grandarchive/[slug]
// Registered in sitemap-index.xml as /api/sitemap-grandarchive
...
export default async (req) => {
  ...
  try {
    ...
    if (totalCount === 0) return empty('grandarchive card pages: pending sync or no priced cards');
    ...
    const xml = '<?xml version="1.0" encoding="UTF-8"?>\n'
      + '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'
      + '  <!-- grandarchive card pages: ' + allCards.length + ' cards with price >= AUD$' + PRICE_THRESHOLD + ' -->\n'
    ...
    console.error('[sitemap-grandarchive] error:', err.message);
    return new Response(
...
export const config = { path: '/api/sitemap-grandarchive' };
