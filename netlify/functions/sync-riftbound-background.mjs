    ...
    for (const set of allSets) {
      ...
      for (const card of setCards) {
        ...
        cardRows.push({
          ...
          // assumed before proceeding: of the 94 riftbound cards affected, ZERO have any price
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-riftbound-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: riftbound_price_snapshots.card_id is a FOREIGN KEY onto
      // riftbound_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('riftbound_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('riftbound_price_snapshots', chunk);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-riftbound] Upsert error for set ${set.name}:`, r.reason?.message);
      ...
      console.log(`[sync-riftbound] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }
