...
// Returns empty urlset with comment if table has no priced data yet
...
export default async (req) => {
  ...
  try {
    ...
    const countRes = await fetch(
      `${SUPABASE_URL}/rest/v1/pokemon_cards?select=id&price_usd=gte.${PRICE_THRESHOLD}&slug=not.is.null&limit=1`,
      {
        headers: {
          'apikey': SUPABASE_ANON_KEY,
          'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
    ...
    // Table exists but no priced data yet — return valid empty sitemap
    if (totalCount === 0) {
      return new Response(
        `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n<!-- Pokemon card pages: pending sync -->\n</urlset>`,
        { status: 200, headers }
      );
    }

    // Fetch all slugs in pages
    const allCards = [];
...
${urls}
    ...
    console.error('[sitemap-pokemon] error:', err.message);
    return new Response(
...
export const config = { path: '/api/sitemap-pokemon' };
