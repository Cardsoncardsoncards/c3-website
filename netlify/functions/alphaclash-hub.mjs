...
const EMOJI       = '&#9889;';
const SEARCH_PH   = "Brandon Nieves, Alpha Clash Champion...";
const SET_PH      = "Champion Deck, Starter Deck...";
...
export default async (req) => {
  ...
  const [setsResult, topCardsResult] = await Promise.allSettled([
    supabaseGet('alphaclash_sets?order=release_date.desc&limit=50&select=id,name,slug,abbreviation,release_date,card_count'),
    supabaseGet('alphaclash_cards?order=market_price.desc&market_price=gt.0.5&market_price=lt.200&image_url=not.is.null&limit=24&select=slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at')
  ]);
  ...
  const carouselHTML = topCards.map(c => {
    ...
    const ebay  = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name+' alpha clash tcg card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<a href="/cards/alphaclash/${c.slug}" class="carousel-card">
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/alphaclash/sets/${encodeURIComponent(s.slug||s.id)}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}">
...
<head>
  ...
  <meta name="description" content="Browse ${sets.length||'16'}+ Alpha Clash TCG sets. Live AUD card prices and eBay AU buy links updated daily. Australia's Alpha Clash TCG price guide.">
  <link rel="canonical" href="${CANONICAL}">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="Alpha Clash TCG Card Prices Australia | Cards on Cards on Cards">
  <meta property="og:description" content="${sets.length||'16'}+ Alpha Clash TCG sets with live AUD prices and eBay AU buy links.">
...
<div class="hero fade-up">
  ...
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length||'16'}</div><div class="stat-label">Sets</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=alpha+clash+tcg&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=5339146789&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:var(--accent);color:#000">&#128722; Shop Alpha Clash TCG on eBay &#8599;</a>
  ...
  <a href="/blog/alpha-clash-tcg-beginners-guide-australia/" class="quick-link" style="background:rgba(var(--accent-rgb),.08);border-color:rgba(var(--accent-rgb),.3);color:var(--accent)">&#128214; Beginners Guide &#8594;</a>
  <a href="/blog/best-alpha-clash-cards-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128230; Most Valuable Cards &#8594;</a>
</div>
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header"><div class="section-title">Search Alpha Clash TCG Cards</div></div>
  ...
  <div class="section fade-up fade-up-3">
    <div class="section-header">
      <div class="section-title">Browse ${sets.length||'16'} Sets</div>
...
<footer>
  ...
  <p style="margin-top:6px;font-size:11px;opacity:.5">Affiliate disclosure: this site earns commissions from eBay AU and Amazon AU purchases made through affiliate links at no extra cost to you. Not affiliated with Alpha Clash Entertainment. USD prices converted to AUD at approximately 1.58.</p>
</footer>
...
async function searchCard() {
  ...
  try {
    const res = await fetch('/api/compare-search?q=' + encodeURIComponent(q) + '&game=alphaclash&limit=24');
    ...
    results.innerHTML = cards.map(c => {
      ...
      return '<a href="/cards/alphaclash/'+c.slug+'" style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center;display:block;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'var(--border)\'">'
