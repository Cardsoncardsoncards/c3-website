...
export default async (req) => {
  ...
  try {
    ...
    const snapshots = await supabaseGet(`forceofwill_price_snapshots?card_id=eq.${encodeURIComponent(card.id)}&snapshot_date=gte.${_snapCutoff}&order=snapshot_date.asc&limit=90&select=snapshot_date,price_aud,market_price`).catch(() => []);
