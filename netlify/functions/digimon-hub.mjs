...
const EMOJI       = '&#128009;';
const SEARCH_PH   = "Agumon, Gabumon, WarGreymon...";
const SET_PH      = "BT-01, ST-01, EX-01...";
const CANONICAL   = 'https://cardsoncardsoncards.com.au/cards/digimon';
...
export default async (req) => {
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/digimon/sets/${encodeURIComponent(s.slug||s.id)}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}">
...
<div class="hero fade-up">
  <div class="hero-eyebrow">Card Vault -- Digimon TCG</div>
  <h1>Digimon TCG Card Prices <span>in Australia</span></h1>
  <p class="hero-sub">Digimon TCG card prices in AUD. Browse by set or search by card name. eBay AU buy links updated daily.</p>
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length||'98'}</div><div class="stat-label">Sets</div></div>
    <div class="stat-item"><div class="stat-num">9k+</div><div class="stat-label">Cards</div></div>
...
<div class="quick-links fade-up fade-up-1">
  ...
  <a href="/blog/best-digimon-booster-boxes-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128230; Best Digimon Boxes &#8594;</a>
</div>

${topCards.length ? `<section class="carousel-section fade-up fade-up-2">
  <div class="carousel-label">Most Valuable</div>
  <div class="carousel-title">Top Digimon TCG Cards by Price (AUD)</div>
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header"><div class="section-title">Search Digimon TCG Cards</div></div>
  ...
  <div class="section fade-up fade-up-3">
    <div class="section-header">
      <div class="section-title">Browse ${sets.length||'98'} Sets</div>
  ...
  <div style="background:rgba(var(--accent-rgb),.04);border:1px solid rgba(var(--accent-rgb),.15);border-radius:var(--radius);padding:22px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px">
    <div>
      <div style="font-size:16px;font-weight:700;color:var(--text);margin-bottom:5px">Track Your Digimon TCG Collection</div>
...
async function searchCard() {
  ...
  try {
    const res = await fetch('/api/compare-search?q=' + encodeURIComponent(q) + '&game=digimon&limit=24');
...
<script>
function filterDigimonByRarity(rarity) {
  document.querySelectorAll('.carousel-card').forEach(function(el) {
    var r = el.dataset.rarity || '';
    el.style.display = (rarity === 'All' || r === rarity) ? '' : 'none';
  });
  document.querySelectorAll('.dg-rarity-btn').forEach(function(b) {
...
export const config = { path: '/cards/digimon' };
