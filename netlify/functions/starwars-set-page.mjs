...
<body><div class="wrap"><div class="icon">🃏</div><h1>Set Not Found</h1><p>We couldn't find the Star Wars: Unlimited set "${setSlug}". It may not be in our database yet.</p><a href="/cards/starwars" class="btn">Browse All Star Wars Cards</a><a href="/" class="btn btn-sec">← Home</a></div></body>
</html>`;
}
...
export default async (req) => {
  ...
  try {
    ...
    const [cards, ebayListings] = await Promise.all([
      supabaseGet(`starwars_cards?set_id=eq.${set.id}&rarity=neq.None&order=market_price.desc.nullslast&limit=200&select=slug,name,number,image_url,market_price,price_aud,rarity,set_name,price_change_7d,price_change_30d`),
    ...
    function moverCard(c, isGainer) {
      ...
      const col = isGainer ? '#FFE81F' : '#F87171';
      return `<a href="/cards/starwars/${c.slug}" style="background:#0e1118;border:1px solid #1e2235;border-radius:8px;padding:10px 12px;text-decoration:none;display:flex;align-items:center;gap:10px;transition:border-color .2s" onmouseover="this.style.borderColor='${col}'" onmouseout="this.style.borderColor='#1e2235'">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:13px;font-weight:700;margin-top:3px;color:#FFE81F">${aud > 0 ? `AU$${aud.toFixed(2)}` : ''}</div>
        </div>
    ...
    <div style="background:linear-gradient(135deg,rgba(255,232,31,.06),rgba(255,232,31,.02));border:1px solid rgba(255,232,31,.2);border-radius:12px;padding:20px;margin-bottom:28px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
        <div>
          <div style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#FFE81F;margin-bottom:8px">&#9650; Biggest Gainers</div>
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .filt-btn:hover,.filt-btn.active{border-color:#FFE81F;color:#FFE81F;background:rgba(255,232,31,.08)}
  </style>
</head>
<body>
<nav>
  <div class="nav-inner">
    ...
    <div class="nav-links">
      ...
      <a href="/cards/starwars" class="nav-link active">Star Wars: Unlimited</a>
...
<div class="wrap">
  ...
  <div class="cta-row">
    ...
    <a href="https://www.amazon.com.au/s?k=${encodeURIComponent(set.name + ' Star')}&tag=${AMAZON_TAG}" target="_blank" rel="noopener" class="cta-btn cta-secondary" style="border-color:rgba(255,153,0,.35);color:#ff9900">Search Amazon AU →</a>
  </div>
  ...
  <div style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:20px;font-size:13px;color:#8892b0">
    <strong style="color:#F0F2FF">About this set:</strong> Star Wars: Unlimited card prices shown in AUD, converted from USD market data at approximately 1.58x. Prices update daily via tcgapi.dev. Always check eBay AU for live Australian market pricing before buying or selling.
    <div style="margin-top:10px"><a href="/cards/starwars" style="color:#FFE81F">← Back to all Star Wars: Unlimited cards</a></div>
  </div>
</div>
