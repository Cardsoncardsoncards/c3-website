...
<div class="wrap">
  ...
  ${(() => {
    ...
    const itemsHTML = sealedItems.slice(0,4).map(p => {
      ...
      return `<a href="/cards/starwars/${p.slug}" style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:14px;display:flex;flex-direction:column;gap:8px;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor='#60A5FA'" onmouseout="this.style.borderColor='#1e2235'">
        ...
        <div style="font-size:15px;font-weight:900;color:#60A5FA;font-family:'Cinzel',serif">${price}</div>
        ${low ? `<div style="font-size:11px;color:#8892b0">${low}</div>` : ''}
      </a>`;
    }).join('');
    const setNm = (set.name||'').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    return `<div class="section" style="background:#60A5FA0a;border:1px solid #60A5FA26;border-radius:14px;padding:22px 24px;margin-bottom:32px">
      <div style="font-size:9px;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:#60A5FA;margin-bottom:8px">Sealed Product</div>
