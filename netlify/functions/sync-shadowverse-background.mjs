// netlify/functions/sync-shadowverse-background.mjs
// Daily sync -- schedule: 0 5 * * * UTC
// Fetches all shadowverse-evolve sets + cards + prices from tcgapi.dev Pro
// Upserts into shadowverse_sets, shadowverse_cards, shadowverse_price_snapshots
...
const GAME_SLUG            = 'shadowverse-evolve';
...
export default async (req) => {
  ...
  console.log('[sync-shadowverse] Starting...');
  const start = Date.now();

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('[sync-shadowverse] Missing Supabase env vars');
    return new Response('Supabase env vars missing', { status: 500 });
  }
  if (!TCGAPI_KEY) {
    console.error('[sync-shadowverse] TCGAPI_KEY not set');
    return new Response('TCGAPI_KEY missing', { status: 500 });
  }

  try {
    const audRate = await getExchangeRate();
    console.log(`[sync-shadowverse] AUD rate: ${audRate}`);
    ...
    console.log(`[sync-shadowverse] Found ${allSets.length} sets`);
    ...
    for (let i = 0; i < setRows.length; i += 100) {
      await supabaseUpsert('shadowverse_sets', setRows.slice(i, i + 100));
    }
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-shadowverse] Bulk price fetch failed for set ${set.id}:`, e.message);
        }
      }
      ...
      for (const card of setCards) {
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-shadowverse-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: shadowverse_price_snapshots.card_id is a FOREIGN KEY onto
      // shadowverse_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('shadowverse_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('shadowverse_price_snapshots', chunk);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-shadowverse] Upsert error for set ${set.name}:`, r.reason?.message);
      ...
      console.log(`[sync-shadowverse] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }

    const elapsed = ((Date.now() - start) / 1000).toFixed(1);
    console.log(`[sync-shadowverse] Done. ${totalCards} cards, ${totalSnaps} snapshots in ${elapsed}s`);
    ...
    console.error('[sync-shadowverse] FATAL:', err.message);
