...
// Group A sync (Mon/Wed/Fri/Sun) - schedule: 0 0 * * 1,3,5,0 UTC
...
export default async (req) => {
  console.log('[sync-bakugan] Starting (background function)...');
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-bakugan] Bulk price error set ${set.id}:`, e.message);
        }
      }
