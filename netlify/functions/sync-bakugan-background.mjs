...
export default async (req) => {
  ...
  try {
    ...
    for (const set of allSets) {
      ...
      const results = await Promise.allSettled([
        (async () => {
          for (let i = 0; i < cardRows.length; i += 200) {
            await supabaseUpsert('bakugan_cards', cardRows.slice(i, i + 200));
          }
        })(),
        (async () => {
          for (let i = 0; i < snapRows.length; i += 500) {
            await supabaseUpsertSnapshots('bakugan_price_snapshots', snapRows.slice(i, i + 500));
          }
        })()
      ]);
      ...
      console.log(`[sync-bakugan] ${set.name}: ${cardRows.length} cards, ${snapRows.length} snapshots`);
    }
