...
export default async (req) => {
  ...
  const setSlug = decodeSlugSegment(url.pathname.replace(/^\/cards\/riftbound\/sets\//, '').replace(/\/$/, ''));
  ...
  try {
    ...
    const legacy = await numericSetRedirect(setSlug, 'riftbound_sets', '/cards/riftbound/sets', supabaseGet)
    ...
    const [cardsR, sealedR, ebayListingsR] = await Promise.allSettled([
      ...
      supabaseGet(`riftbound_cards?set_id=eq.${set.id}&or=(rarity.eq.None,rarity.is.null)&order=market_price.desc.nullslast&limit=300&select=slug,name,image_url,market_price,price_aud,low_price`),
...
<div class="wrap">
  ...
  <div style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:20px;font-size:13px;color:#8892b0">
    <strong style="color:#F0F2FF">About this set:</strong> Riftbound card prices shown in AUD, converted from USD market data at approximately ${audRate.toFixed(2)}x. Prices update daily via tcgapi.dev. Always check eBay AU for live Australian market pricing before buying or selling.
...
export const config = { path: '/cards/riftbound/sets/:slug+', priority: 1 };
