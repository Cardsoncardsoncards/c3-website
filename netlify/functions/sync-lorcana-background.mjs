...
// Card Details enrichment: Lorcast (free, no key). Join key = card name.
async function fetchLorcanaStats(name) {
  if (!name) return null;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 8000);
  try {
    const res = await fetch(`https://api.lorcast.com/v0/cards/search?q=${encodeURIComponent(name)}`, { signal: controller.signal });
    clearTimeout(timeout);
    if (!res.ok) return null;
    const data = await res.json();
    const c = data && Array.isArray(data.results) ? data.results[0] : null;
    if (!c) return null;
    return {
      cost: c.cost ?? null, ink: c.ink ?? null,
      inkwell: (typeof c.inkwell === 'boolean' ? c.inkwell : null),
      strength: c.strength ?? null, willpower: c.willpower ?? null, lore: c.lore ?? null
    };
...
export default async (req) => {
  ...
  try {
    ...
    for (const set of allSets) {
      ...
      for (const r of await supabaseGet(`lorcana_cards?set_id=eq.${set.id}&custom_attributes=not.is.null&select=id,custom_attributes`)) {
      ...
      for (const card of setCards) {
        ...
        if (!enrichedMap.has(card.id) && enrichCount < ENRICH_MAX_PER_RUN) {
          const stats = await fetchLorcanaStats(card.name);
        ...
        cardRows.push({
          ...
          // update_lorcana_price_changes() computes BOTH fields with the identical 1 day
          ...
          //   24h 3,126 to 3,398 (wider)
          //   30d 3,379 to 3,274 (NARROWER by 105, accepted for the same reason)
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-lorcana-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: lorcana_price_snapshots.card_id is a FOREIGN KEY onto
      // lorcana_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('lorcana_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('lorcana_price_snapshots', chunk);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-lorcana] Upsert error for set ${set.name}:`, r.reason?.message);
      ...
      console.log(`[sync-lorcana] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }
