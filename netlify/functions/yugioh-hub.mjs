...
// Confirmed columns: yugioh_cards(slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at,price_change_7d)
// Confirmed columns: yugioh_sets(id,name,slug,release_date,card_count)
// Sets limit 700 -- YGO has 300+ sets
// Carousel uses price_aud=gt.1 -- many YGO commons have near-zero prices
...
const GAME       = 'yugioh';
const GAME_LABEL = 'Yu-Gi-Oh TCG';
const ACCENT     = '#8B5CF6';
const ACCENT_RGB = '139,92,246';
const EMOJI      = '&#128065;';
const SET_PH     = 'Battles of Legend, Tactical Masters, Dark Neostorm...';
const CANONICAL  = 'https://cardsoncardsoncards.com.au/cards/yugioh';
...
export default async (req) => {
  ...
  const SEALED = ['booster box','booster pack',' case','bundle','display','sealed product','starter deck','starter set','trial deck','box set','collection box','premium set','gift set','structure deck'];
  ...
  function carouselCard(c, suffix) {
    ...
    const ebay  = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name+' yugioh card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<div class="carousel-card"><a href="/cards/yugioh/${esc(c.slug)}" style="display:block;text-decoration:none"><div class="carousel-img-wrap"><img src="${esc(c.image_url)}${suffix}" alt="${esc(c.name).replace(/"/g,'&quot;')}" loading="eager" onerror="this.onerror=null;this.src='/card-placeholder.svg';this.style.opacity='1'"></div><div class="carousel-name">${esc(c.name)}</div>${c.rarity?`<div class="carousel-rarity">${esc(c.rarity)}</div>`:''}<div class="carousel-price">${price}</div></a><div class="carousel-buy-row"><a href="${ebay}" target="_blank" rel="noopener" onclick="gtag('event','ebay_click',{'event_category':'affiliate','event_label':'yugioh-hub'})" class="carousel-buy-btn">Buy eBay &#8599;</a></div></div>`;
  }
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/yugioh/sets/${encodeURIComponent(s.slug||s.id)}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}"><span class="set-tile-name">${esc(name)}${newBadge}</span><span class="set-tile-meta">${year}${s.card_count?' &middot; '+s.card_count+' cards':''}</span></a>`;
...
<div class="hero fade-up">
  <div class="hero-eyebrow">Card Vault -- Yu-Gi-Oh TCG</div>
  <h1>Yu-Gi-Oh Card Prices <span>in Australia</span></h1>
  <p class="hero-sub">Yu-Gi-Oh TCG card prices in AUD. Browse by set or search by card name. eBay AU buy links updated daily.</p>
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length||'300'}</div><div class="stat-label">Sets</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=yugioh+cards&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" onclick="gtag('event','ebay_click',{'event_category':'affiliate','event_label':'yugioh-hub'})" class="quick-link" style="background:linear-gradient(135deg,#4c1d95,${ACCENT});color:#fff">&#128722; Shop Yu-Gi-Oh on eBay &#8599;</a>
...
${hasMovers ? `<div style="margin-bottom:28px">
  ...
  <p style="color:var(--text2);font-size:13px;margin-bottom:8px">Biggest price movers across all Yu-Gi-Oh cards in the last 7 days.</p>
  ...
  <div class="section fade-up fade-up-3">
    <div style="text-align:center;margin-bottom:16px">
      <h2 style="font-family:'Cinzel',serif;font-size:24px;font-weight:700;color:var(--text);margin-bottom:6px">Browse Every Yu-Gi-Oh Set</h2>
      <p style="font-size:13px;color:var(--text2)">${sets.length||'300'}+ sets. Pick a letter or search to explore.</p>
    </div>
  ...
  <div style="margin-bottom:28px">
    <h2 style="font-size:18px;margin-bottom:16px">Yu-Gi-Oh Guides and Blogs</h2>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px">
      <a href="/blog/yugioh-booster-boxes-australia/" class="guide-card"><div class="guide-title">Best Yu-Gi-Oh Booster Boxes in Australia</div><div class="guide-desc">Which YGO boxes are worth opening and where to buy in Australia.</div></a>
      <a href="/ev-calculator.html" class="guide-card"><div class="guide-title">Booster Box EV Calculator</div><div class="guide-desc">Is your Yu-Gi-Oh box worth opening? Calculate expected value before you crack it.</div></a>
  ...
  <div style="background:rgba(${ACCENT_RGB},.04);border:1px solid rgba(${ACCENT_RGB},.15);border-radius:var(--radius);padding:22px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px">
    <div>
      <div style="font-size:16px;font-weight:700;color:var(--text);margin-bottom:5px">Track Your Yu-Gi-Oh Collection</div>
...
<footer>
  ...
  <div style="margin-bottom:10px">
    <a href="/">Home</a><a href="/blog">Blog</a><a href="/calendar">Calendar</a><a href="/account">Account</a><a href="/tracker">Tracker</a><a href="/cards">Card Prices</a><a href="/compare">Compare</a><a href="/market">Market</a><a href="/tools">Tools</a><a href="/contact">Contact</a><a href="/legal">Legal</a><a href="https://www.ebay.com.au/str/cardsoncardsoncards?mkcid=1&amp;mkrid=705-53470-19255-0&amp;campid=5339146789&amp;customid=Footer&amp;toolid=10001&amp;mkevt=1" target="_blank" rel="noopener" onclick="gtag('event','ebay_click',{'event_category':'affiliate','event_label':'footer'})">eBay</a><a href="https://blasdigital.etsy.com" target="_blank" rel="noopener">D&amp;D Tools on Etsy &#8599;</a><br><a href="/cards/yugioh">Yu-Gi-Oh</a><a href="/cards/pokemon">Pokemon</a><a href="/cards/mtg">MTG</a><a href="/cards/lorcana">Lorcana</a><a href="/cards/onepiece">One Piece</a>
  </div>
  <p>&#169; 2026 Cards on Cards on Cards &middot; cardsoncardsoncards.com.au</p>
  <p style="margin-top:6px;font-size:11px;opacity:.5">Affiliate disclosure: this site earns commissions from eBay AU and Amazon AU purchases made through affiliate links at no extra cost to you. Not affiliated with Konami. USD prices converted to AUD at approximately ${audRate.toFixed(2)}.</p>
