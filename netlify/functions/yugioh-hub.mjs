...
const GAME_COLOUR       = '#C084FC';

const CALENDAR_EVENTS = []; // No confirmed upcoming YuGiOh AU events in data
...
export default async (req) => {
  ...
  const [sets, topCards] = await Promise.all([
    supabaseGet('yugioh_sets?order=release_date.desc&limit=400&select=id,name,slug,abbreviation,card_count,release_date'),
  ...
  const carouselHTML = topCards.map(c => `
    <a href="/cards/yugioh/${c.slug}" class="carousel-card">
      ...
      ${c.type ? `<div class="carousel-rarity">${c.type}</div>` : ''}
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    const slug = encodeURIComponent(s.abbreviation || s.slug || s.id);
    return `<a href="/cards/yugioh/sets/${slug}" class="set-tile" data-name="${name.toLowerCase().replace(/"/g,'&quot;')}" data-letter="${letterKey}">
...
<head>
  ...
  <style>
    :root{--bg:#0A0C14;--bg2:#111420;--bg3:#181d2e;--gold:#C9A84C;--accent:#C084FC;--text:#e8eaf0;--text2:#9ba3c4;--border:#242840;--radius:12px;--silver:#A0A8C0}
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    body::before{content:'';position:fixed;inset:0;pointer-events:none;z-index:0;background:radial-gradient(ellipse 70% 40% at 50% 0%,rgba(192,132,252,.05),transparent 60%)}
    ...
    .carousel-card:hover{border-color:var(--accent);transform:translateY(-4px);box-shadow:0 8px 20px rgba(192,132,252,.12);text-decoration:none}
    ...
    .set-tile:hover{border-color:var(--accent);background:rgba(192,132,252,.04);text-decoration:none;transform:translateX(2px)}
...
<div class="hero fade-up">
  ...
  <p class="hero-sub">Yu-Gi-Oh card prices in AUD. Browse by archetype, attribute or set. eBay AU buy links updated daily.</p>
  <div class="stat-bar">
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=yugioh+cards&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:linear-gradient(135deg,#9333ea,var(--accent));color:#fff">&#128722; Shop Yu-Gi-Oh on eBay &#8599;</a>
  ...
  <a href="/quizzes/mtg-archetype" class="quick-link" style="background:rgba(192,132,252,.08);border-color:rgba(192,132,252,.3);color:var(--accent)">&#127919; What's Your Archetype? &#8594;</a>
  ...
  <a href="/blog/yugioh-beginners-guide-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128214; YGO Guide &#8594;</a>
</div>
${topCards.length ? `
...
<div class="wrap">
  ...
  <div class="section fade-up fade-up-3">
    ...
    <input type="text" id="set-search" placeholder="Search sets e.g. Blazing Dominion, Burst Protocol..." oninput="filterSets(this.value)" style="margin-bottom:12px">
    <div id="set-list" class="set-grid">${setListHTML}</div>
  </div>
  <div style="background:rgba(192,132,252,.04);border:1px solid rgba(192,132,252,.15);border-radius:var(--radius);padding:22px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px">
    <div>
...
<footer>
  ...
  <p style="margin-top:6px;font-size:11px;opacity:.5">Not affiliated with Konami. Prices converted to AUD at approximately 1.58.</p>
</footer>
<script>
let activeAZ = 'All';
