...
export default async (req) => {
  ...
  // Get unique archetypes sorted by card count
  ...
  const setListHTML = sets.length ? sets.map(s => `
    <div style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:8px 12px"
  ...
  const topCardHTML = topCards.map(c => {
    ...
    return `<a href="/cards/yugioh/${c.slug}" style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center;display:block;transition:border-color .2s" onmouseover="this.style.borderColor='var(--ygo-gold)'" onmouseout="this.style.borderColor='var(--border)'">
      ${c.image_uri_small||c.image_uri ? `<img src="${c.image_uri_small||c.image_uri}" alt="${c.name}" style="width:100%;border-radius:4px;max-height:160px;object-fit:contain">` : ''}
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    body::before{content:'';position:fixed;inset:0;pointer-events:none;background:radial-gradient(ellipse 60% 40% at 50% 0%,rgba(107,45,139,.08),transparent 60%),radial-gradient(ellipse 40% 30% at 80% 80%,rgba(200,163,50,.05),transparent 50%)}
    ...
    .attr-filters{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:16px}
    .attr-btn{padding:4px 12px;border-radius:4px;font-size:11px;font-weight:700;cursor:pointer;border:1px solid transparent;transition:all .2s;background:var(--bg3)}
    .attr-btn.active{border-color:currentColor}
    .archetype-chips{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:0}
    .archetype-chip{padding:5px 12px;border-radius:6px;font-size:12px;font-weight:600;background:rgba(200,163,50,.08);border:1px solid rgba(200,163,50,.2);color:var(--ygo-gold);cursor:pointer;transition:all .2s}
    .archetype-chip:hover{background:rgba(200,163,50,.15);text-decoration:none}
    ...
    #search-results{margin-top:16px;display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:10px}
  </style>
</head>
<body>
<nav>
...
<div class="hero">
  ...
  <p class="hero-sub">Live AUD pricing for Yu-Gi-Oh cards. Browse by archetype, attribute, or set. eBay AU buy links on every card.</p>
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length ? sets.length + '+' : '—'}</div><div class="stat-label">Sets</div></div>
    <div class="stat-item"><div class="stat-num">${topCards.length ? '12k+' : '—'}</div><div class="stat-label">Cards</div></div>
...
<div class="wrap">
  ...
  <div class="quick-links">
    <a href="/tracker.html" class="quick-link" style="background:linear-gradient(135deg,#c8a332,#a87e1a);color:#000">📋 Free Tracker</a>
  ...
  <div class="section">
    ...
    <div class="attr-filters">
      ...
      <button class="attr-btn" style="color:var(--text2)" onclick="clearAttr(this)">All</button>
    </div>
    ...
    <div style="margin-top:4px;margin-bottom:8px;font-size:11px;color:var(--text2);font-weight:700;letter-spacing:.1em;text-transform:uppercase">Popular Archetypes</div>
    <div class="archetype-chips">
      ${topArchetypes.map(a => `<a href="#" class="archetype-chip" onclick="searchArchetype('${a.replace(/'/g,"\\'")}');return false">${a}</a>`).join('')}
    </div>` : ''}
  ...
  <div class="section">
    ...
    <input type="text" id="set-search" placeholder="Search sets e.g. Phantom Nightmare, Metal Raiders..." oninput="filterSets(this.value)" style="margin-bottom:12px">
  ...
  <div style="text-align:center;padding:40px;background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);margin-bottom:28px">
    ...
    <p style="color:var(--text2);font-size:14px;max-width:400px;margin:0 auto 20px">The first Yu-Gi-Oh sync is running overnight. Check back tomorrow — all cards will be live.</p>
...
async function searchArchetype(archetype) {
  const results = document.getElementById('search-results');
  results.innerHTML = '<div style="color:var(--text2);font-size:13px">Loading ' + archetype + ' cards...</div>';
  document.getElementById('card-search').value = archetype;
  try {
    const res = await fetch(
      window.C3_SUPA_URL + '/rest/v1/yugioh_cards?archetype=eq.' + encodeURIComponent(archetype) + '&select=slug,name,image_uri_small,image_uri,price_usd,type,attribute&order=price_usd.desc&limit=24',
      { headers: { 'apikey': window.C3_SUPA_KEY } }
    );
    renderResults(await res.json());
  } catch(e) {
    results.innerHTML = '<div style="color:#f88">Search error.</div>';
  }
}
...
async function searchCard() {
  ...
  try {
    ...
    if (q.length >= 2) url += '&name=ilike.*' + encodeURIComponent(q) + '*';
...
const ATTR_COLOURS = ${JSON.stringify(ATTR_COLOURS)};
...
function renderResults(cards) {
  ...
  results.innerHTML = cards.map(c => {
    const attrColour = ATTR_COLOURS[c.attribute] || '#888';
    return \`<a href="/cards/yugioh/\${c.slug}" style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center;display:block;transition:border-color .2s" onmouseover="this.style.borderColor='var(--ygo-gold)'" onmouseout="this.style.borderColor='var(--border)'">
      \${(c.image_uri_small||c.image_uri) ? \`<img src="\${c.image_uri_small||c.image_uri}" alt="\${c.name}" style="width:100%;border-radius:4px;max-height:140px;object-fit:contain">\` : ''}
      \${c.attribute ? \`<div style="font-size:9px;color:\${attrColour};margin-top:4px;font-weight:700">\${c.attribute}</div>\` : ''}
      <div style="font-size:11px;color:var(--text);line-height:1.3;margin-top:2px">\${c.name}</div>
      <div style="font-size:12px;color:var(--ygo-gold);font-weight:700">\${c.price_usd ? '~AU$'+(c.price_usd*1.58).toFixed(2) : ''}</div>
    </a>\`;
  }).join('');
}
</script>
