...
export default async (req) => {
  ...
  const [sets, topCards] = await Promise.all([
    ...
    supabaseGet('yugioh_cards?order=price_usd.desc&price_usd=gt.0&image_uri=not.is.null&limit=24&select=slug,name,image_uri,image_uri_small,price_usd,type,attribute,archetype')
  ]);
  ...
  const carouselHTML = topCards.map(c => `
    <a href="/cards/yugioh/${c.slug}" class="carousel-card">
      <div class="carousel-img-wrap">
        <img src="${c.image_uri_small||c.image_uri}" alt="${c.name}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=card-placeholder>${c.name[0]}</div>'">
      </div>
...
<head>
  ...
  <meta name="description" content="Browse Yu-Gi-Oh card prices in AUD. Live AUD pricing, eBay AU buy links. Australia's Pokemon price guide updated daily.">
  ...
  <style>
    :root{
      ...
      --ygo-purple:#7B2D8B;--ygo-gold:#C9A84C;--ygo-blue:#4A9EFF;
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .nav-link--pokemon{color:var(--ygo-gold);border-color:rgba(255,204,0,.4);background:rgba(255,204,0,.07)}
    ...
    .stat-num{font-family:'Cinzel',serif;font-size:20px;font-weight:700;color:var(--ygo-gold)}
    ...
    .carousel-label{font-size:10px;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:var(--ygo-gold);margin-bottom:10px;padding:0 24px}
    ...
    .carousel-card:hover{border-color:var(--ygo-gold);transform:translateY(-4px);box-shadow:0 8px 24px rgba(255,204,0,.15);text-decoration:none}
    ...
    .carousel-price{font-size:13px;color:var(--ygo-gold);font-weight:700;margin-top:3px}
    ...
    .set-tile:hover{border-color:var(--ygo-gold);background:rgba(255,204,0,.04);text-decoration:none;transform:translateX(2px)}
...
<div class="hero fade-up">
  ...
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length || '200+'}</div><div class="stat-label">Sets</div></div>
...
<div class="wrap">
  ...
  <div style="background:rgba(255,204,0,.04);border:1px solid rgba(255,204,0,.15);border-radius:var(--radius);padding:24px;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px">
    <div>
      <div style="font-size:17px;font-weight:700;color:var(--text);margin-bottom:6px">Track Your Yu-Gi-Oh Collection</div>
...
async function searchCard() {
  ...
  try {
    const url = window.C3_SUPA_URL + '/rest/v1/yugioh_cards?select=slug,name,image_uri,image_uri_small,price_usd,type,attribute,archetype&order=price_usd.desc&limit=24&name=ilike.*' + encodeURIComponent(q) + '*';
    ...
    results.innerHTML = cards.map(c => \`<a href="/cards/pokemon/\${c.slug}" style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center;display:block;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor='var(--ygo-gold)'" onmouseout="this.style.borderColor='var(--border)'">
