...
async function handleSetPage(setSlug, headers) {
  ...
  const cards = await supabaseGet(`yugioh_cards?set_name=eq.${encodeURIComponent(set.name)}&order=market_price.desc.nullslast&limit=400&select=slug,name,number,image_url,market_price,price_aud,rarity,type,attribute`);
  ...
  const ebaySetURL = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name + ' yugioh')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=5339146789&toolid=10001&mkevt=1`;
  ...
  const top5HTML = top5.map(c => {
    const aud = toAud(c);
    return `<a href="/cards/yugioh/${c.slug}" style="flex:0 0 140px;background:#0e1118;border:1px solid rgba(139,92,246,.35);border-radius:10px;padding:10px;text-align:center;text-decoration:none;display:block">
...
<nav>
  <div class="nav-inner">
    ...
    <div class="nav-links">
      ...
      <a href="/cards/yugioh" class="nav-link" style="color:${accent};border-color:rgba(139,92,246,.4)">Yu-Gi-Oh</a>
...
<div class="wrap">
  ...
  <div class="cta-row">
    ...
    <a href="https://www.amazon.com.au/s?k=${encodeURIComponent(set.name + ' yugioh')}&tag=blasdigital-22" target="_blank" rel="noopener" class="cta-btn cta-secondary" style="border-color:rgba(255,153,0,.35);color:#ff9900">Search Amazon AU →</a>
  </div>
  ...
  <div style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:20px;font-size:13px;color:#8892b0">
    <strong style="color:#F0F2FF">About this set:</strong> Yu-Gi-Oh card prices in AUD. Updated daily.
