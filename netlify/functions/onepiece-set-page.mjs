...
// Serves /cards/onepiece/sets/:setCode
...
export default async (req) => {
  ...
  if (!setCode) {
    return new Response(`<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><title>Set Not Found | Cards on Cards on Cards</title></head><body style="background:#0A0C14;color:#F0F2FF;font-family:sans-serif;padding:60px 24px;text-align:center">${NAV}<h1 style="font-family:'Cinzel',serif;color:#CC0000;margin-top:60px">Set Not Found</h1><p style="color:#A0A8C0;margin-top:16px">This One Piece set doesn't exist or hasn't been synced yet.</p><a href="/cards/onepiece" style="display:inline-block;margin-top:24px;padding:10px 24px;background:rgba(204,0,0,.15);border:1px solid rgba(204,0,0,.4);color:#FF4444;border-radius:8px;text-decoration:none">Browse All Sets</a></body></html>`, { status: 404, headers });
  }

  // Try to find set by abbreviation first, then slug, then id
  let sets = await supabaseGet(`onepiece_sets?abbreviation=eq.${encodeURIComponent(setCode)}&limit=1`);
  if (!sets || !sets[0]) sets = await supabaseGet(`onepiece_sets?slug=eq.${encodeURIComponent(setCode)}&limit=1`);
  if (!sets || !sets[0]) sets = await supabaseGet(`onepiece_sets?id=eq.${encodeURIComponent(setCode)}&limit=1`);

  if (!sets || !sets[0]) {
    return new Response(`<!DOCTYPE html><html lang="en-AU"><head><meta charset="UTF-8"><title>Set Not Found | Cards on Cards on Cards</title><link rel="icon" type="image/png" href="/c3logo.png"></head><body style="background:#0A0C14;color:#F0F2FF;font-family:sans-serif">${NAV}<div style="padding:80px 24px;text-align:center"><h1 style="font-family:'Cinzel',serif;color:#CC0000;margin-bottom:16px">Set Not Found</h1><p style="color:#A0A8C0;margin-bottom:24px">The set "${setCode}" doesn't exist or hasn't synced yet. Try again after 10am AEST.</p><a href="/cards/onepiece" style="display:inline-block;padding:10px 24px;background:rgba(204,0,0,.15);border:1px solid rgba(204,0,0,.4);color:#FF4444;border-radius:8px;text-decoration:none">Browse All One Piece Sets</a></div></body></html>`, { status: 404, headers });
  }
  ...
  const [cards, ebayToken] = await Promise.all([
    supabaseGet(`onepiece_cards?set_id=eq.${set.id}&order=market_price.desc.nullslast&select=id,slug,name,number,rarity,image_url,market_price,price_aud,low_price&limit=500`),
    getEbayToken()
  ]);

  const ebaySearchURL = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name+' one piece card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
  const ebayBoxURL = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(set.name+' one piece booster box')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
  const ebayListings = await getEbayListings(`${set.name} one piece card`, ebayToken);
  ...
  const contextText = pricedCards.length >= 2
    ? `${set.name} contains ${cards.length} One Piece TCG cards. The most valuable are <strong>${pricedCards[0].name}</strong> at ~AU$${toAud(pricedCards[0]).toFixed(0)} and <strong>${pricedCards[1].name}</strong> at ~AU$${toAud(pricedCards[1]).toFixed(0)}. Prices updated daily.`
    : `${set.name} contains ${cards.length} One Piece TCG cards. Prices updated daily in AUD.`;

  const top5HTML = top5.map(c => {
    const aud = toAud(c);
    return `<a href="/cards/onepiece/${c.slug}" style="flex:0 0 150px;background:#0e1118;border:1px solid rgba(204,0,0,.25);border-radius:10px;padding:10px;text-align:center;text-decoration:none;display:block;transition:all .2s" onmouseover="this.style.borderColor='#CC0000';this.style.transform='translateY(-2px)'" onmouseout="this.style.borderColor='rgba(204,0,0,.25)';this.style.transform='none'">
      ...
      <div style="font-family:'Cinzel',serif;font-size:14px;color:#CC0000;font-weight:700;margin-top:3px">~AU$${aud.toFixed(0)}</div>
    </a>`;
  }).join('');
  ...
  const cardGrid = cards.map(c => {
    ...
    return `<a href="/cards/onepiece/${c.slug}" class="card-item" data-rarity="${(c.rarity||'').toLowerCase()}" data-price="${aud.toFixed(2)}">
      ...
      <div style="font-size:11px;color:#CC0000;font-weight:700;margin-top:2px">${priceDisplay}</div>
    </a>`;
  }).join('');
  ...
  const ebayCarouselHTML = ebayListings.length ? `
    <div style="margin-top:40px;padding:28px;background:rgba(204,0,0,.04);border:1px solid rgba(204,0,0,.15);border-radius:14px">
      <p style="font-size:10px;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:#CC0000;margin-bottom:6px">Live eBay AU Listings</p>
      ...
      <div style="display:flex;gap:12px;overflow-x:auto;padding-bottom:12px">
        ${ebayListings.map(item => {
          ...
          return `<a href="${epnUrl}" target="_blank" rel="noopener sponsored" style="flex:0 0 160px;background:#161929;border:1px solid #252840;border-radius:12px;overflow:hidden;text-decoration:none;display:flex;flex-direction:column;transition:all .22s" onmouseover="this.style.borderColor='rgba(204,0,0,.4)'" onmouseout="this.style.borderColor='#252840'">
            ...
            <div style="padding:9px 11px;flex:1;display:flex;flex-direction:column;gap:4px">
              ...
              <div style="font-family:'Cinzel',serif;font-size:13px;font-weight:700;color:#CC0000;margin-top:4px">${price}</div>
              <div style="font-size:10px;color:rgba(160,168,192,.5)">View on eBay ↗</div>
            </div>
          </a>`;
        }).join('')}
      </div>
      <div style="text-align:right;margin-top:8px"><a href="${ebaySearchURL}" target="_blank" rel="noopener" style="font-size:12px;color:#CC0000;text-decoration:none;opacity:.7">View all listings ↗</a></div>
      ...
      <a href="${ebaySearchURL}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:8px;padding:10px 20px;background:rgba(204,0,0,.12);border:1px solid rgba(204,0,0,.3);color:#CC0000;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none">Shop ${set.name} on eBay AU ↗</a>
    </div>`;

  const schemaLD = JSON.stringify({"@context":"https://schema.org","@type":"CollectionPage","name":`${set.name} One Piece Card Prices Australia`,"description":`Browse all ${cards.length} ${set.name} One Piece cards with AUD prices and eBay AU buy links.`,"url":`https://cardsoncardsoncards.com.au/cards/onepiece/sets/${setCode}`});
...
<title>${set.name} Card Prices Australia | One Piece TCG | Cards on Cards on Cards</title>
<meta name="description" content="Browse all ${cards.length} ${set.name} One Piece cards with AUD prices. Live pricing and eBay AU buy links updated daily.">
<link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/onepiece/sets/${setCode}">
...
<style>
:root{--bg:#0A0C14;--bg2:#111420;--bg3:#181d2e;--text:#F0F2FF;--text2:#A0A8C0;--border:#252840;--accent:#CC0000;}
...
<div class="wrap" style="padding-top:32px">
  <div style="margin-bottom:8px">
    <a href="/cards/onepiece" style="font-size:12px;color:var(--text2);text-decoration:none">← All One Piece Sets</a>
  </div>
  ...
  <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:28px">
    <a href="${ebaySearchURL}" target="_blank" rel="noopener" style="background:rgba(204,0,0,.12);border:1px solid rgba(204,0,0,.3);color:#CC0000;padding:9px 16px;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none">🛒 Buy Singles on eBay AU ↗</a>
    ...
    <a href="/blog/best-one-piece-tcg-booster-boxes-australia/" style="background:var(--bg2);border:1px solid var(--border);color:var(--text2);padding:9px 16px;border-radius:8px;font-size:13px;text-decoration:none">📖 Is it Worth Opening?</a>
  </div>
  <div style="background:rgba(204,0,0,.04);border:1px solid rgba(204,0,0,.15);border-radius:10px;padding:16px 20px;margin-bottom:24px;font-size:13px;color:var(--text2);line-height:1.6">${contextText}</div>
  ...
  <div style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:24px;margin-top:36px">
    ...
    <details style="margin-top:12px;border-bottom:1px solid var(--border);padding-bottom:12px">
      ...
      <p style="font-size:13px;color:var(--text2);margin-top:8px">${set.name} contains ${cards.length} One Piece TCG cards in the C3 database.</p>
    </details>
...
export const config = { path: '/cards/onepiece/sets/:setCode+' };
