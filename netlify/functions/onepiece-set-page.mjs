...
<head>
  ...
  <title>Set Not Found | One Piece TCG | Cards on Cards on Cards</title>
  ...
  <style>*{box-sizing:border-box;margin:0;padding:0}body{background:#0A0C14;color:#F0F2FF;font-family:'DM Sans',sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px;text-align:center}.wrap{max-width:420px}.icon{font-size:48px;margin-bottom:16px}h1{font-family:'Cinzel',serif;color:#F97316;font-size:22px;margin-bottom:10px}p{color:#8892b0;font-size:14px;margin-bottom:24px;line-height:1.6}.btn{display:inline-block;background:#F97316;color:#000;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px}.btn-sec{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);color:#F0F2FF}</style>
</head>
<body><div class="wrap"><div class="icon">🃏</div><h1>Set Not Found</h1><p>We couldn't find the One Piece TCG set "${setSlug}". It may not be in our database yet.</p><a href="/cards/onepiece" class="btn">Browse All One Piece Cards</a><a href="/" class="btn btn-sec">← Home</a></div></body>
</html>`;
}
...
export default async (req) => {
  ...
  try {
    ...
    const [cards, ebayListings] = await Promise.all([
      supabaseGet(`onepiece_cards?set_id=eq.${set.id}&rarity=neq.None&order=market_price.desc.nullslast&limit=200&select=slug,name,number,image_url,market_price,price_aud,rarity,set_name,price_change_7d,price_change_30d`),
    ...
    function moverCard(c, isGainer) {
      ...
      const col = isGainer ? '#F97316' : '#F87171';
      return `<a href="/cards/onepiece/${c.slug}" style="background:#0e1118;border:1px solid #1e2235;border-radius:8px;padding:10px 12px;text-decoration:none;display:flex;align-items:center;gap:10px;transition:border-color .2s" onmouseover="this.style.borderColor='${col}'" onmouseout="this.style.borderColor='#1e2235'">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:13px;font-weight:700;margin-top:3px;color:#F97316">${aud > 0 ? `AU$${aud.toFixed(2)}` : ''}</div>
        </div>
    ...
    <div style="background:linear-gradient(135deg,rgba(249,115,22,.06),rgba(249,115,22,.02));border:1px solid rgba(249,115,22,.2);border-radius:12px;padding:20px;margin-bottom:28px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
        <div>
          <div style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#F97316;margin-bottom:8px">&#9650; Biggest Gainers</div>
      ...
      const aud = toAud(c);
      return `<a href="/cards/onepiece/${c.slug}" style="flex:0 0 140px;background:#0e1118;border:1px solid rgba(249,115,22,.35);border-radius:10px;padding:10px;text-align:center;text-decoration:none;transition:all .2s;display:block" onmouseover="this.style.borderColor='#F97316';this.style.transform='translateY(-2px)'" onmouseout="this.style.borderColor='rgba(249,115,22,.35)';this.style.transform='none'">
        ...
        ${c.rarity ? `<div style="font-size:10px;color:#F97316;margin-bottom:3px">${c.rarity}</div>` : ''}
    ...
    const allCardsHTML = cards && cards.length ? cards.map(c => {
      const aud = toAud(c);
      return `<a href="/cards/onepiece/${c.slug}" class="card-item" data-rarity="${(c.rarity||'none').toLowerCase().replace(/ /g,'-')}" data-price="${aud}" data-change7d="${c.price_change_7d||''}" data-name="${(c.name||'').toLowerCase().replace(/"/g,'')}" data-number="${c.number||''}" style="background:#0e1118;border:1px solid #1e2235;border-radius:8px;padding:8px;text-decoration:none;text-align:center;display:block;transition:all .2s" onmouseover="this.style.borderColor='#F97316'" onmouseout="this.style.borderColor='#1e2235'">
    ...
    const ebayListingsHTML = ebayListings.length ? ebayListings.slice(0,4).map(item => {
      ...
      return `<a href="${epnUrl}" target="_blank" rel="noopener" style="background:#0e1118;border:1px solid #1e2235;border-radius:8px;padding:12px;text-decoration:none;display:flex;gap:10px;align-items:center;transition:border-color .2s" onmouseover="this.style.borderColor='#F97316'" onmouseout="this.style.borderColor='#1e2235'">
    ...
    const metaDesc = `Browse ${cards?.length||0} One Piece TCG cards from ${set.name}. View card prices in AUD, find the most valuable cards and buy on eBay AU. Updated daily.`;


    const setSchemaLD = {
      "@context": "https://schema.org", "@type": "CollectionPage",
      "name": `${set.name} Riftbound Card Prices Australia`,
      "description": `Browse all ${cards?.length||0} ${set.name} One Piece TCG cards with AUD prices and eBay AU buy links.`,
      ...
      "breadcrumb": {
        ...
        "itemListElement": [
          ...
          { "@type": "ListItem", "position": 2, "name": "One Piece TCG", "item": "https://cardsoncardsoncards.com.au/cards/onepiece" },
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .hero-eyebrow{font-size:11px;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:#F97316;margin-bottom:8px}
    ...
    .meta-badge{background:rgba(249,115,22,.08);border:1px solid rgba(249,115,22,.35);color:#F97316;padding:3px 10px;border-radius:100px;font-size:11px;font-weight:700}
    ...
    .cta-primary{background:#F97316;color:#000}
    ...
    .nav-link.active{border-color:rgba(249,115,22,.35);color:#F97316;background:rgba(249,115,22,.08)}
    ...
    .filt-btn:hover,.filt-btn.active{border-color:#F97316;color:#F97316;background:rgba(249,115,22,.08)}
  </style>
</head>
<body>
<nav>
  <div class="nav-inner">
    ...
    <div class="nav-links">
      ...
      <a href="/cards/onepiece" class="nav-link active">One Piece TCG</a>
...
<div class="wrap">
  <div class="hero">
    <div class="hero-eyebrow">One Piece TCG · Set</div>
    <h1 class="hero-title">${set.name}</h1>
    <div class="hero-meta">
      <span class="meta-badge">One Piece TCG</span>
  ...
  <div class="cta-row">
    ...
    <a href="https://www.amazon.com.au/s?k=${encodeURIComponent(set.name + ' One')}&tag=${AMAZON_TAG}" target="_blank" rel="noopener" class="cta-btn cta-secondary" style="border-color:rgba(255,153,0,.35);color:#ff9900">Search Amazon AU →</a>
  </div>
  ...
  ${ebayListingsHTML ? `<div class="section">
    ...
    <a href="${ebaySetURL}" target="_blank" rel="noopener" style="font-size:13px;color:#F97316;text-decoration:none">See all listings on eBay AU →</a>
  </div>` : ''}
  ...
  ${(() => {
    ...
    const itemsHTML = sealedItems.slice(0,4).map(p => {
      ...
      return `<a href="/cards/onepiece/${p.slug}" style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:14px;display:flex;flex-direction:column;gap:8px;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor='#FB923C'" onmouseout="this.style.borderColor='#1e2235'">
        ...
        <div style="font-size:15px;font-weight:900;color:#FB923C;font-family:'Cinzel',serif">${price}</div>
        ${low ? `<div style="font-size:11px;color:#8892b0">${low}</div>` : ''}
      </a>`;
    }).join('');
    const setNm = (set.name||'').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    return `<div class="section" style="background:#FB923C0a;border:1px solid #FB923C26;border-radius:14px;padding:22px 24px;margin-bottom:32px">
      <div style="font-size:9px;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:#FB923C;margin-bottom:8px">Sealed Product</div>
      ...
      <a href="https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent((set.name||'')+' one piece booster')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:8px;font-weight:700;font-size:12px;text-decoration:none;background:#FB923C;color:#fff">&#128722; More sealed on eBay AU &#8599;</a>
    </div>`;
  })()}
  ...
  <div style="background:#0e1118;border:1px solid #1e2235;border-radius:10px;padding:20px;font-size:13px;color:#8892b0">
    <strong style="color:#F0F2FF">About this set:</strong> One Piece TCG card prices shown in AUD, converted from USD market data at approximately 1.58x. Prices update daily via tcgapi.dev. Always check eBay AU for live Australian market pricing before buying or selling.
    <div style="margin-top:10px"><a href="/cards/onepiece" style="color:#F97316">← Back to all One Piece TCG cards</a></div>
  </div>
</div>
