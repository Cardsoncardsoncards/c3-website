// netlify/functions/sync-digimon-background.mjs
// Daily sync -- schedule: 30 2 * * * UTC
// Fetches all digimon-card-game sets + cards + prices from tcgapi.dev Pro
// Upserts into digimon_sets, digimon_cards, digimon_price_snapshots
...
const GAME_SLUG            = 'digimon-card-game';
...
export default async (req) => {
  ...
  console.log('[sync-digimon] Starting...');
  const start = Date.now();

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('[sync-digimon] Missing Supabase env vars');
    return new Response('Supabase env vars missing', { status: 500 });
  }
  if (!TCGAPI_KEY) {
    console.error('[sync-digimon] TCGAPI_KEY not set');
    return new Response('TCGAPI_KEY missing', { status: 500 });
  }

  try {
    const audRate = await getExchangeRate();
    console.log(`[sync-digimon] AUD rate: ${audRate}`);
    ...
    console.log(`[sync-digimon] Found ${allSets.length} sets`);
    ...
    for (let i = 0; i < setRows.length; i += 100) {
      await supabaseUpsert('digimon_sets', setRows.slice(i, i + 100));
    }
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-digimon] Bulk price fetch failed for set ${set.id}:`, e.message);
        }
      }
      ...
      for (const card of setCards) {
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-digimon-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: digimon_price_snapshots.card_id is a FOREIGN KEY onto
      // digimon_cards(id), so a snapshot row is the CHILD of a card row. Racing
    ...
    console.error('[sync-digimon] FATAL:', err.message);
