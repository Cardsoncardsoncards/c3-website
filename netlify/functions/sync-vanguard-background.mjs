// netlify/functions/sync-vanguard-background.mjs
// Daily sync -- schedule: 0 1 * * * UTC
// Fetches all cardfight-vanguard sets + cards + prices from tcgapi.dev Pro
// Upserts into vanguard_sets, vanguard_cards, vanguard_price_snapshots
...
const GAME_SLUG            = 'cardfight-vanguard';
...
export default async (req) => {
  ...
  console.log('[sync-vanguard] Starting...');
  const start = Date.now();

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('[sync-vanguard] Missing Supabase env vars');
    return new Response('Supabase env vars missing', { status: 500 });
  }
  if (!TCGAPI_KEY) {
    console.error('[sync-vanguard] TCGAPI_KEY not set');
    return new Response('TCGAPI_KEY missing', { status: 500 });
  }

  try {
    const audRate = await getExchangeRate();
    console.log(`[sync-vanguard] AUD rate: ${audRate}`);
    ...
    console.log(`[sync-vanguard] Found ${allSets.length} sets`);
    ...
    for (let i = 0; i < setRows.length; i += 100) {
      await supabaseUpsert('vanguard_sets', setRows.slice(i, i + 100));
    }
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-vanguard] Bulk price fetch failed for set ${set.id}:`, e.message);
        }
      }
      ...
      for (const card of setCards) {
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-vanguard-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: vanguard_price_snapshots.card_id is a FOREIGN KEY onto
      // vanguard_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('vanguard_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('vanguard_price_snapshots', chunk);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-vanguard] Upsert error for set ${set.name}:`, r.reason?.message);
      ...
      console.log(`[sync-vanguard] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }

    const elapsed = ((Date.now() - start) / 1000).toFixed(1);
    console.log(`[sync-vanguard] Done. ${totalCards} cards, ${totalSnaps} snapshots in ${elapsed}s`);
    ...
    console.error('[sync-vanguard] FATAL:', err.message);
