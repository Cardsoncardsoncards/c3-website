// netlify/functions/sync-warhammer-background.mjs
// Daily sync -- schedule: 45 11 * * * UTC
// Fetches all warhammer-old-world sets + cards + prices from tcgapi.dev Pro
// Upserts into warhammer_sets, warhammer_cards, warhammer_price_snapshots
...
const GAME_SLUG            = 'warhammer-age-of-sigmar-champions-tcg'; // task-137: tcgapi.dev renamed from 'warhammer-old-world'
...
export default async (req) => {
  ...
  console.log('[sync-warhammer] Starting...');
  const start = Date.now();

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('[sync-warhammer] Missing Supabase env vars');
    return new Response('Supabase env vars missing', { status: 500 });
  }
  if (!TCGAPI_KEY) {
    console.error('[sync-warhammer] TCGAPI_KEY not set');
    return new Response('TCGAPI_KEY missing', { status: 500 });
  }

  try {
    const audRate = await getExchangeRate();
    console.log(`[sync-warhammer] AUD rate: ${audRate}`);
    ...
    console.log(`[sync-warhammer] Found ${allSets.length} sets`);
    ...
    for (let i = 0; i < setRows.length; i += 100) {
      await supabaseUpsert('warhammer_sets', setRows.slice(i, i + 100));
    }
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-warhammer] Bulk price fetch failed for set ${set.id}:`, e.message);
        }
      }
      ...
      for (const card of setCards) {
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-warhammer-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: warhammer_price_snapshots.card_id is a FOREIGN KEY onto
      // warhammer_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('warhammer_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('warhammer_price_snapshots', chunk);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-warhammer] Upsert error for set ${set.name}:`, r.reason?.message);
      ...
      console.log(`[sync-warhammer] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }

    const elapsed = ((Date.now() - start) / 1000).toFixed(1);
    console.log(`[sync-warhammer] Done. ${totalCards} cards, ${totalSnaps} snapshots in ${elapsed}s`);
    ...
    console.error('[sync-warhammer] FATAL:', err.message);
