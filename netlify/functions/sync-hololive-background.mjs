...
// Group A sync (Mon/Wed/Fri/Sun) - schedule: 15 0 * * 1,3,5,0 UTC
// Fetches all hololive-official-card-game sets + cards + prices from tcgapi.dev Pro
...
const GAME_SLUG            = 'hololive-official-card-game';
...
export default async (req) => {
  console.log('[sync-hololive] Starting (background function)...');
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-hololive] Bulk price error set ${set.id}:`, e.message);
        }
      }
