// netlify/functions/sync-hololive-background.mjs
// Daily sync -- schedule: 30 11 * * * UTC
// Fetches all hololive-trading-card-game sets + cards + prices from tcgapi.dev Pro
// Upserts into hololive_sets, hololive_cards, hololive_price_snapshots
...
const GAME_SLUG            = 'hololive-official-card-game'; // task-137: tcgapi.dev renamed from 'hololive-trading-card-game'
...
export default async (req) => {
  ...
  console.log('[sync-hololive] Starting...');
  const start = Date.now();

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('[sync-hololive] Missing Supabase env vars');
    return new Response('Supabase env vars missing', { status: 500 });
  }
  if (!TCGAPI_KEY) {
    console.error('[sync-hololive] TCGAPI_KEY not set');
    return new Response('TCGAPI_KEY missing', { status: 500 });
  }

  try {
    const audRate = await getExchangeRate();
    console.log(`[sync-hololive] AUD rate: ${audRate}`);
    ...
    console.log(`[sync-hololive] Found ${allSets.length} sets`);
    ...
    for (let i = 0; i < setRows.length; i += 100) {
      await supabaseUpsert('hololive_sets', setRows.slice(i, i + 100));
    }
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-hololive] Bulk price fetch failed for set ${set.id}:`, e.message);
        }
      }
      ...
      for (const card of setCards) {
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-hololive-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: hololive_price_snapshots.card_id is a FOREIGN KEY onto
      // hololive_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('hololive_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('hololive_price_snapshots', chunk);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-hololive] Upsert error for set ${set.name}:`, r.reason?.message);
      ...
      console.log(`[sync-hololive] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }

    const elapsed = ((Date.now() - start) / 1000).toFixed(1);
    console.log(`[sync-hololive] Done. ${totalCards} cards, ${totalSnaps} snapshots in ${elapsed}s`);
    ...
    console.error('[sync-hololive] FATAL:', err.message);
