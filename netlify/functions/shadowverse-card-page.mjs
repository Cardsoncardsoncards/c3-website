...
<div class="wrap">
  ...
  <div class="cta-row">
    ...
    <a href="https://www.amazon.com.au/s?k=${encodeURIComponent(set.name + ' Shadowverse Evolve')}&tag=${AMAZON_TAG}" target="_blank" rel="noopener" class="cta-btn cta-secondary" style="border-color:rgba(255,153,0,.35);color:#ff9900">Amazon AU →</a>
  </div>
...
export default async (req) => {
  ...
  try {
    ...
    const snapshots = await supabaseGet(`shadowverse_price_snapshots?card_id=eq.${encodeURIComponent(card.id)}&snapshot_date=gte.${new Date(Date.now()-90*86400000).toISOString().slice(0,10)}&order=snapshot_date.asc&limit=400&select=snapshot_date,price_aud,market_price`).catch(() => []);
...
<div class="card-hero">
  ...
  <div class="card-details">
    ...
    ${followBlockHtml({ game: 'shadowverse', slug: card.slug, cardName: card.name, printingId: card.tcgplayer_id })}
...
${viewTrackingScript('shadowverse', card.slug)}
</body>
</html>`;
