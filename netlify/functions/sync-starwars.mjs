...
export default async (req) => {
  ...
  try {
    ...
    for (const set of allSets) {
      ...
      setCount++;
      if (setCount % 10 === 0) {
        console.log(`[sync-starwars] Progress: ${setCount} new sets, ${skippedCount} skipped, ${totalCards} cards so far`);
      }
      const setCards = [];
      let cardPage = 1;
    ...
    console.log(`[sync-starwars] Done. ${setCount} new sets, ${skippedCount} skipped. ${totalCards} cards, ${totalSnaps} snapshots. ${elapsed}s`);
