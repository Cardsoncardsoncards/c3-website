...
export default async (req) => {
  ...
  const [_psr0, _psr1] = await Promise.allSettled([
    supabaseGet('dragonball_sets?order=release_date.desc&limit=300&select=id,name,slug,abbreviation,card_count,release_date'),
    supabaseGet('dragonball_cards?order=market_price.desc&market_price=gt.0&image_url=not.is.null&limit=24&select=slug,name,number,image_url,market_price,price_aud,rarity,set_name')
  ]);
  ...
  const carouselHTML = topCards.length ? topCards.map(c => `
    <a href="/cards/dragonball/${c.slug}" class="carousel-card">
...
<head>
  ...
  <meta name="description" content="Browse ${sets.length}+ Dragon Ball Super sets. Live AUD card prices, eBay AU buy links. Australia's Dragon Ball TCG price guide updated daily.">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/dragonball">
  ...
  <style>
    :root{--bg:#0A0C14;--bg2:#111420;--bg3:#181d2e;--gold:#C9A84C;--accent:#F97316;--text:#e8eaf0;--text2:#9ba3c4;--border:#242840;--radius:12px;--silver:#A0A8C0}
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    body::before{content:'';position:fixed;inset:0;pointer-events:none;z-index:0;background:radial-gradient(ellipse 70% 40% at 50% 0%,rgba(249,115,22,.05),transparent 60%)}
    ...
    .carousel-card:hover{border-color:var(--accent);transform:translateY(-4px);box-shadow:0 8px 20px rgba(249,115,22,.12);text-decoration:none}
    ...
    .set-tile:hover{border-color:var(--accent);background:rgba(249,115,22,.04);text-decoration:none;transform:translateX(2px)}
...
<div class="hero fade-up">
  ...
  <p class="hero-sub">Dragon Ball Super Card Game prices in AUD. Browse by set, find the most valuable cards, and buy on eBay AU. Updated daily.</p>
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length || '101'}</div><div class="stat-label">Sets</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=dragon+ball+super+tcg+cards&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:linear-gradient(135deg,#dc2626,var(--accent));color:#fff">&#128722; Shop Dragon Ball on eBay &#8599;</a>
  <a href="/tracker.html" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128203; Free Tracker</a>
  <a href="/quizzes/dragonball-character" class="quick-link" style="background:rgba(249,115,22,.08);border-color:rgba(249,115,22,.3);color:var(--accent)">&#127919; Which DBS Character Are You? &#8594;</a>
  <a href="/blog/dragon-ball-super-card-game-beginners-guide-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128214; Beginners Guide &#8594;</a>
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header"><div class="section-title">Search Dragon Ball Cards</div></div>
    <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
      <input type="text" id="card-search" placeholder="Card name e.g. Goku, Vegeta, Gohan..." onkeyup="if(event.key==='Enter')searchCard()">
  ...
  <div class="section fade-up fade-up-3">
    ...
    <input type="text" id="set-search" placeholder="Search sets e.g. Fearsome Rivals, Prismatic Clash..." oninput="filterSets(this.value)" style="margin-bottom:12px">
    <div id="set-list" class="set-grid">${setListHTML}</div>
  </div>
  <div style="background:rgba(249,115,22,.04);border:1px solid rgba(249,115,22,.15);border-radius:var(--radius);padding:22px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px">
    <div>
...
<footer>
  ...
  <p style="margin-top:6px;font-size:11px;opacity:.5">Not affiliated with Bandai or Toei Animation. Prices converted to AUD at approximately 1.58.</p>
</footer>
<script>
let activeAZ = 'All';
