...
const GAME       = 'dragonball';
const GAME_LABEL = 'Dragon Ball Super TCG';
const ACCENT     = '#EAB308';
const ACCENT_RGB = '234,179,8';
const EMOJI      = '&#128165;';
const SET_PH     = 'Blazing Aura, Cross Spirits, Critical Blow...';
const CANONICAL  = 'https://cardsoncardsoncards.com.au/cards/dragonball';
...
export default async (req) => {
  ...
  const [setsRes, cardsRes, gainersRes, losersRes] = await Promise.allSettled([
    supabaseGet('dragonball_sets?order=release_date.desc.nullslast&limit=300&select=id,name,slug,release_date,card_count'),
    supabaseGet('dragonball_cards?order=price_aud.desc&price_aud=gt.0&image_url=not.is.null&rarity=not.is.null&rarity=neq.None&limit=24&select=slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at'),
    supabaseGet('dragonball_cards?order=price_change_7d.desc&price_change_7d=gt.5&price_aud=gt.2&price_change_7d=lt.5000&image_url=not.is.null&limit=5&select=slug,name,image_url,price_aud,price_change_7d,set_name'),
    supabaseGet('dragonball_cards?order=price_change_7d.asc&price_change_7d=lt.-5&price_aud=gt.2&image_url=not.is.null&limit=5&select=slug,name,image_url,price_aud,price_change_7d,set_name')
  ]);
  ...
  function carouselCard(c, suffix) {
    ...
    const ebay  = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name+' dragon ball super card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<div class="carousel-card"><a href="/cards/dragonball/${esc(c.slug)}" style="display:block;text-decoration:none"><div class="carousel-img-wrap"><img src="${esc(c.image_url)}${suffix}" alt="${esc(c.name).replace(/"/g,'&quot;')}" loading="eager" onerror="this.onerror=null;this.src='/card-placeholder.svg';this.style.opacity='1'"></div><div class="carousel-name">${esc(c.name)}</div>${c.rarity?`<div class="carousel-rarity">${esc(c.rarity)}</div>`:''}<div class="carousel-price">${price}</div></a><div class="carousel-buy-row"><a href="${ebay}" target="_blank" rel="noopener" onclick="gtag('event','ebay_click',{'event_category':'affiliate','event_label':'dragonball-hub'})" class="carousel-buy-btn">Buy eBay &#8599;</a></div></div>`;
  }
  ...
  const setListHTML = sets.length ? sets.map(s => {
    ...
    return `<a href="/cards/dragonball/sets/${encodeURIComponent(s.slug||s.id)}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}"><span class="set-tile-name">${esc(name)}${newBadge}</span><span class="set-tile-meta">${year}${s.card_count?' &middot; '+s.card_count+' cards':''}</span></a>`;
...
<head>
  ...
  <meta name="description" content="Browse ${sets.length||'60'}+ Dragon Ball Super TCG sets. Live AUD card prices, eBay AU buy links. Australia's most complete Dragon Ball Super price guide, updated daily.">
  <link rel="canonical" href="${CANONICAL}">
  ${hubSchemaHtml({ game: 'dragonball', label: "Dragon Ball Super", name: "Dragon Ball Card Prices Australia", description: `Browse ${sets.length||'60'}+ Dragon Ball Super TCG sets. Live AUD card prices, eBay AU buy links. Australia's most complete Dragon Ball Super price guide, updated daily.` })}
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="Dragon Ball Card Prices Australia | Cards on Cards on Cards">
  <meta property="og:description" content="${sets.length||'60'}+ Dragon Ball sets. Live AUD prices and eBay AU buy links updated daily.">
...
<div class="hero fade-up">
  <div class="hero-eyebrow">Card Vault -- Dragon Ball Super TCG</div>
  <h1>Dragon Ball Super Card Prices <span>in Australia</span></h1>
  <p class="hero-sub">Dragon Ball Super TCG card prices in AUD. Browse by set or search by card name. eBay AU buy links updated daily.</p>
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length||'60'}</div><div class="stat-label">Sets</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=dragon+ball+super+cards&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&campid=${EPN_CAMPID}&toolid=10001&mkevt=1" target="_blank" rel="noopener" onclick="gtag('event','ebay_click',{'event_category':'affiliate','event_label':'dragonball-hub'})" class="quick-link" style="background:linear-gradient(135deg,#92400e,${ACCENT});color:#000">&#128722; Shop Dragon Ball Super on eBay &#8599;</a>
...
${topCards.length ? `<section class="carousel-section fade-up fade-up-2">
  ...
  <div class="carousel-title">Top Dragon Ball Super Cards by Price (AUD)</div>
...
${hasMovers ? `<div style="margin-bottom:28px">
  ...
  <p style="color:var(--text2);font-size:13px;margin-bottom:8px">Biggest price movers across all Dragon Ball Super cards in the last 7 days.</p>
  ...
  <div class="section fade-up fade-up-3">
    <div style="text-align:center;margin-bottom:16px">
      <h2 style="font-family:'Cinzel',serif;font-size:24px;font-weight:700;color:var(--text);margin-bottom:6px">Browse Every Dragon Ball Super Set</h2>
      <p style="font-size:13px;color:var(--text2)">${sets.length||'60'}+ sets. Pick a letter or search to explore.</p>
    </div>
  ...
  <div style="margin-bottom:28px">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px">
      ...
      <a href="/ev-calculator.html" class="guide-card"><div class="guide-title">Booster Box EV Calculator</div><div class="guide-desc">Is your Dragon Ball Super box worth opening? Calculate expected value before you crack it.</div></a>
  ...
  <div style="background:rgba(${ACCENT_RGB},.04);border:1px solid rgba(${ACCENT_RGB},.15);border-radius:var(--radius);padding:22px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px">
    <div>
      <div style="font-size:16px;font-weight:700;color:var(--text);margin-bottom:5px">Track Your Dragon Ball Super Collection</div>
...
<footer>
  ...
  <div style="margin-bottom:10px">
    <a href="/">Home</a><a href="/blog">Blog</a><a href="/calendar">Calendar</a><a href="/account">Account</a><a href="/tracker">Tracker</a><a href="/cards">Card Prices</a><a href="/compare">Compare</a><a href="/market">Market</a><a href="/tools">Tools</a><a href="/contact">Contact</a><a href="/legal">Legal</a><a href="https://www.ebay.com.au/str/cardsoncardsoncards?mkcid=1&amp;mkrid=705-53470-19255-0&amp;campid=5339146789&amp;customid=Footer&amp;toolid=10001&amp;mkevt=1" target="_blank" rel="noopener" onclick="gtag('event','ebay_click',{'event_category':'affiliate','event_label':'footer'})">eBay</a><a href="https://blasdigital.etsy.com" target="_blank" rel="noopener">D&amp;D Tools on Etsy &#8599;</a><br><a href="/cards/dragonball">Dragon Ball</a><a href="/cards/pokemon">Pokemon</a><a href="/cards/mtg">MTG</a><a href="/cards/yugioh">Yu-Gi-Oh</a><a href="/cards/onepiece">One Piece</a>
  </div>
  <p>&#169; 2026 Cards on Cards on Cards &middot; cardsoncardsoncards.com.au</p>
  <p style="margin-top:6px;font-size:11px;opacity:.5">Affiliate disclosure: this site earns commissions from eBay AU and Amazon AU purchases made through affiliate links at no extra cost to you. Not affiliated with Bandai. USD prices converted to AUD at approximately ${audRate.toFixed(2)}.</p>
...
export const config = { path: '/cards/dragonball' };
