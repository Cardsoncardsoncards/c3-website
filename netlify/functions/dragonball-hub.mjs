...
const GAME_LABEL  = 'Dragon Ball Super';
const ACCENT      = '#F97316';
const ACCENT_RGB  = '249,115,22';
const EMOJI       = '&#128007;';
const SEARCH_PH   = 'Goku, Vegeta, Gohan, Frieza...';
const SET_PH      = 'Fearsome Rivals, Prismatic Clash...';
...
export default async (req) => {
  ...
  const [setsResult, topCardsResult] = await Promise.allSettled([
    supabaseGet('dragonball_sets?order=release_date.desc&limit=200&select=id,name,slug,abbreviation,release_date,card_count'),
    supabaseGet('dragonball_cards?order=market_price.desc&market_price=gt.0&image_url=not.is.null&limit=24&select=slug,name,image_url,market_price,price_aud,rarity,set_name,updated_at')
  ]);
  ...
  const carouselHTML = topCards.map(c => {
    ...
    const ebaySearch = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(c.name + ' dragon ball super card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
    return `<a href="/cards/dragonball/${c.slug}" class="carousel-card">
  ...
  const setListHTML = sets.length
    ? sets.map(s => {
        ...
        const setId = encodeURIComponent(s.slug || s.id);
        return `<a href="/cards/dragonball/sets/${setId}" class="set-tile" data-name="${esc(name.toLowerCase())}" data-letter="${letterKey}">
...
<head>
  ...
  <title>Dragon Ball Super Card Game Prices Australia | AUD Updated Daily | C3</title>
  <meta name="description" content="Browse 100+ Dragon Ball Super sets. Live AUD card prices, eBay AU buy links. Australia's Dragon Ball TCG price guide updated daily.">
  <link rel="canonical" href="${CANONICAL}">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="Dragon Ball Super Card Prices Australia | Cards on Cards on Cards">
  <meta property="og:description" content="100+ Dragon Ball Super sets with live AUD prices and eBay AU buy links updated daily.">
...
<nav>
  <div class="nav-inner">
    ...
    <div class="nav-links">
      ...
      <a href="/cards/dragonball" class="nav-link" style="color:#F97316;border-color:#F9731680;background:#F9731614">Dragon Ball</a>
...
<div class="hero fade-up">
  <div class="hero-eyebrow">Card Vault -- Dragon Ball Super</div>
  <h1>Dragon Ball Super <span>Card Prices AU</span></h1>
  <p class="hero-sub">Dragon Ball Super Card Game prices in AUD. Browse by set, find the most valuable cards, and buy on eBay AU.</p>
  <div class="stat-bar">
    <div class="stat-item"><div class="stat-num">${sets.length || '102'}</div><div class="stat-label">Sets</div></div>
...
<div class="quick-links fade-up fade-up-1">
  <a href="https://www.ebay.com.au/sch/i.html?_nkw=dragon+ball+super+tcg+cards&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=5339146789&toolid=10001&mkevt=1" target="_blank" rel="noopener" class="quick-link" style="background:linear-gradient(135deg,#dc2626,#F97316);color:#fff">&#128722; Shop Dragon Ball on eBay &#8599;</a>
  ...
  <a href="/blog/dragon-ball-super-booster-boxes-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128230; Best DBS Boxes &#8594;</a>
  <a href="/blog/dragon-ball-super-card-game-beginners-guide-australia/" class="quick-link" style="background:rgba(var(--accent-rgb),.08);border-color:rgba(249,115,22,.3);color:var(--accent)">&#128214; Beginners Guide &#8594;</a>
  <a href="/blog/how-to-sell-dragon-ball-super-cards-australia/" class="quick-link" style="background:var(--bg2);border-color:var(--border);color:var(--text)">&#128200; How to Sell DBS Cards &#8594;</a>
</div>
...
<div class="wrap">
  <div class="section fade-up fade-up-2">
    <div class="section-header">
      <div class="section-title">Search Dragon Ball Super Cards</div>
    </div>
  ...
  <div class="section fade-up fade-up-3">
    <div class="section-header">
      <div class="section-title">Browse ${sets.length || '102'} Sets</div>
  ...
  <div style="background:rgba(var(--accent-rgb),.04);border:1px solid rgba(var(--accent-rgb),.15);border-radius:var(--radius);padding:22px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px">
    <div>
      <div style="font-size:16px;font-weight:700;color:var(--text);margin-bottom:5px">Track Your Dragon Ball Collection</div>
...
<footer>
  ...
  <p style="margin-top:6px;font-size:11px;opacity:.5">Affiliate disclosure: this site earns commissions from eBay AU and Amazon AU purchases made through affiliate links at no extra cost to you. Not affiliated with Bandai or Toei Animation. USD prices converted to AUD at approximately 1.58.</p>
</footer>
...
async function searchCard() {
  ...
  try {
    const res = await fetch('/api/compare-search?q=' + encodeURIComponent(q) + '&game=dragonball&limit=24');
    ...
    results.innerHTML = cards.map(c => {
      ...
      return '<a href="/cards/dragonball/' + c.slug + '" style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center;display:block;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'var(--border)\'">'
