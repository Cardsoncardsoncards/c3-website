// netlify/functions/sync-battlespiritssaga-background.mjs
// Daily sync -- schedule: 15 10 * * * UTC
...
const GAME_SLUG            = 'battle-spirits-saga';
...
export default async (req) => {
  ...
  console.log('[sync-battlespiritssaga] Starting...');
  const start = Date.now();

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('[sync-battlespiritssaga] Missing Supabase env vars');
    return new Response('Supabase env vars missing', { status: 500 });
  }
  if (!TCGAPI_KEY) {
    console.error('[sync-battlespiritssaga] TCGAPI_KEY not set');
    return new Response('TCGAPI_KEY missing', { status: 500 });
  }

  try {
    const audRate = await getExchangeRate();
    console.log(`[sync-battlespiritssaga] AUD rate: ${audRate}`);
    ...
    console.log(`[sync-battlespiritssaga] Found ${allSets.length} sets`);
    ...
    for (let i = 0; i < setRows.length; i += 100) {
      await supabaseUpsert('battlespiritssaga_sets', setRows.slice(i, i + 100));
    }
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-battlespiritssaga] Bulk price fetch failed for set ${set.id}:`, e.message);
        }
      }
      ...
      for (const card of setCards) {
        ...
        if (marketPrice && marketPrice >= 0.5) {
          snapRows.push({
            card_id:          card.id,
            snapshot_date:    today,
            source:           'sync-battlespiritssaga-background',
            market_price:     marketPrice,
            low_price:        lowPrice,
      ...
      // they are not independent: battlespiritssaga_price_snapshots.card_id is a FOREIGN KEY onto
      // battlespiritssaga_cards(id), so a snapshot row is the CHILD of a card row. Racing
      ...
      try {
        for (let i = 0; i < cardRows.length; i += 200) {
          ...
          await supabaseUpsert('battlespiritssaga_cards', chunk);
      ...
      try {
        for (let i = 0; i < snapRows.length; i += 500) {
          ...
          await supabaseUpsertSnapshots('battlespiritssaga_price_snapshots', chunk);
      ...
      for (const r of results) {
        if (r.status === 'rejected') {
          console.error(`[sync-battlespiritssaga] Upsert error for set ${set.name}:`, r.reason?.message);
      ...
      console.log(`[sync-battlespiritssaga] ${set.name}: ${cardsWritten}/${cardRows.length} cards, ${snapsWritten}/${snapRows.length} snapshots written`);
    }

    const elapsed = ((Date.now() - start) / 1000).toFixed(1);
    console.log(`[sync-battlespiritssaga] Done. ${totalCards} cards, ${totalSnaps} snapshots in ${elapsed}s`);
    ...
    console.error('[sync-battlespiritssaga] FATAL:', err.message);
