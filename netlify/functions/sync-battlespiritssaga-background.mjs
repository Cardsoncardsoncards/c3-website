...
// Group B sync (Tue/Thu/Sat) - schedule: 30 0 * * 3,5,0 UTC
...
export default async (req) => {
  console.log('[sync-battlespiritssaga] Starting (background function)...');
    ...
    for (const set of allSets) {
      ...
      for (let i = 0; i < cardIds.length; i += 500) {
        ...
        try {
          ...
          console.error(`[sync-battlespiritssaga] Bulk price error set ${set.id}:`, e.message);
        }
      }
