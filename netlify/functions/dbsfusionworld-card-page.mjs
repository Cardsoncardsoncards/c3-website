...
export default async (req) => {
  ...
  try {
    ...
    const snapshots = await supabaseGet(`dbsfusionworld_price_snapshots?card_id=eq.${encodeURIComponent(card.id)}&snapshot_date=gte.${new Date(Date.now()-90*86400000).toISOString().slice(0,10)}&order=snapshot_date.asc&limit=400&select=snapshot_date,price_aud,market_price`).catch(() => []);
...
${followBlockHtml({ game: 'dbsfusionworld', slug: card.slug, cardName: card.name, printingId: card.tcgplayer_id })}
