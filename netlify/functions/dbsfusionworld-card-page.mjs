...
// DBS Fusion World individual card pages with AUD pricing and affiliate links
// Built: 20 May 2026
...
const GAME_LABEL        = 'DBS Fusion World';
const GAME_KEY          = 'dbsfusionworld';
...
function graceful404(slug) {
  ...
  const ebayUrl = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent(cardName+' dragon ball fusion world card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
  return `<!DOCTYPE html>
<html lang="en-AU">
<head>
  <meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
  <title>${esc(cardName)} | DBS Fusion World | Cards on Cards on Cards</title>
  <meta name="robots" content="noindex">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <style>*{box-sizing:border-box;margin:0;padding:0}body{background:#0A0C14;color:#F0F2FF;font-family:'DM Sans',sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px;text-align:center}.wrap{max-width:440px}h1{color:#FF6B35;font-size:24px;margin-bottom:12px;font-family:'Cinzel',serif}p{color:#8892b0;font-size:14px;margin-bottom:24px;line-height:1.6}.btn{display:inline-block;background:#FF6B35;color:#000;padding:12px 24px;border-radius:8px;font-weight:700;text-decoration:none;font-size:14px;margin:4px}.btn-sec{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);color:#F0F2FF}</style>
</head>
<body><div class="wrap">
  <h1>${esc(cardName)}</h1>
  <p>We could not find that DBS Fusion World card. It may not be in our database yet.</p>
  <a href="${ebayUrl}" target="_blank" rel="noopener" class="btn">Search eBay AU &#8599;</a>
  <a href="/cards/dbsfusionworld" class="btn btn-sec">Browse DBS Fusion World</a>
</div></body></html>`;
}
...
export default async (req) => {
  ...
  const slug    = url.pathname.replace(/^\/cards\/dbsfusionworld\//, '').replace(/\/$/, '');
  ...
  if (!slug || slug === '') {
    return Response.redirect('https://cardsoncardsoncards.com.au/cards/dbsfusionworld', 302);
  }

  // Parallel: fetch card + set info
  const [cardResult] = await Promise.allSettled([
    supabaseGet(`dbsfusionworld_cards?slug=eq.${encodeURIComponent(slug)}&limit=1&select=*`)
  ]);
  ...
  const [relatedResult, sealedResult, setResult, prevNextResult] = await Promise.allSettled([
    ...
    card.set_id
      ? supabaseGet(`dbsfusionworld_cards?set_id=eq.${card.set_id}&slug=neq.${encodeURIComponent(slug)}&order=market_price.desc.nullslast&limit=12&select=slug,name,image_url,market_price,price_aud,rarity&$dbsfusionworld_filter`)
      : Promise.resolve([]),
    // Sealed products for this set
    card.set_id
      ? supabaseGet(`dbsfusionworld_cards?set_id=eq.${card.set_id}&select=slug,name,image_url,market_price,price_aud,low_price&order=market_price.desc.nullslast&limit=6`)
      : Promise.resolve([]),
    // Set info
    card.set_id
      ? supabaseGet(`dbsfusionworld_sets?id=eq.${card.set_id}&limit=1&select=id,name,slug,release_date`)
      : Promise.resolve([]),
    // Prev/next by card number
    card.set_id
      ? supabaseGet(`dbsfusionworld_cards?set_id=eq.${card.set_id}&select=slug,name,number&order=number.asc&limit=500`)
      : Promise.resolve([])
  ]);
  ...
  // Social share
  const pageUrl   = encodeURIComponent(`https://cardsoncardsoncards.com.au/cards/dbsfusionworld/${slug}`);
  ...
  const shareBar  = `<button onclick="navigator.clipboard.writeText('https://cardsoncardsoncards.com.au/cards/dbsfusionworld/${slug}').then(()=>{this.textContent='Copied!';setTimeout(()=>this.textContent='Discord',1500)})" style="padding:6px 12px;background:#5865F2;color:#fff;border:none;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer">Discord</button>
    ...
    <button onclick="navigator.clipboard.writeText('https://cardsoncardsoncards.com.au/cards/dbsfusionworld/${slug}').then(()=>{this.textContent='Copied!';setTimeout(()=>this.textContent='Copy Link',1500)})" style="padding:6px 12px;background:#111420;border:1px solid #242840;color:#e8eaf0;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer">Copy Link</button>`;  const priceDisplay = priceAud ? `AU$${priceAud.toFixed(2)}` : 'Price TBC';
  const customAttrs  = parseCustomAttrs(card.custom_attributes);

  const ebayCardUrl  = `https://www.ebay.com.au/sch/i.html?_nkw=${encodeURIComponent((card.name||slug.replace(/-/g,' '))+' dragon ball fusion world card')}&_sacat=183454&mkcid=1&mkrid=705-53470-19255-0&siteid=15&campid=${EPN_CAMPID}&toolid=10001&mkevt=1`;
  const amazonUrl    = `https://www.amazon.com.au/s?k=${encodeURIComponent((card.name||'')+' dragon ball fusion world card')}&tag=${AMAZON_TAG}`;
  const setPageUrl   = set?.slug ? `/cards/dbsfusionworld/sets/${esc(set.slug)}` : `/cards/dbsfusionworld`;
  ...
  const relatedHTML = relatedCards.length ? relatedCards.map(c => {
    ...
    return `<a href="/cards/dbsfusionworld/${c.slug}" style="background:#111420;border:1px solid #242840;border-radius:8px;padding:8px;text-align:center;display:block;text-decoration:none;transition:border-color .2s" onmouseover="this.style.borderColor='#FF6B35'" onmouseout="this.style.borderColor='#242840'">
      ...
      ${p > 0 ? `<div style="font-size:11px;color:#FF6B35;font-weight:700">AU$${p.toFixed(0)}</div>` : ''}
    </a>`;
  }).join('') : '';
...
<head>
  ...
  <title>${esc(card.name)} Price Australia | ${esc(card.set_name||'DBS Fusion World')} | Cards on Cards on Cards</title>
  <meta name="description" content="${esc(card.name)} from ${esc(card.set_name||'DBS Fusion World')}. ${priceDisplay} AUD. Buy on eBay AU. Australia's DBS Fusion World price guide.">
  <link rel="canonical" href="https://cardsoncardsoncards.com.au/cards/dbsfusionworld/${esc(slug)}">
  <link rel="icon" type="image/png" href="/c3logo.png">
  <meta property="og:title" content="${esc(card.name)} | DBS Fusion World | Cards on Cards on Cards">
  ...
  <meta property="og:url" content="https://cardsoncardsoncards.com.au/cards/dbsfusionworld/${esc(slug)}">
  <script type="application/ld+json">{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"Home","item":"https://cardsoncardsoncards.com.au"},{"@type":"ListItem","position":2,"name":"DBS Fusion World","item":"https://cardsoncardsoncards.com.au/cards/dbsfusionworld"},{"@type":"ListItem","position":3,"name":"${(card.name||'').replace(/"/g,'&quot;')}","item":"https://cardsoncardsoncards.com.au/cards/dbsfusionworld/${esc(slug)}"}]}</script>
  ...
  <style>
    :root{--bg:#0A0C14;--bg2:#111420;--bg3:#181d2e;--accent:#FF6B35;--accent-rgb:255,107,53;--gold:#C9A84C;--text:#e8eaf0;--text2:#9ba3c4;--border:#242840;--radius:12px}
...
<nav>
  <div class="nav-inner">
    ...
    <div class="nav-links">
      ...
      <a href="/cards/dbsfusionworld" class="nav-link" style="color:#FF6B35;border-color:#FF6B3580;background:#FF6B3514">DBS Fusion World</a>
...
<div class="wrap">
  <div class="breadcrumb">
    ...
    <a href="/cards/dbsfusionworld">DBS Fusion World</a> &rsaquo;
    ...
    <div class="card-info">
      ...
      <div class="price-block">
        ...
        <div class="cta-row">
          ...
          <a href="/compare?cards=dbsfusionworld:${esc(slug)}" class="btn btn-ghost">&#9878; Compare</a>
        </div>
      </div>

      <div class="section">
        <div class="section-title">Where to Buy ${esc(card.name)} in Australia</div>
        <p style="font-size:13px;color:var(--text2);line-height:1.6">${esc(card.name)} is a DBS Fusion World card from ${esc(card.set_name||'this set')}${card.rarity ? `, classified as ${esc(card.rarity)}` : ''}. The estimated price is ${priceDisplay} AUD, converted from US market data and updated daily. Buy directly on eBay AU for the best local pricing and fast shipping.</p>
      </div>
    </div>
  </div>
  ...
  ${sealedCards.length ? `<div style="margin-bottom:24px">
    ...
    <div style="display:flex;gap:10px;overflow-x:auto;padding-bottom:8px;scrollbar-width:none">
      ${sealedCards.map(p => {
        ...
        return `<a href="/cards/dbsfusionworld/${p.slug}" style="flex-shrink:0;width:150px;background:#111420;border:1px solid #242840;border-radius:8px;padding:10px;text-decoration:none;display:block">
  ...
  <div style="background:rgba(var(--accent-rgb),.04);border:1px solid rgba(var(--accent-rgb),.15);border-radius:var(--radius);padding:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px;margin-bottom:20px">
    <div>
      <div style="font-size:14px;font-weight:700;color:var(--text);margin-bottom:4px">Track Your DBS Fusion World Collection</div>
...
<footer>
  <div style="margin-bottom:8px">
    <a href="/">Home</a><a href="/cards">Card Vault</a><a href="/cards/dbsfusionworld">DBS Fusion World</a>
...
document.querySelectorAll('a[href*="ebay"]').forEach(a => a.addEventListener('click', () => {
  if (typeof gtag !== 'undefined') gtag('event','ebay_card_click',{card_name:'${(card.name||'').replace(/'/g,"\\'")}',game:'dbsfusionworld'});
}));
</script>
...
export const config = { path: '/cards/dbsfusionworld/:slug+' };
